...
- Respect this project's own `CLAUDE.md` boundaries (module/package boundaries, its own I/O-boundary
...
2. **Ground it in the repo, in parallel.** Dispatch **2–4 read-only subagents** (`Explore`) — one per
   ...
   - Existing patterns the phase should imitate rather than invent (the nearest sibling component,
     the nearest parser, the nearest schema, the nearest test).
...
| 2 | `Behaviour` | Functionality & edge cases · Data model & contract · Persistence & migration · Concurrency & cancellation |
...
- **Budget: 6–14 questions across 2–4 AskUserQuestion calls** (≤4 per call). Toward 6 for one area on
  a small phase; toward 14 for four-plus areas on a large phase. **Never fewer than 6.**
...
| Visual design & theming | Which existing tokens/classes, density at both row heights, both light and dark, what a theme change does to it |
| Accessibility & keyboard | The chord (and it must not collide — check wherever this project's own keybinding registry lives), focus order, focus trap, aria roles/labels, what a screen reader announces on the async bits |
| Empty / loading / error states | The literal copy for each of empty vs loading vs error vs "too many results", where truncation is made visible instead of silent, what a failure envelope renders as |
| Functionality & edge cases | The specific inputs that break it — empty/degenerate input, unusual encodings, huge payloads, concurrent edits, whatever this domain's own edge cases are |
| Data model & contract | The exact schema fields and their optionality, the channel/endpoint name and its prefix, request/response shapes, whether it is streamed, the migration for the persisted store's `version` |
| Persistence & migration | What is persisted, in which store, under what serialization, and the migration arm for the version bump |
| Concurrency & cancellation | What supersedes what, whether it goes through a write queue or lock, what an abort does to a half-consumed stream, what happens on context switch mid-flight |
| Performance & scale | The number that matters (rows, files, items, bytes), where virtualization starts, what is memoized/cached and keyed on what, the budget a perf spec asserts, if one exists |
| Testing & verification | The layer (unit vs integration vs e2e), the named spec file, the exact assertion, the fixture, what a mock/stub must learn |
| Observability & diagnostics | What is logged, what surfaces in a diagnostics UI if this project has one, what a failure looks like to a user vs in the console |
