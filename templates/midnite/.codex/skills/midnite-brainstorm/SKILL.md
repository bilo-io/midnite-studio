...
- Phase plans live in **`.midnite/tasks/phases/`**, one file per phase: `phase-N-<slug>.md`, with `_INDEX.md`, `done.md` (append-only completed log) and `outstanding.md` (deferred scope) one level up in `.midnite/tasks/`. If this project has its own design doc (an architecture/plan file), treat it as the source of truth; the conventions are otherwise the house style of the existing phase docs + the comments in `_INDEX.md`.
- Match the **house style** of the existing phase docs (read a couple first — pick two or three recent ones under `.midnite/tasks/phases/`): a `# Phase N — Title` heading; a short framing paragraph up top (what this builds on + scope guardrails); a **Deliverables** checklist (grouped into lettered Themes if the phase is parallelisable); a **Verification** checklist; references to any prior art already in this repo that solves a similar problem.
- Respect this project's own architecture boundaries and conventions — whatever `CLAUDE.md` (and its `AGENTS.md`/`GEMINI.md` mirrors, if present) documents. Proposals must fit the architecture, not fight it.
...
The scan is read-heavy — **every** `.midnite/tasks/phases/phase-*.md` (potentially a couple of dozen files) plus git/PR state — so **delegate it to a single read-only subagent** (e.g. the `Explore` agent) and keep the raw file dumps out of this conversation. You only need the digest it returns to render the table.
...
2. **Print an overview table first thing** — before any question — from the digest, covering all phases:
   ...
   |-------|--------|---------|------|-----------|
   | 0 · Scaffold | ✅ | Monorepo skeleton, toolchain pinned | 100% | — |
   | 8 · Office fidelity | 🔄 | Sprites + movement in; assets pending | 60% | PR #21 |
...
2. If `$ARGUMENTS` seeded a topic, make the 5 proposals **variations/angles on that topic**; otherwise spread them across the product's own surfaces (whatever this project's main modules/features are — its UI, its core engine/data layer, its infra/packaging, its testing, its docs, new surfaces).
...
4. Tell the user the file path + the commit/push result, give a 2–3 line recap, and suggest that `/midnite-exec` is how they'd later pick up a slice of it.
