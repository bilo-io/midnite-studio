...
## Respect
- `CLAUDE.md` (and its `AGENTS.md`/`GEMINI.md` mirrors, if present) = this project's own conventions — architecture/module boundaries, commit style, pre-push gate. Re-read the relevant bits before coding. If this project has its own design doc, that's the source of truth for its architecture.
- `.midnite/tasks/` = tracker: **`_INDEX.md` (the roll-up you scan first — phase status, progress, `🔄 WIP`/`◻ TODO` themes)**, `phases/phase-N-*.md` (open checklist per phase), `done.md` (append-only, newest first), `outstanding.md` (deferred scope). Markers: `- [ ]` open · `- [x]`/`✅` done · `◐ PARTIAL` · `⏳ deferred` · `❌ OUT OF SCOPE`. Never pick `deferred`/`OUT OF SCOPE` unless told. `_INDEX.md` is the source of truth for what's claimed/in-flight — keep it current (Stages 2.7 + 10).
- Parallel work → git worktrees in the repo-root **`.worktrees/<branch>/`** dir (git-ignored; **never** under `.git/` — that path gets pruned by parallel `git worktree` runs and some dev servers deny `.git/**`); keep the primary checkout as home base.
- **`.worktrees/` is outside `.git/`, so this project's full test/lint/typecheck command runs fine inside the worktree** — no need to hop back to the primary checkout.
...
Skip any decision already unambiguously settled in the phase doc (its *Decisions / open questions* section) — but **never drop below the 5-question floor**: if skipping the settled ones leaves you with fewer than 5, surface the next-most-useful choices (edge-case handling, test strategy, naming / API shape, error states, rollout) until you have at least 5. **Do not implement until all of them are answered.**

## 2.6 · Rename session
Once the batch is chosen, immediately set the terminal/session title so it's visible at a glance:
...
3. Run: `printf '\033]0;Loop: exec %s\007' "<label>"` — this updates the terminal title.

## 2.7 · Claim the theme(s) on `main` — before the worktree
So parallel `/midnite-create` loops don't grab the same slice, **claim the whole batch in the index first**:
...
2. Commit **straight to `main`** and push immediately, one commit for the whole batch:
   ...
   ```
   (Small index-only touch-up → committing to `main` should already be sanctioned by this project's own `CLAUDE.md`. If the push races another loop: `git pull --rebase origin main` and re-push.)
...
## 6 · Pre-push gate
Run this project's own full test/lint/typecheck command (from `CLAUDE.md`) inside the worktree.
...
- **Update the trackers in the branch first, so the merge auto-publishes them** (don't wait to do this on `main` afterward). The batch spans one or more phases — repeat this for **every phase touched**, not just the first:
  - **Phase doc** (`phase-<N>-*.md`), per phase in the batch: mark that phase's landed theme/items done (`✅ DONE (PR #<n>, <date>)`) and **move** the completed `- [ ]` items into `done.md` (today's date, matching its existing entry format) — don't just tick in place.
...
- **Post-merge `_INDEX.md` sync on `main` — MANDATORY every merge, never skip.** The squash carried your in-branch `_INDEX.md` edit, but a parallel loop's merge can land between and leave it stale. Back in the primary checkout, refresh `main` and verify the index reflects reality:
  ...
  ```
  Then run the **whole-index drift guard** — it catches any `phase-*.md` (yours *or* one a `/midnite-ideate` run left unregistered) that has no `## Phases` row. It must print nothing:
  ```bash
  ...
  ```
  If it names a phase, add that row (Status / counts / bar / theme letters, from its phase doc) directly on `main` and push — an unregistered phase is invisible to the next `/midnite-create` and reads as stale on every surface.
...
Then:
- **On `/loop`** → re-invoke exactly as launched (`/loop /midnite-create` or `/midnite-create`, preserving `$ARGUMENTS`) and start again from Stage 1 on the next batch. Re-run Stage 2.6 as soon as the next batch is chosen so the session title stays current.
