...
- **Issues and code live in the same repo** (adjust this section if your setup differs — e.g. issues tracked in a separate repo), so `Fixes #N` in a PR body **auto-closes** the issue on merge, bare `gh` commands hit the right repo, and cross-references show up on the issue.
...
- If `gh` reports the wrong repo or account, check which GitHub account/org is active and switch if needed (`gh auth switch`).

## Respect

- `CLAUDE.md` (and its `AGENTS.md`/`GEMINI.md` mirrors, if present) = this project's own conventions (module/package boundaries; commit style; pre-push gate). Re-read the relevant bits before coding.
- Parallel work → git worktrees in the repo-root **`.worktrees/<branch>/`** dir (git-ignored; **never** under `.git/` — that path gets pruned by parallel `git worktree` runs and some dev servers deny it). Keep the primary checkout as home base; all multi-file work happens in the worktree.
...
Then read the **body** of every plausible candidate (`gh issue view <n> --comments`) — the reporter's environment details are the reproduction recipe.
...
1. **Correctness/trust** — does it show users *wrong data* or corrupt/lose their work? That is worse than a visual nit.
2. **Blast radius** — how many users / how central is the surface? A crash or a core-flow bug beats a corner case.
...
Present via **AskUserQuestion**: **exactly 4 options** — your highest-impact recommendation **first**, then 3 genuine alternatives (vary the shape: a quick win, a different surface, a bigger swing — not three near-clones of the top pick).
...
1. Rename the session so it's visible at a glance:
...
- Replay the reporter's environment: which surface, which build/mode, and the shape of input/state that triggers it. Build a minimal scratch reproduction where possible.
- Locate the real code (Grep/Glob, or delegate a search to an `Explore` agent when the surface is unclear — but you own the conclusion).
- For a wrong-data bug, find where the value is *produced*, not where it's rendered — usually the layer closest to the raw source of truth, not the UI. Check the raw input the producing code saw before blaming the presentation layer.
...
Present the consequential choices as separate **AskUserQuestion** calls. **Minimum 3; use 5+ for anything `[M]` or larger, or when the fix touches a shared contract.** Always include:
...
- **Scope boundary** — fix only the reported symptom, or the whole class of it (sibling code with the same bug)? Say which siblings you found.
- **Test proof** — what test pins it, at which layer.

Then, as the task warrants: any contract/schema change, version-sensitivity of an external tool this project shells out to, destructive-op safety (confirm gating), fallback/error UX.
...
Track the agreed plan with TodoWrite. **Do not implement until every question is answered.**
...
cd .worktrees/issue-<N>-<slug> && <this project's own install command>
```
...
- **Visual change → before/after screenshots** with whatever this project's own visual-testing tooling is, against a dev server or the real app. Read the images so they render inline in this thread; the same shots go in the PR body.
- **Build/packaging-only bug → verify in a real build**, not just dev, or say plainly that you couldn't and what remains unverified.
- Pre-push gate, in the worktree, all green — never push red: run this project's own full test/lint/typecheck command (from `CLAUDE.md`). Redirect output to a file and check the exit code; piping to `tail` masks failures. If a spec fails only under the full suite, re-run the file alone and confirm against `main` before calling it a regression.
