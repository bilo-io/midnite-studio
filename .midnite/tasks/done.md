...
The re-indexation question the phase doc flagged — `repaint()`'s `skipIndexation: true` cannot carry position changes, so what should? — was settled by reading sigma's own source (`refresh()` always ends in a full `process()` pass regardless of options, so the real lever is whether the REDUCER also re-runs for the whole graph every frame) and then measuring both paths with a headless-Chromium/SwiftShader micro-benchmark against a synthetic graph at this repo's real scale (15,292 nodes / 37,036 links, from `graphify-out/graph.json`'s own counts): `refresh({partialGraph, skipIndexation:false})` scoped to the moving node subset beat a bare `sigma.refresh()` by 28% (35.1ms vs 49.0ms median) at a realistic ~15% of nodes moving, and by 12% (44.7ms vs 51.1ms) even with every node bursting on the same frame. The scoped path shipped; the numbers and methodology are in a code comment beside `introTick`.

`prefers-reduced-motion` (via Phase 46's `useResolvedMotion()`) and `paused` (window blurred at mount, Phase 84) both skip the intro outright — final positions on first paint — matching `pulse()`'s own start-time-only check rather than interrupting an in-flight burst. 16 vitest cases cover the pure tween/centroid/edge-alpha logic with an injected clock, exactly as `knowledge-bounce.test.ts` covers `PulseTracker`.

## 2026-09-16 — Phase 87 Themes D, E — The sigma canvas and its four interactions

[PR #411](https://github.com/bilo-io/midnite-studio/pull/411). Raw `sigma` + `graphology` behind a ~200-line local hook (`use-sigma-graph.ts`, Decision 6) render the full graph WebGL canvas: built once per payload identity from Theme A's cached layout (no force simulation in the renderer), zoom+degree label LOD via sigma's `nodeReducer`, node colour from `community` reusing the browser's `--tab-group-N` tokens (8 hue buckets × 5 lightness tiers, enough spread for ~600 communities), size from degree, edge alpha from weight, WebGL-context-acquisition failure thrown and caught by the view's own Phase 60 `ErrorBoundary`, and a `MutationObserver` on `<html class>` (the same pattern `app.tsx`'s `useWindowBackgroundSync` uses) that repaints every node's colour on a light/dark flip. Confirmed via `bundle-report.mjs` that the entry chunk is byte-identical before/after (411.7 KB) — sigma/graphology land only in the Knowledge view's own lazy chunk. The one animated thing this hook does (the camera-fly on search focus) is gated on `useWindowFocused()` and snaps instead of animating while the window is blurred, honouring Phase 84's visibility gates.

The four interactions: click a node opens an Explorer-preview side panel (`knowledge-node-panel.tsx`, Decision 7) at its parsed `source_location` line, mounting the same `FilePreview` component `search-view.tsx` already mounts inline; search flies the camera to the first match and highlights its neighbourhood, dimming everything else; a virtualized, searchable community filter (`@tanstack/react-virtual`, the same library the command palette uses) handles the ~600 communities as a block-list (shown by default, toggle off) rather than 600 checkboxes; relation/weight/confidence edge filtering defaults to `relation === 'calls'` alone (Decision 8) — measured against this repo's own `graph.json`, the doc's literal "calls, plus anything above a confidence threshold" default would have let through 99.3% of edges (35,763/36,032 links score `confidence_score === 1`), so the legible default is `calls` alone (~17%) with confidence left as a live, user-adjustable slider. Filter/selection state lives in a small per-repo, non-persisted zustand store (`knowledge-filters-store.ts`, `files-store.ts`'s own `ensureScope` shape).

`packages/knowledge`'s lean projection gains `confidence` back onto each link — Theme A's own note flagged this as deliberately deferred ("left as a small addition for it"); this is that addition, defaulted to `1` same as `weight`, bumping `PROJECTION_FORMAT_VERSION` 1→2.

This PR landed after Theme F (PR #410) merged, which had built its own `use-knowledge-graph.ts` (react-query, repo-switch-safe by construction) and rewritten `knowledge-view.tsx`'s empty/absent/stale states — deliberately leaving `ready` a placeholder ("Theme D's canvas is where that progress bar belongs... so the two themes' diffs don't collide"). Rebased onto it: adopted F's `useKnowledgeGraph`/`knowledge-state.ts` wholesale (dropping this PR's own earlier hook), added a small additive `use-knowledge-layout-progress.ts` for the one thing F's hook deliberately didn't cover (streaming `knowledgeLayoutProgress` for the loading spinner), and replaced only `ready`'s placeholder body with the canvas + filters panel + node panel.

Two real bugs caught during manual verification (a throwaway, uncommitted Playwright screenshot against a synthetic graph — Theme G owns the permanent canvas e2e coverage), not in review: sigma calls its reducers synchronously from inside its own constructor, before the `renderer = new Sigma(...)` assignment completes, so the zoom-LOD reducer's `renderer.getCamera()` read `undefined` on first paint (caught cleanly by the `ErrorBoundary`, fixed with an optional-chained fallback ratio); and sigma's WebGL colour parser recognises only `#hex`/`rgb()`, never `hsl()` (`node_modules/sigma/dist/colors-*.js`), so every node/edge was painting solid black until every themed colour was converted through a new `knowledge-color-math.ts` (`hslToRgb`) first.

## 2026-09-16 — Phase 87 Theme F — Repo switching, empty and stale states

[PR #410](https://github.com/bilo-io/midnite-studio/pull/410). A new `knowledgeCheckGraph` IPC channel — a `stat` on `graphify-out/graph.json`, never `knowledgeGetGraph`'s parse or a potential cold ForceAtlas2 layout — backs `useKnowledgeGraphExists`, which the rail's pinned `KNOWLEDGE_ITEM` reads to grey its icon for a repo that exists but has never been graphified. The Knowledge view itself now resolves a real state machine (`resolveKnowledgeViewState` in `knowledge-state.ts`) from `knowledgeGetGraph`'s 4-arm envelope: `absent` renders an instructional panel (what graphify is, `pip install graphifyy`, `graphify update .` — copy is instructions, not an error), `unreadable`/`malformed`/`error` each get their own distinct message, and `ready` shows a "N commits behind HEAD" banner when `KnowledgeGraphPayloadSchema`'s new `commitsBehind` field (computed server-side via `rev-list --count builtAtCommit..HEAD`, `null` rather than a guessed `0` when the commit can't be resolved) is a known positive count — reported only, nothing re-runs graphify. The "a repo switch mid-load cannot land the previous repo's graph" requirement rides react-query's own cache-key semantics (`repoId` baked into `keys.knowledgeGraph`/`keys.knowledgeStatus`) rather than bespoke cancellation code, proven with an integration test that resolves two repos' promises out of order. Theme D's canvas (parallel branch, not yet merged) still owns the actual GPU-resource cleanup on repo switch — nothing here holds a WebGL context to leak yet.

## 2026-09-16 — Phase 87 Theme G — Tests, the measured numbers, and two real Theme D/E bugs

[PR #412](https://github.com/bilo-io/midnite-studio/pull/412). Closes Phase 87. Adds `packages/app/e2e/knowledge-canvas.spec.ts` — the **canvas-only** Playwright suite Theme G asks for: real WebGL context acquisition, real `getBoundingClientRect` hit-testing against WebGL geometry, and a real pointer drag to pan the camera, none of it assertable under jsdom. Everything else in the feature (filters, search matching, state machines, envelope shapes) was already vitest with the canvas mocked, which is what Phase 82's decision rule wants; this theme adds only the missing piece. The functional-e2e ratchet moves 433 → 436 with its own justification comment.

**It was also the clean-up pass for [PR #411](https://github.com/bilo-io/midnite-studio/pull/411)**, which a human merged deliberately while 14 of its 15 CI checks had never run (the macOS runner pool was saturated). Writing a real-browser test against that canvas caught **two genuine bugs #411 had shipped** — exactly the class "unverified by CI" predicts:

  - **The sigma canvas never resized when its own flex container did.** Opening `KnowledgeNodePanel` shrinks the container, but sigma re-measures only on `render()` and only *schedules* a render off a `window` resize event — a pure CSS/layout resize fires no window event. The canvas kept its old, larger size, overlapped the panel, and **intercepted its own Close button's clicks** (reproduced with `elementFromPoint`). Fixed with a `ResizeObserver` in `use-sigma-graph.ts`.
  - **Search-and-focus used the wrong coordinate space.** `camera.setState({x, y})` was handed the node's *raw* graph coordinates, but sigma's camera state lives in its normalized "framed graph" space (`createNormalizationFunction`). With nodes spread outside `[0,1]` — i.e. this repo's own layout — "fly to the searched node" landed nowhere near it. Fixed by reading `renderer.getNodeDisplayData(nodeId)`, sigma's own already-normalized data.

Both are two-line fixes; without them 2 of the 3 new e2e tests fail, with them 3/3 pass across `--repeat-each=3`. Themes A–F are otherwise unaffected (the 14-file/101-test `knowledge` vitest suite stayed green).

**Measured numbers**, fresh against this repo's own 14,881-node / 36,032-link `graph.json`: on-disk 20.14 MB, raw parsed 15.73 MB, lean projection 8.47 MB (~54% of raw); `readGraph` 46 ms, `projectGraph` 3 ms, cold ForceAtlas2 4.23 s, **warm-cache open 40 ms (~100x faster than a cold layout)**; entry chunk 411.8 KB, unchanged, with `graphology` confirmed absent from it and present only in the lazy `knowledge-view` chunk.

**Left open, deliberately (9 verification lines, non-blocking):** the committed visual baseline — this repo only ever commits `-linux.png`, generated through a Docker container unavailable in the sandbox, and a macOS-rendered file under that name would hand the cross-platform lane a false baseline; and a Knowledge-view-specific `idle-cpu.mjs --blurred` reading, since the perf harness seeds a *repo*, not a *view*. Both stay human passes.

## 2026-09-16 — Phase 87 Theme C — The Knowledge rail row and view registration

[PR #407](https://github.com/bilo-io/midnite-studio/pull/407). `'knowledge'` joins `VIEW_IDS` directly after `'notes'` — that array position *is* the rail order, so it is the only place the third pinned row's placement is written down. `VIEW_ICON.knowledge = SiGrapheneos` from `react-icons/si` (per-set import, never the root barrel), and `icon-names.test.ts` now guards the `si` set the same way it already guarded `lu`. `KNOWLEDGE_ITEM` sits beside `PINNED_ITEM`/`NOTES_ITEM` in `app.tsx` with a header comment saying why it is pinned rather than in `WORKSPACE_NAV_ITEMS`. The lazy registry entry follows `loadNotesView` but is deliberately **not** `global: true` — unlike Notes, a knowledge graph is meaningless with no repo open. No chord this phase (Decision 5), so `nav-chords.ts` gains no entry and the row shows no hover bubble.

One fix landed with it, found by CI rather than by review: Theme C's first cut gave the view the keyword token `graph`. The companion's `longestViewMatch` scores a view's label, its id and each keyword *word* by word count and breaks a tie on first match in `VIEW_IDS` order — and `knowledge` is `VIEW_IDS[3]` against `graph`'s `[10]` — so "show me the graph" silently started routing to Knowledge instead of the Commit Graph, failing `companion-panel.spec.ts:451`. Dropped the bare token (`graphify` still prefix-matches `graph` in the palette's fuzzy search) and added `services/palette/view-keywords.test.ts` as a directional guard: a keyword equal to another view's id is a bug only when the declaring view sorts earlier. That guard surfaced two collisions that predate this phase — `projects → issues` and `graph → history` — which are allowlisted rather than fixed, since changing them alters companion routing outside Theme C's scope; both are written up in [`outstanding.md`](outstanding.md).

## 2026-09-16 — Phase 87 Themes A, B — The electron-free engine, and the IPC contract off the main thread

[PR #408](https://github.com/bilo-io/midnite-studio/pull/408). New `packages/knowledge` — same shape as `git-engine`/`db-engine`, plain Node/TS, never imports `electron`: `readGraph(repoPath)` resolves `graphify-out/graph.json` into `{ok:true, graph}` | `{ok:false, kind:'absent'|'unreadable'|'malformed'}` (absent is the common, un-graphified-repo case, not an error); `projectGraph()` strips everything the picture never reads (`_origin`, `context`, `confidence_score`, `_callable_class`, `norm_label`, the long `source_location` strings), keeping `id`/`label`/`community`/`community_name`/`file_type` per node and `source`/`target`/`relation`/`weight` per link — measured ~50% of the raw parsed size against this repo's own 14,881-node/36,032-link graph; `buildDetailIndex()` keeps `source_file`/`source_location` retrievable by id, out-of-band. `layoutGraph()` runs ForceAtlas2 (`graphology-layout-forceatlas2`) seeded from a deterministic circular layout (nodes sorted by id) so two runs settle at the same coordinates — measured ~7.1s for the full graph at 100 iterations, confirming real compute. `layout-worker.ts` is the `worker_threads` entry point (Decision 3, resolved unattended per the doc's own recommendation). `readLayoutCache`/`writeLayoutCache` key on `built_at_commit` plus `PROJECTION_FORMAT_VERSION`, write-then-rename into a caller-supplied cache dir, never `graphify-out/` itself. Added the third `git-engine`-shaped eslint boundary group.

Theme B added `knowledgeGetGraph`/`knowledgeGetNodeDetail` channels and the `knowledgeLayoutProgress` event to `shared`, plus `KnowledgeResult` — its own envelope, shaped like `GitOpResult`/`DbOpResult` but with a 4-arm failure (`absent`/`unreadable`/`malformed`/`error`, since `conflict` means nothing here). The main handler (`knowledge-handlers.ts`) resolves the repo via the existing repo-registry, serves a cached layout directly on a `built_at_commit` match, otherwise spawns the `worker_threads` worker (resolved via `require.resolve('@midnite/studio-knowledge')` + a sibling-file join) and forwards progress to the requesting window only, then writes the cache. `knowledgeGetNodeDetail` is a separate per-id fetch from an in-memory index, rebuilt on demand. `window.midniteStudio.knowledge.{getGraph, getNodeDetail, onLayoutProgress}`. 40 new tests (21 knowledge, 9 shared domain, 10 desktop handler); `moon run :typecheck :lint :test` green across every package.
...
## 2026-09-13 — Phase 86 Theme A — One list, one truth

[PR #376](https://github.com/bilo-io/midnite-studio/pull/376). Merged live rows (`useTerminalStore`) and closed rows (`useSessionHistory`) into a unified `ManagedSession[]` selector discriminated by `liveness: 'running' | 'asleep' | 'closed'`. Live rows sort above closed rows while preserving existing `session-order.ts` grouping and sorting. In `SessionRow`, moved the agent icon to the left of the title/name, added a keyboard-focusable state dot with a tooltip indicating live activity (`thinking | waiting | idle`) from `SessionActivitySchema`, made Purge strictly closed-only, and added liveness facets to session filters. 7 deliverables completed.

## 2026-09-13 — Phase 82 Theme F — Write the convention down, and ratchet it

[PR #368](https://github.com/bilo-io/midnite-studio/pull/368). Established the testing strategy, conventions, and budget ratchets for the righted test pyramid. Added `docs/TESTING.md` documenting the three layers (Vitest logic, Playwright visual locator crops, Playwright functional e2e), the decision rule (*"a new test is a vitest test unless it needs real layout, real CSS, pointer coordinates, xterm, canvas, focus order, or paint order — name which in the spec header comment"*), test runner commands, and the Playwright Docker baseline regeneration procedure. Synced the testing convention bullet to `CLAUDE.md`, `AGENTS.md`, and `GEMINI.md`. Added `scripts/e2e-budget.mjs` and `scripts/e2e-budget.test.mjs`, wired as `root:e2e-budget` into `moon.yml` and the CI `gate-node` job: ratchets declared functional E2E tests at 433, enforces the visual baseline cap via `checkVisualBudget`, and fails on wall-clock duration assertions in unit tests. Enabled full typechecking on `packages/app/e2e/**/*` in `tsconfig.json`, resolving all ~25 first-time type errors across the suite. Removed 3 pre-existing wall-clock assertions from `packages/desktop` unit tests. All 6 Theme F items completed; Phase 82 stands at 52/75 (69%).
...
85%) and **closes the phase's eight themes** — it landed last of the three parallel wave-2 PRs,
after C+H ([#270](https://github.com/bilo-io/midnite-studio/pull/270)) and F+G
([#272](https://github.com/bilo-io/midnite-studio/pull/272)). The phase's spine: the scripted opening, the words a request is recognised by, the route from
