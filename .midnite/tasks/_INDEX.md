...
| [51 · The terminal, made steady](phases/phase-51-terminal-steadiness.md) | 🔄 WIP | — | 8/37 | `██░░░░░░░░` | 22% | — | C D E F G |
...
- ◻ **A** — One filter toolbar shared by Table and Board: text search plus assignee/label/type/state facets, with truncation kept honest.
- ◻ **B** — Group by any single-select or iteration field, replacing the match on the literal name `Status`; iteration grouping is read-only.
- ◻ **C** — Tri-state sortable table columns, one comparator per `dataType`, single-select following option order.
- ◻ **D** — Filter/group/sort/collapse persisted — keyed by `projectId`, not `repoId`, and LRU-bounded.
