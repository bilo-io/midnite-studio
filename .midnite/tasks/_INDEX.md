...
| [90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md) | 🔄 WIP | — | 70/101 | `███████░░░` | 69% | B D | K |
...
- ◐ **B** (PR [#501](https://github.com/bilo-io/midnite-studio/pull/501), PR [#503](https://github.com/bilo-io/midnite-studio/pull/503)) — accounts, the vault as a third binding over `main/secure-store.ts` (Phase 76 Theme D had already extracted it, so no `secret-vault.ts` was created), `whoami` per provider, five `mstudio:forge:*` channels, the title-bar avatar, Settings ▸ Accounts, and the `agentApiKeys['github']` → vault migration. #503 closed two of the three items #501 left open: `forge.scopeReposToActiveAccount`/`forge.syncGhAuthSwitch` now join the persisted settings (`forgeAccounts`/`forgeActiveAccountId` had already reached `ui-store.ts` with #501), and `normalizeForgeAccountHost` closes the account `host` field's scheme gap (the general `shell.openExternal` path was already checked before this phase). **One item stays open, deliberately**: the per-provider redaction patterns, genuinely deferred to E/F/G by their own checklist text — logged in [`outstanding.md`](outstanding.md)
