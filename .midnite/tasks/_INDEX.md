...
- **[Phase 56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md)** (10% · 3/29, [PR #148](https://github.com/bilo-io/midnite-studio/pull/148)) — **Themes A, B, E landed** (2026-09-04): 4 → 8 e2e shards (`timeout-minutes` 20 → 10), `fullyParallel: true` (inherited by the CI ratchet config), and an `actions/cache@v4` step for Vite's `.vite` dir keyed on the lockfile/config/source hash. Measured on the landing PR's own CI run: 3m24s–5m17s per shard, all 8 green, down from the 6m39s–7m30s baseline at 4 shards. **Theme D (retry trim 2→1) was tried and reverted**: it passed on the first CI run, but a second full CI run — triggered by an unrelated docs-only rebase, no code change — failed `titlebar-agents.spec.ts`'s "reduced motion keeps a running launcher glow and full opacity" (not in `KNOWN_RED`, reliable across many recent `main` runs) twice in a row, while an exact local reproduction of that shard passed 77/77 clean. That is the one-run-in-two CI variance the retry budget exists to absorb, and one retry wasn't enough margin — reverted to `retries: 2` pending a real fix. Remaining: **C** — a live `workers: 2` vs `workers: 1` CI comparison, needing multiple runs before a value is chosen; **F/G** — gating raw `page.screenshot()` calls behind `MSTUDIO_SHOTS` and extracting a shared `shots-helper.ts`, each touching a large batch of spec files, left for their own pass.
...
| [56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md) | 🔄 WIP | — | 3/29 | `█░░░░░░░░░` | 10% | — | C D F G |
...
- ◻ **A** — The tool contract in `shared`: an `MCP_TOOLS` registry in the single-source-of-truth shape of `COMMANDS`, reusing `ipc/schemas.ts`'s zod payloads rather than re-typing them.
- ◻ **B** — The server in main: a `userData` Unix socket named by build fingerprint, the broker's length-prefixed framing, dispatching to services directly rather than through `ipcMain`.
- ◻ **C** — The stdio shim: a bundled node script speaking MCP `initialize`/`tools/list`/`tools/call`, serving the tool list from the shared registry and failing cleanly when the app is not running.
- ◻ **D** — The read-only tool set v1: `repo.list`, `repo.current`, `status.get`, `graph.log`, `diff.file`, `branch.list`, `forge.pulls`, `forge.checks` — each better than the command it replaces.
- ◻ **E** — Consent, scope and audit: a default-off setting, every `repoPath` through `fs-scope.ts`, a `0600` socket, and one audit line per call through the log seam.
- ◻ **F** — The Settings page and status readout: the switch, the copyable `claude mcp add` line, the tool list rendered from the registry, and the last N calls.
...
- ◻ **C** — Worker concurrency trial: evaluate `workers: 2` vs `workers: 1` on CI and land the measured winner.
...
- ◻ **F** — Screenshot gating in functional specs: gate raw `page.screenshot()` calls in functional specs behind `MSTUDIO_SHOTS`.
