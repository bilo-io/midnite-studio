...
- **[Phase 38 · Paying off the e2e suite](phases/phase-38-e2e-suite-repair.md)** (0% · 0/49) — Planned, not started. The Playwright suite was run in full on 2026-09-01 for the first time in weeks, to give it a CI job at last, and **45 of its 442 specs failed across 17 of 58 files** — none of it a fresh regression: a bisect against `ec2c75e` found the same three sample files 15-red *before* Phase 36's performance work against 13 on `main`. This is the drift `outstanding.md` predicted on 2026-08-27, when it recorded that nothing runs `app:e2e` automatically. The new CI job closes the hole going forward but can only block on what passes, so it ships with a **ratchet** — `playwright.ci.config.ts` names the 17 files and CI blocks on the other 41 — and this phase empties that list. Themes follow root causes rather than filenames: **A** alone covers seven failures across `fab-loops` and `terminal-links` that share one error string (`pty:activity was not delivered to pty-1`), and **B** covers a `changes-panel` file that fails all ten of its specs on the same assertion. Three failures read like genuine product bugs rather than stale specs — a checkout dropped on view switch, a destructive-confirm dialog that no longer takes focus, and a pointer-events delay the browser pane guards against. **H** deletes the ratchet once the list is empty.
...
|-------|--------|---------|------|----------|---|--------|--------|
| [39 · One rail, five chords and four loops](phases/phase-39-status-bar-shortcut-rail.md) | 🔄 WIP | — | 0/64 | `░░░░░░░░░░` | 0% | A B C D E F | G |
| [38 · Paying off the e2e suite](phases/phase-38-e2e-suite-repair.md) | ◻ TODO | — | 0/49 | `░░░░░░░░░░` | 0% | — | A B C D E F G H |
