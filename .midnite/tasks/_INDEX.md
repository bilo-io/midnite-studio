...
- **[Phase 56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md)** (14% · 4/29, [PR #148](https://github.com/bilo-io/midnite-studio/pull/148)) — **Themes A, B, D, E landed** (2026-09-04): 4 → 8 e2e shards (`timeout-minutes` 20 → 10), `fullyParallel: true` (inherited by the CI ratchet config), CI retries 2 → 1, and an `actions/cache@v4` step for Vite's `.vite` dir keyed on the lockfile/config/source hash. Measured on the landing PR's own CI run: 3m24s–5m17s per shard, all 8 green, down from the 6m39s–7m30s baseline at 4 shards — inside the phase's 2–4 minute target on the low end, not yet on the high end, with the Vite cache's win still to show on a warm-cache run. Remaining: **C** — a live `workers: 2` vs `workers: 1` CI comparison, needing multiple runs before a value is chosen; **F/G** — gating raw `page.screenshot()` calls behind `MSTUDIO_SHOTS` and extracting a shared `shots-helper.ts`, each touching a large batch of spec files, left for their own pass.
...
| [56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md) | 🔄 WIP | — | 4/29 | `█░░░░░░░░░` | 14% | — | C F G |
...
- ✅ **D** (PR #148) — Retry trim: 2 → 1 in CI to avoid 3-minute burns per failure now that `KNOWN_RED` is down to 1 file.
