...
| [92 · Agentic execution from Projects](phases/phase-92-agentic-execution-from-projects.md) | 🔄 WIP | — | 21/36 | `██████░░░░` | 58% | E | — |
...
- **[Phase 93 · Issue board reporting from inside the app](phases/phase-93-issue-board-reporting.md)** (94% · 32/34) — **All five themes landed** (2026-09-20). Two verification items remain and are deliberately not this pass's to close: filing a real bug report and a real feature request from a packaged app against the now-public `bilo-io/midnite-apps` board (a human pass — no agent files a real issue there), and observing `/midnite-address-issue`'s Stage 1 digest list a candidate from each board at once (needs the apps board to actually receive traffic first, which Themes A-D are what makes possible). Confirmed, not assumed: two issue boards exist. `bilo-io/midnite-studio` (this repo) is **private**, one collaborator, and carries a real but internal engineering backlog no external user can reach. `bilo-io/midnite-apps` is **public**, `has_issues: true`, already ships issue-form templates and a label set (`bug`, `enhancement`, `app: midnite-studio`, `needs-triage`) built for exactly this — and has **zero issues filed today**, because nothing in the app drives traffic to it beyond a bare "open a blank browser tab" link. [Phase 65](phases/phase-65-somewhere-for-a-crash-to-go.md) already built the whole machine this phase completes — a redacted diagnostics bundle, a rotating log, a "Copy diagnostics" button — and stopped one function short: `CrashReporting()`'s "Report a bug" is `openExternal(NEW_ISSUE_URL)`, nothing more. This phase adds the missing write: a `gh issue create -R bilo-io/midnite-apps` call (`gh-app-issue.ts` — the one write in `forge/` with no `Forge` parameter, since it always targets a fixed repo regardless of which one is open) behind a composer dialog that prefills title, body and the existing redacted diagnostics block, applies the right labels itself, and falls back to today's exact browser-open behaviour when `gh` is missing or signed out. **A** the fixed-target write and its command construction; **B** the IPC surface, extending the existing `report` bridge group rather than inventing a new domain; **C** the composer dialog, wired from both existing "Report a bug" entry points; **D** confirms redaction stays the single path — no new secret pattern, no new vault, no telemetry; **E** teaches `/midnite-address-issue` to scan both boards and use cross-repo `Fixes bilo-io/midnite-apps#N` syntax, the one gap in a skill whose own header already named the two-repo split and chose not to build for it. Explicitly not this phase: issue creation for whichever repo the user has open ([Phase 54](phases/phase-54-issues-view.md)'s own deferred scope, a different target entirely), an embedded GitHub issue form (Phase 71 Theme B's decision stands), and any new auth flow — the composer authenticates as whoever the user's own `gh` CLI is already logged into, the same model every other forge call in this app already uses. 34 items, five themes, no new dependency.

- **[Phase 92 · Agentic execution from Projects](phases/phase-92-agentic-execution-from-projects.md)**
  (58% · 21/36) — **Themes A-D done (PR #478, PR #481), E outstanding.** A kanban card's Play button and the dependency graph
...
- ◻ **B** — `sandbox: true` on both windows: `homedir`/`hostname` via `additionalArguments`, `node:os` out of the preload, a bundle test that keeps `node:` out.
- ◻ **C** — A header-based CSP from a pure `buildCsp()` (dev/packaged, `persist:browser` skipped) and a `will-navigate`/`will-redirect` guard on the app window; zero violations across `app:e2e`.
- ◻ **D** — The Twelve Data key into a `safeStorage` vault behind an enum-keyed `secrets` channel, finance fetches proxied through main, `ipwho.is` behind a default-off Privacy switch.
- ◻ **E** — 45 `ipcMain.on` sites onto `handleSend`, two raw `ipcMain.handle` onto `handle`, an eslint rule confining both to `ipc/handle.ts`.
