...
| [90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md) | 🔄 WIP | — | 63/101 | `██████░░░░` | 62% | B D H I | K |
...
- ◐ **H** (PR [#502](https://github.com/bilo-io/midnite-studio/pull/502)) — `capabilitiesFor` is now a compiler-enforced `Record<ForgeKind, ForgeCapability>`, `capabilities.test.ts` asserts the same exhaustiveness at the value level, and `app.tsx`'s `FORGE_GATED_VIEWS` reads it per field (`actions → checks`, `reviews → pulls`, `issues → issues`, `projects → projects`) instead of one boolean gating all four. **One item open**: the `'partial'`-capability sentence in each view. Two providers now report one: Bitbucket's `threadResolution: 'partial'` (Theme F, PR #504) and GitLab's `projects: 'partial'` (Theme E, PR #505) — both make this reachable, noted in each rather than built, since the four-view UI is this theme's own scope, not E's or F's
