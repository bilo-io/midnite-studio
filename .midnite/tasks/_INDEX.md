...
| [53 · The first release](phases/phase-53-first-release.md) | 🔄 WIP | x1 | 39/59 | `███████░░░` | 66% | — | — |
...
| [46 · The lock screen, and a motion policy that holds](phases/phase-46-lock-screen-and-motion.md) | 🔄 WIP | x1 | 37/55 | `███████░░░` | 67% | H | Verification (human keyboard + eye pass) |
...
| [33 · Application Installation, CLI Tool & Desktop Integration](phases/phase-33-installable-app-and-cli-integration.md) | 🔄 WIP | x1 | 22/59 | `████░░░░░░` | 37% | C | — |
...
- ◻ **H** — The verification residue, as work. Three of the eleven open `## Verification` lines turn out to be genuinely machine-executable: extracting `styles-motion-guards.test.ts`'s module-local helpers into a fixture-testable `styles-motion-guards.ts` so the Theme F guard proves itself instead of asking a human to hand-mutate CSS, a pill keyboard-reachability e2e case, and a reduced-motion assertion on the lock screen's own `screensaver-sheen` animation. Four more lines were already covered by tests that landed with Themes E–G and are marked `(**unchanged**)`; the remaining four stay in `## Verification` for a human.
...
- 🔄 **C** — `midnite-studio://` Custom Protocol Handler & Deep-Link Dispatch. Claimed — the single-instance lock, `open-url`, argv forwarding and a pure `parseDeepLink` already exist; closing out the jail's consent UI for an unknown `open`/`clone` proposal.
