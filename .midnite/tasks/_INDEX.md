...
- 🔄 **D** (claimed, worktree `p86-d`) — The pane that shows a live terminal: closed → transcript, running → the real xterm, with **one xterm per pty** via `revealSession()` rather than a second mount; a store-level send-input action that does not exist today.
...
- ◻ **F** — Notes on disk: a main-process per-repo store under userData (never in the user's repos), a channel quartet, the `Note` shape moved to `shared`, and a localStorage→disk migration that writes-then-verifies and never deletes.
- 🔄 **G** (claimed, worktree `p86-g`) — The Notes page proper: notes sidenav + content pane, lazy-loaded Monaco (a *third* Monaco surface — flagged against Phase 77), a markdown preview toggle over the existing `react-markdown` path, and Brainstorm / Execute-adhoc / status / delete carried across via `use-skill-handoff`.
- ◻ **H** — Verification: merged-selector and adapter unit tests, migration paths, visual baselines for both surfaces, one e2e only for the real-xterm pane, and four human passes including a real Claude and a real Codex resume.
