...
| [90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md) | 🔄 WIP | — | 25/101 | `██░░░░░░░░` | 25% | B D H | C E F G I K |
...
- 🔄 **B** (PR [#501](https://github.com/bilo-io/midnite-studio/pull/501)) — accounts, the vault as a third binding over `main/secure-store.ts` (Phase 76 Theme D had already extracted it, so no `secret-vault.ts` was created), `whoami` per provider, five `mstudio:forge:*` channels, the title-bar avatar, Settings ▸ Accounts, and the `agentApiKeys['github']` → vault migration. **Three items open**: `forge.activeAccountId` never reached `ui-store.ts` (the active pointer lives in main only); the per-provider redaction patterns are deferred to E/F/G by their own checklist text; and the `shell.openExternal` scheme check is unimplemented
