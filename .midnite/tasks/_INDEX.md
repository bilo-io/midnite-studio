...
- ◐ **I** (PR #108, PR #TBD) — Wiring + verification: a chord-less `workflow.run` command, which turned out to need `PALETTE_SAFE`'s allowlist too (declaring it in `COMMANDS` alone left it undiscoverable — caught by an e2e run against the real palette) and a new `workflow-run-command-store.ts` seam (the `commit-box-store.ts` shape) since "which workflow is open" is local view state. The settings page (default timeout, run-history cap) reaches main for real over a new one-way `workflowSetDefaults` channel — `update.setChannel`'s shape, `EngineDeps`/`trimRunsPerWorkflow` gained injected overrides. **PR #TBD (2026-09-05)** actually re-ran the phase's own Verification checklist rather than leaving it ticked from memory: full gate, boundary/import checks, every named unit/RTL test by name, a real `app:build`+`desktop:bundle` bundle report (workflows view confirmed in its own lazy chunk, outside the entry), and the committed screenshots visually re-checked — 19/20 Verification lines now checked. One human-only real-network pass stays open (the only one nothing here can drive unattended).
