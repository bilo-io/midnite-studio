...
| [66 · API Client](phases/phase-66-api-client.md) | 🔄 WIP | x1 | 12/73 | `██░░░░░░░░` | 16% | B C E F G | D H |
...
- ◻ **B** — Nav and registration, which is four edits and not two: the `ViewId` union **and** the separate `VIEW_IDS` array (`viewForPath` derives from the array), `VIEW_COMPONENT` — a `Record`, so a missing entry is a typecheck failure — the **sixth** `WORKSPACE_NAV_ITEMS` entry (it has five today, not three), and a chord-free `view.apiClient` beside `view.video`.
