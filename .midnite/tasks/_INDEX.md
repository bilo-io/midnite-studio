...
| [38 · Paying off the e2e suite](phases/phase-38-e2e-suite-repair.md) | 🔄 WIP | — | 37/59 | `██████░░░░` | 63% | G I | D H |
...
- ◐ **D** — The terminal panel ([PR #47](https://github.com/bilo-io/midnite-studio/pull/47)): reload rehydration + independent list resize — both genuine spec races (an async chunk-load beat, and a bounding box measured mid-tween), fixed and stable over 3 local runs each. **Attempted and reverted**: dropping the whole file from `KNOWN_RED` — green at 38/38 on macOS, then CI (GPU-less Linux) surfaced real failures in *other* specs, folded into Theme I's scope below.
...
- ◻ **I** — The terminal does not render on the CI runner: **8 specs**, plus `terminal.spec.ts` as a **new, uncounted sighting** ([PR #47](https://github.com/bilo-io/midnite-studio/pull/47)) — un-ratcheting it for the first time hit the same wall on ≥3 agent-mark specs before the job was cancelled, true count still unknown. `@xterm/addon-webgl` gets no context on a GPU-less runner. A 15s timeout and SwiftShader both failed to fix it (SwiftShader also cost 60% runtime, reverted); the real answer is still open.
