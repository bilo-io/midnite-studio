...
<!-- Newest-first. Status: ✅ DONE · 🔄 WIP · ◻ TODO. Done = checked/total in-scope items; Progress = 10-cell bar. Keep this table right at the top of the file so latest progress is immediately visible. -->
...
| [90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md) | 🔄 WIP | — | 88/110 | `████████░░` | 80% | — | — |
...
| [76 · The renderer in a sandbox, and the headers it never sent](phases/phase-76-the-renderer-in-a-sandbox.md) | 🔄 WIP | — | 24/35 | `███████░░░` | 69% | A | — |
...
| [68 · Where focus goes when the dialog closes](phases/phase-68-where-focus-goes.md) | 🔄 WIP | — | 32/37 | `█████████░` | 86% | — | — |
| [67 · The sessions you closed](phases/phase-67-the-sessions-you-closed.md) | 🔄 WIP | x1 | 44/64 | `███████░░░` | 69% | — | Verification (human/e2e passes) |
| [66 · API Client](phases/phase-66-api-client.md) | 🔄 WIP | x1 | 71/73 | `██████████` | 97% | — | H (2 human passes only) |
...
| [46 · The lock screen, and a motion policy that holds](phases/phase-46-lock-screen-and-motion.md) | 🔄 WIP | x1 | 40/55 | `███████░░░` | 73% | — | Verification (human keyboard + eye pass) |
| [45 · The leak audit](phases/phase-45-leak-audit.md) | ✅ DONE | — | 35/35 | `██████████` | 100% | — | — |
| [44 · Video Studio](phases/phase-44-video-studio.md) | ✅ DONE | — | 64/64 | `██████████` | 100% | — | — |
| [43 · Workflows](phases/phase-43-workflows-mvp.md) | ✅ DONE | x1 | 77/77 | `██████████` | 100% | — | — |
...
| [37 · A glow that knows which tab](phases/phase-37-fab-tab-glow.md) | ✅ DONE | — | 44/44 | `██████████` | 100% | — | — |
| [36 · Faster, lighter, same app](phases/phase-36-performance-diet.md) | ✅ DONE | x1 | 64/64 | `██████████` | 100% | — | — |
...
| [26 · Side by side, and the room to show it](phases/phase-26-side-by-side-diffs.md) | 🔄 WIP | x1 | 67/70 | `██████████` | 96% | — | (3 human passes) |
...
- **[Phase 97 · Workflow graph primitives, loops and templates](phases/phase-97-workflow-graph-primitives.md)** (89% · 81/91, [PR #557](https://github.com/bilo-io/midnite-studio/pull/557)) — **Planned 2026-09-24, brainstormed with the user**, seeded by the Graph / Harness / Loop Engineering articles in `docs/agentic_engineering/`. Makes the Workflows editor able to draw those articles' diagrams: typed ports and edge kinds (data / conditional / loop / error) with a connect-time type check, per-edge routing, a join node (all / any / allSettled) and an error port in place of `skipDownstream`, bounded loop back-edges (mandatory maxIterations + budget, dry-round convergence, failures carried forward, escalation), human gates (run panel, bell, MCP, PR comment), verifier and router nodes, durable run state with checkpoint/resume and per-node failure policy, cron / forge-PR triggers while the app is open, a harness frame + policy gate, per-kind canvas styling, receipts on Phase 94 Theme A's run record with iteration replay, a `{{demo.baseUrl}}` demo endpoint group, and five built-in templates in a gallery. One engine, no LLM judge. **Theme A landed** ([PR #557](https://github.com/bilo-io/midnite-studio/pull/557)): `WorkflowPortSchema`, edge kinds, `portsForNode`, `canConnect`, and `migrateWorkflowEdges` so every persisted workflow keeps running unchanged. **Theme B landed** ([PR #558](https://github.com/bilo-io/midnite-studio/pull/558)): per-edge readiness (`taken`/`dead`/`pending`), the `join` node settled inline in the engine, and the `error` port replacing `condition`'s `skipDownstream`. **Theme M landed** ([PR #559](https://github.com/bilo-io/midnite-studio/pull/559)): the `{{demo.baseUrl}}` demo endpoint group for HTTP nodes and `WORKFLOW_RESERVED_INTERPOLATION_ROOTS` (`demo`/`loop`/`state`). **Theme C landed** ([PR #560](https://github.com/bilo-io/midnite-studio/pull/560)): `kind: 'loop'` edges as the one deliberate exception to acyclicity, a pure `loop-controller.ts` that overrides `settledPort` on a stop rather than running a second cascade, four `loopExit` reasons (`converged`/`max-iterations`/`budget`/`cancelled`) redirecting through a new `exhausted` port, dry-rounds convergence dedupe, and `{{loop.iteration}}`/`{{loop.previous.X}}`/`{{loop.failures}}` carried forward into a retried body. **Theme J landed** ([PR #561](https://github.com/bilo-io/midnite-studio/pull/561)): per-kind edge styling (`workflow-edge-view.tsx`), one coloured `Handle` per `portsForNode()` entry with live `canConnect` dimming and a drop-time rejection tooltip, a per-kind node-shape map (`condition`'s diamond header, `join`'s pill), taken/dead post-run highlighting, and dagre excluding `loop` edges from ranking — scoped to the kinds `main` has today, with a documented extension point per map for D/E/F/H/I's `gate`/`verify`/`router`/`trigger`/`frame`. **Theme D landed** ([PR #562](https://github.com/bilo-io/midnite-studio/pull/562)): the `gate` node kind (`approved`/`rejected` out-ports, settled exactly like `condition`'s `true`/`false`), a new `waiting` `WorkflowNodeStatus` that pauses the run without failing it, and all four decide surfaces — the run panel's Approve/Reject row, a notification-bell toast, a third `Settings ▸ MCP` switch gating the first non-read-only MCP tool (`workflow_gate_decide`), and a PR/issue comment poll through the existing multi-forge `ForgeAdapter` — plus the kill switch's Flow scope now actually cancelling the run (a gate has no pty to close) and a `shield` canvas shape (Theme J's own extension point). **Theme F landed** ([PR #563](https://github.com/bilo-io/midnite-studio/pull/563)): the `router` node kind — expression mode (first matching `when` wins, reusing `condition.ts`'s own evaluator) or agent-label mode (an embedded agent whose done marker names one of the router's own case ids, `route=<caseId>`, falling through to `default` rather than a guess on an unrecognised one), settled via the same `port`/`settledPort` mechanism `condition`/`gate` already use — no engine change needed. `agent.ts`'s pty-watch loop was extracted into a reusable `runAgentToDoneMarker` so the agent-label mode shares it rather than duplicating it. **Theme E landed** ([PR #564](https://github.com/bilo-io/midnite-studio/pull/564)): the `verify` node kind (`pass`/`fail` `verdict` out-ports, never an executor failure — same principle as `condition`'s `true`/`false`), a `check` config union (`agent` reusing that same shared `runAgentToDoneMarker`; `exit-code`/`test-counts` running headlessly through `process-runner.ts` rather than an interactive pty; `json-path` reusing `condition.ts`'s comparison), fresh `vitest`/`jest`/`junit-xml`/`tap` test-count parsers in `shared`, capped+redacted evidence that now feeds `loop-controller.ts`'s `{{loop.failures}}`, a new `severity` field on `WorkflowIssue` so the maker == checker rule can warn without blocking Run, and a `check-badge` canvas shape showing the node's actual last verdict. **Theme H landed** ([PR #565](https://github.com/bilo-io/midnite-studio/pull/565)): the `trigger` node kind (`manual | schedule {cron} | forge-pr {repoId, events, branchFilter?}`, at most one per workflow, no in-ports) — a hand-rolled 5-field cron parser (`workflow-cron.ts`, no npm dep, keeping `shared` zod-only) backs a main-side `trigger-scheduler.ts` that arms one re-arming timer per schedule (no poll, no missed-tick replay after relaunch) and subscribes `forge-pr` triggers through a small additive pub-sub on the existing `ForgePoller` rather than a second poller; skip-while-running is logged against the workflow, and the trigger's per-run dynamic payload (e.g. which PR fired it) threads through `EngineDeps.triggerPayload` rather than living in the node's static config. **Theme G landed** ([PR #574](https://github.com/bilo-io/midnite-studio/pull/574)): a per-run **state store** (a new `state` node kind, `WorkflowRun.state`, read anywhere as `{{state.<key>}}`, read-modify-written inside the SAME run lock every settle already holds and capped at 65536 bytes), **resume** (a dangling `running` run is now marked `interrupted` on boot rather than swept to `cancelled`, and the run panel's new Resume button restarts `drive()` from exactly that checkpoint — settled nodes stay settled, reset nodes re-run, a waiting gate re-registering its own waiter for free), and per-node **`onFailure`** (`retry` with a fixed backoff via the engine's own injected clock and an http `idempotent` opt-in gate, `fallback`/`skip`, and `repair`/`escalate` which synthesize a deterministic `error`-kind edge straight into the run so the existing taken-edge cascade routes to them with no second pass). **Theme I landed** ([PR #575](https://github.com/bilo-io/midnite-studio/pull/575)): the canvas-only `frame` node (six markdown slots, membership via a new `WorkflowNodeBaseSchema.frameId` read off the member rather than a list on the frame, rendered through the same `WorkflowNodeView` sized from its own config, `autoLayout` bounding it to its members' padded bounding box), Contract+Context prepended to a contained `agent` node's prompt via a new `ExecutorContext.promptPrefix` (never a fourth reserved interpolation root), and the `policy` node — a permission gate over a closed `WORKFLOW_ACTIONS` vocabulary, governance by graph reachability (`governingPolicies` reusing `ancestorIds`), a denied action blocking Run, and a `requireApprovalFor` action pausing through an **implicit gate that reuses Theme D's exact runtime path** (`gate-waiters.ts`/`decideWorkflowGate`, unmodified, keyed by the governed node's own id) rather than a synthetic gate node. **Theme K landed** (2026-09-26): a **change receipt** (`workflow-receipt.ts`'s pure `buildWorkflowRunReceipt`) attached to Phase 94 Theme A's `AgentRun` via a new `receipt` field and `fromWorkflowRun` adapter — context sources, policy version, node kinds/agents used, verifier verdicts, loop iterations, human decisions, wall-clock and the accepted artifact, no cost field — plus an additive **iteration scrubber** ("pass 2 of 3", `canvas/run-replay-iteration.ts`) ordered `(iteration, settledAt)` deliberately kept separate from the existing flat step scrubber (a global sort by that tuple would misorder anything downstream of a loop — documented on the p97 board), the run output panel's Nodes tab grouping a looped node's runs by pass, and a markdown "## Receipt" section.

- **[Phase 96 · Ollama: local and cloud models for agents](phases/phase-96-ollama-local-and-cloud-models.md)** (79% · 58/73, [PR #540](https://github.com/bilo-io/midnite-studio/pull/540), [PR #541](https://github.com/bilo-io/midnite-studio/pull/541), [PR #542](https://github.com/bilo-io/midnite-studio/pull/542), [PR #547](https://github.com/bilo-io/midnite-studio/pull/547), [PR #548](https://github.com/bilo-io/midnite-studio/pull/548), [PR #549](https://github.com/bilo-io/midnite-studio/pull/549)) — **Planned 2026-09-24, brainstormed with the user. All nine themes landed; the verification items remain, mostly human/packaged-app passes.** Adds Ollama to the toolchain (Health row, daemon probe, install, start), a main-side Ollama client, a Models rail view (installed, ollama.com search scraped and cached, cloud catalogue, streamed pulls) with a rich model-detail modal and a one-click 64k-context variant, and a per-agent "native | Ollama" backend for claude, codex, cline, opencode and copilot — env vars for claude/copilot, `--oss -m` for codex, `ollama launch` for cline/opencode — honoured by every launch path, plus the wand and Plan-with-AI on an Ollama model. **Themes A and B landed** (2026-09-24, PR #540): the foundation client/schemas/IPC and the Health page row. **Theme C landed** (2026-09-24, PR #541): the Models rail view (Installed list, pull queue, Settings ▸ Ollama host override + default model). **Theme H landed** (2026-09-24, PR #542): per-session pty env, `AgentDefinition.backends`, the persisted `{backend, model}` binding, `ollama-launch.ts`'s recipe table + `resolveAgentLaunch()` resolver wired into `start-agent.ts` (terminal/cards/loops/companion), `council-runner.ts` and workflow `executors/agent.ts`, a Settings ▸ Agent backend/model picker, and a session-row `SiOllama` mark. **Themes D and F landed** (2026-09-24, PR #547): the `ollama.com/search` scraper + Discover tab (debounced search, Local/Cloud scope, capability chips, variant picker), the `ollama.apiKey` vault slot + `secrets.has`, the Cloud tab + sign-in detection + Settings ▸ Ollama wiring, the `:cloud`/`-cloud` naming normaliser, and Theme H's `useOllamaKey` marker (closing its one open checkbox). **Themes E and G landed** (2026-09-24, PR #548): `agentFitness()`/`effectiveContextLength()`/`deriveNumCtx()`/`deriveEmbeddingLength()` in `shared/src/ollama.ts`; the `features/models/model-detail.tsx` modal (stats, capabilities, fit verdict, Modelfile/Template/Parameters/Licence tabs, Delete/Unload, Set-as-default) wired into Theme C's `ModelRow.onOpenDetail`; the one-click `<model>-64k` variant; and the same fit warning on Settings ▸ Agent's Ollama picker — since rebased onto PR #547 so Discover/Cloud rows for an already-installed model also open the modal (a not-yet-installed row still just shows Pull/Use, a documented follow-up). **Theme I landed** (2026-09-24, PR #549): `startAgent`'s per-launch `modelOverride` (it wins over `agentBackends` and never stacks a second `--model`), an Ollama group plus fit badge in the card composer's model picker, "Launch with…" in the detail modal, and the wand and Plan with AI on an Ollama model via `/api/chat`, chosen in Settings ▸ Agent ▸ "Headless AI features use". Follow-ups: the loop composer's Ollama group and the not-yet-pulled cloud catalogue in the picker.


- **[Phase 95 · Agentic improvements](phases/phase-95-agentic-improvements.md)** (83% · 63/76, [PR #524](https://github.com/bilo-io/midnite-studio/pull/524), [PR #526](https://github.com/bilo-io/midnite-studio/pull/526), [PR #527](https://github.com/bilo-io/midnite-studio/pull/527), [PR #528](https://github.com/bilo-io/midnite-studio/pull/528), [PR #532](https://github.com/bilo-io/midnite-studio/pull/532), [PR #533](https://github.com/bilo-io/midnite-studio/pull/533), [PR #534](https://github.com/bilo-io/midnite-studio/pull/534), [PR #535](https://github.com/bilo-io/midnite-studio/pull/535), [PR #536](https://github.com/bilo-io/midnite-studio/pull/536), [PR #537](https://github.com/bilo-io/midnite-studio/pull/537)) — **Planned 2026-09-23, brainstormed with the user. All ten themes landed; 13 verification items remain, mostly human/packaged-app passes.** Ten themes: one activity palette + glow (with a metallic ring for plain shells and an agent-icon / terminal badge wherever it shows) and its own Settings ▸ Activity page (A–C); forge issue/project CRUD with dependency-link writes (D); a magic-wand field rewrite and a Plan-with-AI blueprint that only touches the forge on Confirm (E–F); card controls and drag-to-skill (G); Auto-mate and a five-scope kill switch (H); and workflow editor parity with midnite on lazy-loaded React Flow, with agent/script nodes grouped per run in the terminal (I–J). A landed first (the shared `ActivityStatus` palette + `.activity-glow` CSS family + token migration for three of the six hardcoded status maps, two more consolidated, two left as documented exceptions — see the phase doc's own Theme A checklist). B added Settings ▸ Activity: presets, per-status overrides, agent/shell style and speed/intensity, live preview. C landed next: the one `useActivityGlow` hook plus an `ActivityBadgeStack` identity badge, wired into kanban cards, project graph nodes, terminal session rows and the Sessions view (workflow canvas nodes deferred to Theme I, which does not exist yet; kanban/graph nodes keep their existing ring paint, `useActivityGlow` now driving it as the decision layer — see the phase doc's own Theme C checklist for both calls). D added `ForgeAdapter` issue/project CRUD, per-operation capabilities and blocked-by / sub-issue link writes with a `Blocked by #N` body fallback (backend only; the delete confirm UI moves to Theme E). E added `IssueDialog`/`ProjectDialog` create/edit, a per-provider cheap/fast model registry (`shared/src/ai-models.ts`), a headless `ai:improveField` wand behind each text field (read-only + `.activity-glow` while generating, one-step Undo, client-side Esc-cancel), and the wand's provider defaulting to `primaryAgent` (Settings ▸ Agent) rather than new recency tracking. G split the old single Play/reveal button into Start/Stop/`>_` on cards and graph nodes, and added drag-to-skill: a drop into a column mapped in Settings ▸ Projects (`In progress`/`In review` by default, per-project overrides) moves the card then starts the mapped skill on the issue/PR link behind a 5s Undo toast. H added `projectRef`/`workflowRunRef`/`forgeAccountKey` session attribution; Auto-mate, a project-board toggle that fills up to a per-project concurrency cap (1–5, default 1) with unblocked Todo cards — reusing Theme G's own column → skill map rather than a second mapping surface, keyed off `ForgeGraphNode.blocked` rather than its narrower `.ready` flag — and stops on the first failed task; and the five-scope (Flow/Project/Repo/Forge user/Global) kill switch modal behind `automate.kill`, wired to a board-header toggle, a status-bar chip and the command palette (see the phase doc's own Theme H checklist for the unattended decisions). I ported midnite's workflow editor onto lazy-loaded `@xyflow/react` + `@dagrejs/dagre` (loaded only with the Workflows view — entry chunk unchanged, +236.6 KB confined to that lazy chunk), the node palette, config/run-history panel, run-replay step-through, live per-node run state on the editing canvas via a widened `workflowRunChanged` payload, and the card-style node view wired onto `useActivityGlow` (Theme C). F added an inline "Plan with AI" prompt + button (Projects toolbar, create-project dialog, issue
detail pane) wearing `TextField`'s new `gradient` prop for the "full glow when focused, gradient
border otherwise" look; a headless `ai:planBlueprint` (`fastModelFor`, one retry on bad JSON) drafts
a `{project, tasks, edges}` blueprint; an editable review sheet (locked under `.activity-glow` while
generating/re-planning, a mini dependency preview reusing `layoutForgeGraph` over synthetic draft
nodes) leaves the forge untouched until Confirm, whose pure, injected `plan-confirm.ts` sequencer
creates the board (or targets the one selected, or skips it on a forge with no board write) → issues
→ board adds → blocked-by edges — or sub-issues of an existing issue — stopping on the first failure
with a Retry-remaining that skips whatever already succeeded. J added the `agent`/`script` node kinds
themselves, executing in a real pty via the existing broker path (`node-sessions.ts`, never a second
spawn route) — a script node settles on `; exit $?` like a council member, an agent node on
activity-idle plus an explicit done marker; sessions land in their own terminal accordion group
(`WorkflowRunSessionsGroup`, pulled out of the by-repo grouping via a `WORKFLOW_SESSION_REPO_ID`
sentinel), with reveal-in-Workflows and a kill switch finally wired to the `flow` scope Theme H
shipped disabled; node glow reuses Theme C's `useActivityGlow` precedence unchanged, only threading
real sessions through it. Every theme is now landed — what remains is entirely the phase's own
human/packaged-app verification passes.
...
- **[Phase 92 · Agentic execution from Projects](phases/phase-92-agentic-execution-from-projects.md)**
  (100% · 36/36, PR #478, PR #481, PR #482) — **Landed, all five themes.** A kanban card's Play button and the dependency graph
  ...
  **E** closed out verification: the vitest coverage for every pure function, the store round-trip
  and both component suites had already landed with B-D and were confirmed present rather than
  rewritten; the one genuinely new piece was two Playwright cases per surface
  (`e2e/kanban.spec.ts`, `e2e/project-graph.spec.ts`) proving the menu-open/launch-and-close fork
  against the real `ContextMenu` portal and the real `pty:create` `initialInput`, which
  `use-card-play.test.tsx`'s isolated hook render cannot reach — raising the e2e ratchet 441 → 445
  with a committed justification. 36 items, five themes, no new dependency, no new IPC channel.
- **[Phase 90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md)** (72% · 79/110, [PR #490](https://github.com/bilo-io/midnite-studio/pull/490), [PR #491](https://github.com/bilo-io/midnite-studio/pull/491), [PR #501](https://github.com/bilo-io/midnite-studio/pull/501), [PR #503](https://github.com/bilo-io/midnite-studio/pull/503), [PR #502](https://github.com/bilo-io/midnite-studio/pull/502), [PR #504](https://github.com/bilo-io/midnite-studio/pull/504), [PR #505](https://github.com/bilo-io/midnite-studio/pull/505), [PR #506](https://github.com/bilo-io/midnite-studio/pull/506), [PR #508](https://github.com/bilo-io/midnite-studio/pull/508), [PR #511](https://github.com/bilo-io/midnite-studio/pull/511)) — **All eleven lettered themes (A-K) now landed; a twelfth, Theme L (the account switcher), was added to scope after K merged and is still open.** Not flipped to DONE: the phase's own Verification section and Theme K's own checklist both carry human-only, live-credential passes (a real GitLab/Bitbucket/Azure DevOps repo with a real PAT each, and a live `gh auth switch` round-trip) that no session has exercised since the phase began — recorded honestly in `outstanding.md` rather than rounded up. **#511** landed Theme K: fixture-driven mapper tests (one `__fixtures__/*.json` per provider, run end to end through each adapter's real read surface, not a private mapper called directly) plus a cross-provider comparison test proving the same three PR scenarios map to the same `ForgePull.state`/`isDraft` across all four adapters; a no-token-on-wire test wiring the *real* account registry behind the *real* IPC handler for all four providers; the account-switch cache-bleed test extended from two spot-checked query keys to a table over all sixteen; one e2e (`onboarding-wizard.spec.ts`, ratchet 448→449) proving `OnboardingModal`'s optional step is skippable despite a genuinely-simultaneous second modal (`FirstRunModal`) stacking underneath it by real DOM paint order; and one visual baseline (`pricing-page.spec.ts`) reaching into `packages/website`'s own dev server via a second `webServer` entry in `playwright.visual.config.ts`, its committed Linux baseline left for whoever next has Docker (this session's sandbox had neither Docker nor network access to install it). Also found and fixed along the way: `bitbucket-reads.ts` and `azure-reads.ts`'s own `listPulls` orchestration had zero test coverage beyond their pure mapper-table tests — Theme K's fixtures are their first. **#508** closed Theme B's and Theme D's last open items with no adapter code change: B's per-provider redaction patterns were already covered in `redact.ts` by the time E/F/G landed theirs (GitLab, Bitbucket, and Azure's own deliberate no-pattern decision), and D's `ForgeCliStatus` docblock now describes three HTTP adapters plus `gh` rather than `gh` alone. **#506** landed Theme G (Azure DevOps) — the third and last of the three HTTP-backed adapters, over the same `main/forge/http.ts` #504/#505 already settled, following the identical shape: `ForgeAdapter` untouched, `registry.ts`'s `adapterFor` synchronous, `setThreadResolved`'s thread id folding the PR number in. Work items mapped honestly as work items (not issues, `System.State`'s category rather than its name), the numeric `-10`/`-5`/`0`/`5`/`10` review vote mapped per the phase doc's own warning about the two cases a naive mapper gets wrong, and Azure Boards' column `stateMappings` read directly rather than needing a GitLab-style label rewrite — the phase's third `'partial'` capability row (`requestChanges`). Azure's Git API has no unified-diff endpoint at all, so `azure-diff.ts` adds a small tested LCS line differ that synthesises one from a change list plus raw blob content. Also discovered: an Azure account carries no organization in its own schema, closed by calling Azure's own account-listing endpoint in `reachable-repos.ts`. **#504** and **#505** landed in parallel, both behind Theme D's adapter seam, and reconciled onto one shared `main/forge/http.ts`: Theme F (Bitbucket Cloud, #504) built it first as the provider-neutral HTTP client Theme D deferred (auth as a strategy, a per-host request budget, one `Retry-After` retry) and landed the phase's first genuinely mixed capability row (`projects: 'none'`, `threadResolution: 'partial'`, otherwise `'full'`); Theme E (GitLab, #505) merged second and adopted #504's `http.ts` rather than keeping its own near-identical copy, adding GitLab's own row (`projects: 'partial'`, `requestChanges: 'none'` — no `CHANGES_REQUESTED` exists — otherwise `'full'`, including `repoListing`). Both adapters follow the same shape `registry.ts` settled on: `adapterFor` stays synchronous, closing over `account: ForgeAccount | null` and resolving its vaulted token lazily inside each call rather than eagerly up front. GitLab's MRs, issues, pipelines/jobs, MR discussions (conversation vs. diff-anchored threads) and Issue Boards (one synthetic label-backed field) all map onto the GitHub-canonical vocabulary; one interface-shaped workaround, not a widening of `ForgeAdapter`: `setThreadResolved` carries no MR number in its request, so the GitLab adapter encodes it into the thread id it itself hands out (`${mrNumber}:${discussionId}`) rather than reshaping the shared interface for one provider. Two GO-LIVE requirements merged, because they share one provider/credential abstraction: GitHub-equivalent Actions/Issues/Reviews/Projects for GitLab, Bitbucket and Azure DevOps, and an active-user avatar whose switch rescopes the repo list and runs `gh auth switch`. The grounding found the seam is two symbols wide — `githubForge()` and `hasGithubForge` — and that the credential decision is already precedented by `db/credential-vault.ts`. Billing is **out**: a pricing page ships on the website, a tier vocabulary ships unwired. **#503** closed Theme B's `forge.scopeReposToActiveAccount`/`forge.syncGhAuthSwitch` settings and the account-host scheme gap (its one remaining item, the per-provider redaction patterns, is genuinely deferred to Themes E/F/G by the checklist's own wording — E and F's own slices closed in #505/#504) and landed all of Theme C: repos hidden rather than closed on a switch (never hiding an org repo without positive evidence from the reachable-repos listing), a clone-or-open picker (now GitHub *and* GitLab), a gated `gh auth switch`, and a cache-poisoning fix built as `cancelQueries` + broad `invalidateQueries` rather than an `accountId` segment on all sixteen forge query-key builders — smaller, and clear of Theme D/H's concurrent edits to the same view files. **#502** landed Theme D's adapter seam — `main/forge/adapter.ts`, the moved `gh-*.ts` files behind `createGitHubAdapter()`, and `registry.ts`'s `adapterFor(forge, account)` as `forge-handlers.ts`'s single dispatch point, with a disclosed side benefit: a GitLab/Bitbucket/Azure repo now reports "no forge" instead of misfiring `gh` against the wrong owner/repo — and Theme H's exhaustive `capabilitiesFor` matrix (a compiler-enforced `Record<ForgeKind, ForgeCapability>`) read per-field by `app.tsx`'s `FORGE_GATED_VIEWS`, now with two real `'partial'` rows to render. Found and fixed along the way: `packages/app`'s `"type": "module"` broke every e2e spec's load of `test-support/mock-bridge.ts` the moment it imported a real value (not just types) from the CommonJS `@midnite/studio-shared` build, and that same mock had no `forgeAccounts` namespace at all (the gap #501's own log entry flagged) to answer the new per-view capability read.
...
- **[Phase 85 · The monitor that lied, and the memory it hid](phases/phase-85-the-monitor-that-lied.md)** (95% · 56/59 · **Refined x1**, [PR #362](https://github.com/bilo-io/midnite-studio/pull/362), [PR #363](https://github.com/bilo-io/midnite-studio/pull/363), [PR #364](https://github.com/bilo-io/midnite-studio/pull/364), [PR #365](https://github.com/bilo-io/midnite-studio/pull/365), [PR #366](https://github.com/bilo-io/midnite-studio/pull/366), [PR #367](https://github.com/bilo-io/midnite-studio/pull/367)) — **Themes A, B, C, D, E, F, and G landed.** The Workspace Optimizer's Memory tab reads `0` for every process's bytes and %CPU on some Macs, and the brainstorm reproduced it live on the reporter's M1 Pro: it is a **locale** bug, not an Apple Silicon one. `ps` formats `%CPU` through `LC_NUMERIC`, `parsePsOutput`'s six-column regex (`agent-process.ts:172`) accepts a dot only, and on `en_ZA.UTF-8` **all 679 rows** fall through to a four-column back-compat branch that hard-codes `rssBytes: 0, cpuPercent: 0` — and shifts the command line, so `commandName` returns `22560`, `PROTECTED_PROCESS_NAMES` becomes dead code, the kill button rejects legitimate kills on an `expectArgv` that moves between reads, and the session auto-namer matches polluted argv. Nothing could catch it: `nonnegative()` accepts `0`, and every fixture is four-column. **The x1 refinement changed four premises.** The fix **already exists one package over** — `git-exec.ts:52` has pinned `LC_ALL:'C'` since git-engine was written — so Theme A finishes a policy rather than inventing one. **`idle-cpu.mjs:98` has the identical bug** (`cputime` is `MM:SS.ss`; `Number('12,34')` is `NaN` and the row is dropped), so Theme F's own instrument reads zero and the fix moves into A as its prerequisite. **Electron 33 cannot supply macOS `phys_footprint`** — `ProcessMemoryInfo.residentSet` is `@platform linux,win32` — so Theme E drops footprint, keeps `getAppMetrics()` for our own tree joined with `ps` RSS, and *documents* the Activity Monitor delta instead of chasing it with a subprocess on a 5-second poll. And `styles.css` holds **32 `infinite` animations against exactly one `paused`**, which turns Theme F from "bisect the cause" into a ranked list — the Optimizer hero's three simultaneous drifts and its `background-clip: text` shimmer first — fixed with the `:root:has(…)` pause idiom already at `styles.css:809` and held by a `findUngatedLoops()` guard beside the existing `findUnguardedKeyframes`. **A** pins the locale on seven parsed main sites plus all three `scripts/perf/` `ps` calls, carves out four pty paths by name, adds the repo's first `no-restricted-syntax` rule and a `gate-locale` job on **macos-14** (ubuntu ships no `de_DE.UTF-8`); **B** deletes the inventing fallback, recaptures six-column fixtures, makes the numbers nullable rendering `—`, adds `ProcessTableResult.error`, and gives each of the three dead guards a test that would have failed; **C** lifts one shared process classifier (the two scripts' have diverged), adds CDP heap-snapshot diffing and an unattended `--soak`, and lands a verdict on the recorded `retainedPerCycleKb` breach; **D** attributes the idle RSS floor per process before cutting anything, with the deferred main-side repo snapshot as the named candidate and an honest "nothing to cut, here is why" as a landed outcome; **E** rebuilds the process table on `getAppMetrics()` with an Owner column; **F** closes Phase 36 Theme G — 28 of the 32 ambient `infinite` loops now `paused` at rest behind `:root:has(…)` gates, 4 allowlisted in `LOOP_GATE_ALLOWLIST` with human-written reasons, `findUngatedLoops` guard asserted; **G** puts every number in `budgets.json` with the run and machine behind it, verifies the README's "What is not measured here" is current (`--soak`/`--heap-diff`/`--idle` are automated, idle %CPU still needs a human), and confirms outstanding.md's Phase 84 Theme J and Phase 45 Theme F stale entries were already removed by Theme C. 59 items, seven themes, no new dependency. Themes A and B ship together or not at all — B without A empties the table instead of filling it with wrong numbers.

- **[Phase 84 · Live everywhere, lighter when hidden](phases/phase-84-live-everywhere-lighter-when-hidden.md)** (88% · 60/68) — **Landed.** Themes A, D and I landed ([PR #348](https://github.com/bilo-io/midnite-studio/pull/348)); Themes E and F landed ([PR #349](https://github.com/bilo-io/midnite-studio/pull/349)); Themes G and H landed ([PR #350](https://github.com/bilo-io/midnite-studio/pull/350)); Themes B and C landed ([PR #351](https://github.com/bilo-io/midnite-studio/pull/351)); Theme K landed ([PR #353](https://github.com/bilo-io/midnite-studio/pull/353)); Theme J landed ([PR #352](https://github.com/bilo-io/midnite-studio/pull/352)) — real before/after numbers for Themes E-I via two new `memory-report.mjs` modes (`--hidden-sessions`, `--hidden-tabs`) and `idle-cpu.mjs`'s new git/gh subprocess census, measured against `543869af` (the commit before Phase 84's first PR) and current `main`: idle RSS, per-session/per-tab held-cost floors and the popout RSS delta all read flat before → after (expected — E/F/G/H's savings only show up once a session/tab/view goes hidden past its own threshold, which a snapshot-style instrument doesn't reach), while B/C's fix is the one number that moves — a 90s blurred window spawned 3 (before)/2 (after G/H, before B/C)/0 (current `main`) `gh` processes. All eleven themes now landed. Surfaced, not fixed, and recorded in `outstanding.md`: `retainedPerCycleKb`'s existing 500 KB/cycle budget is breached by the `terminal`/`browser-tabs` retention actions on *both* sides equally, a pre-existing calibration gap, not a Phase 84 regression; the literal renderer-UI-driven E.6/F.5 numbers; K.7's own perf number; and Changes/Projects/Sessions' cascade wiring. The "why do I have to reload the other window" phase, and the grounding found the bug is one missing mount: main already runs one `RepoWatcher` per repo and `broadcastToAllWindows(watchEvent)` to every window, but `useWatchInvalidation` is mounted only in `app.tsx:616` — `detached-root.tsx` never subscribes, so a detached Graph receives the event and drops it, and `relayWatchEvent` has had zero callers since Phase 55 I (its test still passes). The rest of liveness is timers in the wrong place: auto-fetch is a renderer hook in the main window; forge data has no push and no poll (`FORGE_STALE_MS = 60_000` + navigation); `WindowDescriptor.repoId` is hardcoded `null`. **A** mounts the listener in `DetachedShell` and deletes the dead relay; **B** moves auto-fetch into a main `fetch-scheduler.ts` behind an any-window-visible + idle gate with an explicit `refs` broadcast (own-write suppression would swallow the fs event); **C** an interest-based `forge-poller.ts` — refcounted `{repoId, kind}` subscriptions, hash-diffed, `forgeChanged` ping, rate-limit back-off; **D** makes `repoId` real so broadcasts can be scoped. On memory: every open terminal session keeps a live xterm, hidden browser tabs keep their renderer process, views unmount hard and re-stream on return, and a popout boots the whole app — **E** disposes hidden sessions past visible + 3 recent after 2 min and rehydrates from main's scrollback, **F** discards hidden tabs Chrome-style after 10 min with a sleeping glyph and Keep awake, **G** keeps the last-left heavy view alive under a TTL and row ceiling with deferred re-stream, **H** puts popouts on a diet, **I** adds a liveness dot to every window's status bar, **J** measures all of it before and after. **K** lands a shared `useCascadeReveal` so graph, repos, explorer, Actions, Issues and Reviews cascade top-to-bottom on mount/reveal/repo-switch — never on a data refresh, which the graph used to do on every save — and terminal, companion, the repos panel and the FAB/Loops panel fade on every reveal, with an app-owned reduced-motion rule and a guard that can finally see Tailwind's keyframes. Explicitly deferred: a main-side repo state snapshot (N windows → one subprocess set), the natural Phase 85. 68 items, eleven themes, no new dependency.
...
- **[Phase 78 · Whose hands were on the keyboard](phases/phase-78-whose-hands-were-on-the-keyboard.md)** (81% · 26/32, [PR #456](https://github.com/bilo-io/midnite-studio/pull/456), [PR #460](https://github.com/bilo-io/midnite-studio/pull/460), [PR #459](https://github.com/bilo-io/midnite-studio/pull/459), [PR #465](https://github.com/bilo-io/midnite-studio/pull/465)) — **Themes A, B, C, D, and E landed; F open.** The novelty audit's one phase: agent provenance as a first-class fact in the graph. The log format reads no trailers (`LOG_FORMAT` stops at `%s`) although every agent this app runs already writes `Co-Authored-By:`; the roster knows five agents and none of their signatures; closed sessions carry `agentId`/`repoId`/`createdAt`/`closedAt`, exactly the join a commit needs. **A** reads `Co-Authored-By`/`Midnite-Session` trailers into `CommitSchema` (NUL-delimited, `%x1f` inside the field); **B** a pure `classifyProvenance` in `shared` with an ordered confidence (`session-trailer` › `co-author` › `author` › `session-window`) that renders *probably* out loud; **C** the mark on the graph row, an All/Humans/Agents filter that dims rather than hides, a Provenance line in commit detail; **D** the sessions ↔ commits crosswalk — "N commits" per closed session, a link from a commit to the transcript that made it; **E** the opt-in write side (`MSTUDIO_SESSION_ID` in agent ptys, a never-clobbering `prepare-commit-msg` hook behind a default-off switch); **F** an agent-share tile on the dashboard. Read-side first and works on history that predates the app; line-level blame attribution is the named sequel.

- **[Phase 77 · Thirteen megabytes of editor, and the lanes that never move](phases/phase-77-thirteen-megabytes-of-editor.md)** (0% · 0/42 · **Refined x1**) — **Planned, not started.** The performance audit, read against `budgets.json`'s own `_measured` block: total JS is **35.4 MB against a 15.95 MB budget** (2.2×, disclosed by Phase 64 G, unfixed) and `dist/` says where — `monaco-loader` 13.2 MB and `ts.worker` 6.8 MB (the tail — `emacs-lisp` 771 KB, `cpp` 767 KB — is **Shiki's, not Monaco's**, corrected at refinement x1: Monaco ships no emacs-lisp and its whole cpp tokenizer is 8 KB). `monaco-loader.ts:2` imports the package root, which is `editor.main` with every language contribution. **A** switches to `editor.api` + the feature list the code proves it needs, a lazy contribution registry behind a `useMonacoLanguage(id)` mount gate (no host calls `editor.create`, so there is nowhere else to hook), and an on-demand TS worker, aiming at the *existing* budget; **B** moves the lane layout's O(lanes)-per-row pass-through edges (`lane-layout.ts:155–167`, its own class doc's named bottleneck, Kilo scan #6) out of `edges` into a dense `through: number[]` carrying `.default([])` — **not** into column runs, which x1 found impossible where described (each row is its own `<svg>`, positioned by the virtualizer, so a multi-row line has no host element); the wins are layout time, IPC bytes and heap, measured on the 50k fixture with a new `graph-scroll` budget and proved by a painted-lane multiset assertion, since `MSTUDIO_SHOTS` never compared anything; **C** measures then debounces the whole-slice `localStorage` write on every `set` — retargeted at x1 from `useResizable` (already correct) to four `<input type="range">` handlers and the dashboard's `onLayoutChange`, widening to `midnite-studio.dashboard`; **D** rebaselines only on a documented miss and adds a non-blocking `perf-report` job writing the bundle table to `$GITHUB_STEP_SUMMARY` — not a PR comment, which would cost this repo its first job-level `permissions:` block — so a 2.2× breach never again sits unseen for a month.

- **[Phase 76 · The renderer in a sandbox, and the headers it never sent](phases/phase-76-the-renderer-in-a-sandbox.md)** (0% · 0/35) — **Planned, not started.** The security audit found the boundaries well drawn (zod at every `invoke`, a real fs jail, a `sandbox: true` preload-less browser partition, `safeStorage` for DB passwords, redaction on write, no `rehype-raw` anywhere) and five places the app is one layer thinner than its comments believe. **A** — *human-run, never a swarm*: Electron is `33.4.11`, off the supported line since 36 shipped; the theme is the deliberate upgrade with the native-ABI, `verify-dist`, perf-suite and broker legacy-peer checklist. **B**: `sandbox: false` on every app window for a reason that is no longer true — `scripts/bundle.mjs` inlines `shared` into the preload, and the only Node touch left is `node:os` for `homedir`/`hostname`, which the existing `additionalArguments` pattern removes; flip both windows to `sandbox: true` and add a bundle test that keeps `node:` out. **C**: there is no CSP anywhere and no `will-navigate` guard on the app's own window — a header-based policy from a pure `buildCsp()` (dev/packaged variants, `persist:browser` skipped) plus the guard. **D**: the Twelve Data key sits in plaintext `localStorage` (the store says so) and `ipwho.is` geolocates the user by IP on first title-bar load — a `safeStorage` vault behind an enum-keyed `secrets` channel, the finance fetch proxied through main, and IP lookup behind a default-off Privacy switch. **E**: the sweep Phase 65 B left — 45 hand-rolled `ipcMain.on` sites onto `handleSend`, two raw `ipcMain.handle` onto `handle`, and an eslint rule that keeps the count at zero. Renovate's open dependency PRs are untouched by every theme.

- **[Phase 75 · What blocks what, and the agent you point at it](phases/phase-75-what-blocks-what.md)** (93% · 99/106 · **Refined x1**) — **Closed as ✅ DONE** ([PR #204](https://github.com/bilo-io/midnite-studio/pull/204), [#206](https://github.com/bilo-io/midnite-studio/pull/206), [#207](https://github.com/bilo-io/midnite-studio/pull/207), [#208](https://github.com/bilo-io/midnite-studio/pull/208), [#205](https://github.com/bilo-io/midnite-studio/pull/205), [#210](https://github.com/bilo-io/midnite-studio/pull/210), [#215](https://github.com/bilo-io/midnite-studio/pull/215), [#216](https://github.com/bilo-io/midnite-studio/pull/216)) — all eight lettered themes (A–H) landed; the remaining 7 items are two scriptable perf checks plus five human-only product verifications, logged in [`outstanding.md`](outstanding.md). A board is a set; a backlog is a graph. [Phase 40](phases/phase-40-github-projects.md) brought a ProjectV2 board into the app and [Phase 41](phases/phase-41-agentic-kanban.md) let an agent run a card, but nothing in either can say that #212 cannot start until #199 lands, or that #204 has been startable for a week. This adds a third `projectsMode` beside table and board: a left-to-right dependency DAG with blockers upstream, blocked nodes greyed, and each edge animated according to the state of its two ends. `~/Dev/midnite`'s task graph is the design brief — the same `ready`/`unmetBlockerCount`/`foreign` vocabulary and the same five-state edge appearance — with everything underneath replaced, because that app owns its database and this one reads somebody else's board over `gh api graphql`. **The x1 refinement re-grounded every claim against the repo and found five premises wrong.** The kanban card **has worn the rainbow since Phase 37** — `styles.css:2094` defines `.card-run-glow` as a `--rainbow-ramp` conic border spun by `loop-glow-spin`, applied by `task-card.tsx:106`, with `kanban-glow-shots.spec.ts` already holding it — so Theme F generalises the class rather than building it, and `kanban.spec.ts:265-314` asserts the **literal class name**, making a rename a same-commit spec edit. `workflow-canvas.tsx` **paints nodes in raw SVG**, and `.card-run-glow` is a `background-clip: padding-box, border-box` trick no SVG element can wear, so nodes are DOM over an SVG edge layer with `workflow-geometry.ts`/`workflow-path.ts` reused verbatim. **There is no reusable card start path**: `launch()` is a component-local closure in `card-composer.tsx:139` and `startAgent` (`start-agent.ts:34`) is the only export, so the node opens the composer instead of starting anything — one start UI, one caller. **`CardPanelStack` has no exported open action** and `selectedItemId` was a `useState` local to `BoardView`, so selection lifts into `ProjectsView` and both modes share one panel. And **this repo has no `--status-*` vars at all** — the original edge table cited the crib's — so Theme E defines `--dep-done`/`--dep-active`/`--dep-idle` in both theme blocks. The refinement also killed a scale trap: `useCardStatus` takes three whole-slice store selectors and does a linear session scan, which at 300 nodes is 300 × O(sessions) per store tick, so the canvas gets one `useGraphAgentStates` hook building a `Map` in a single pass and the node becomes a pure component. Dependencies come from a four-source ladder — GitHub's own `blockedBy` (verified live, no preview header, no extra scope), a conventionally-named project field, then a prose parse — with `parent`/`subIssues` kept as a **separate** containment layer, because merging it would grey out every epic. The whole data change is four fields inside `PROJECT_ITEMS_QUERY`'s existing Issue fragment, so there is **no new IPC channel, no second round trip and no N+1**; `blockedBy` returns its nodes' titles and repos, so foreign blockers render for free. `dagre` was declined at ~90KB for ~40 lines of ranking. 106 items, eight themes, read-only, no new dependency.

- **[Phase 70 · The API client grows an environment, a test and a run](phases/phase-70-api-client-environments-tests-and-runs.md)** (0% · 0/50 · **Refined x1**) — **Planned, not started.** The second half of [Phase 66](phases/phase-66-api-client.md), split out of it during that phase's x1 refinement. It is a separate phase for a reason that is not arithmetic: **this is the phase that executes user-supplied JavaScript in the main process, and the phase that writes a secret to disk** — and [`workflow/executors/transform.ts:13`](phases/phase-43-workflows-mvp.md) already declined the first of those once, in writing, on the grounds that *"opening it here would drag a security review into a phase that otherwise has none."* Phase 66 keeps that property; this one accepts the cost deliberately, in one place, where it can be reviewed. Four themes. **Environments** get a two-tier `{{var}}` resolution and a secret overlay: a `type: 'secret'` row is written to a gitignored sibling `*.local.json` while the committed base file keeps the row with an **empty value** — a form for the next teammate to fill in, not a missing key they discover through an unresolved-variable warning — and `.midnite/api/.gitignore` is written into the *user's* repository, which is the correction that matters, since Phase 66's root-`.gitignore` entry protected exactly one repo's secrets: this one. The **sandbox** is a Node `vm` and the doc says plainly that `vm` is not a security boundary, so the boundary is built from what is: no `require`, no `process`, no `Buffer`, `codeGeneration.strings: false`, and — because `vm`'s `timeout` only interrupts synchronous code — **no async primitive at all**, so a script cannot hang asynchronously because it cannot start. Five specific escape attempts are asserted unreachable rather than assumed. Scripts are off by default for a collection this machine did not import, and the trust marker lives in the gitignored overlay so that trusting a file cannot be committed on the whole team's behalf. The **runner** is the one thing here that streams (`run-progress`/`run-done`, on `dbQueryBatch`'s pattern) — a run is unbounded in time where Phase 66's capped response is not — walks in file order, does not stop on a transport failure, and lists what an abort skipped instead of silently omitting it. **History** persists metadata only: no headers, no bodies, secret-matching query values rewritten to `{{key}}`, 200 entries with no sidecar file to forget to evict. `toCurl`/`toFetch` leave `{{var}}` unresolved on purpose, because the most common destination for a copied curl is a chat message. 50 items, five themes, no new dependency, blocked on Phase 66 Themes A, C, E and G.
...
- **[Phase 67 · The sessions you closed](phases/phase-67-the-sessions-you-closed.md)** (0% · 0/64 · **Refined x1**) — **Planned, not started.** The rail has had a **Sessions** row since Phase 23 and it still renders `SessionsPlaceholder` — the only one of nineteen `ViewId`s whose `VIEW_COMPONENT` entry is a stand-in rather than a view. What it should be is not a guess: `nav-icons.ts:81` gives it `LuHistory`, and the palette already calls it *'Agent Sessions'* with the keywords *'agent session history transcripts'*. **The fact the phase turns on is that there is no history to show.** Closing a session does not end it, it erases it — `terminal:forget` drops the row from `terminals.json`, throws away the in-memory ring and `rm`s `scrollback/<id>.bin` — and the three modelled states (`live`, `asleep`, `ended`) all describe a session that still exists. A fourth, *closed*, is nowhere. So the work is to record the ending, then render it. The pattern is already proven next door: `asleep` is documented as *'process killed, **transcript kept**'*, and the transcript is already on disk, already capped at 1 MB, already flushed every 15 s by two writers — nothing ever reads one back except a live terminal. This adds a `ClosedSession` record, a `session-history-store.ts` in `trust-store.ts`'s shape with a 200-cap that evicts the **file** as well as the row (Phase 45 found that exact bug twice), a `forgetTerminal` that **flushes before it renames** — without which the last 15 s of every session, the part you close it to read, is lost — three `mstudio:sessions:*` channels, a list built on `issues-view.tsx` rather than Councils, and a read-only DOM-rendered xterm beside it that spends none of the twelve WebGL contexts. It also fixes three live bugs found on the way: the palette shows `session.title` for every row, which is documented as *'the **repo name**'*; `state-dot.tsx` paints `exited` identically to `idle`; and the palette navigates to FAB sessions the terminal panel refuses to render. **Refined x1** re-grounded every path against the repo — Phase 60 landed in the meantime, so the view ternary the old Theme D edited no longer exists and the arm is a one-line `VIEW_COMPONENT` edit — split the oversized view theme into a list and a transcript, added a detachable-page theme now that the exclusion reason (*'a placeholder with no view behind it yet'*) is retired, and resolved all five opens. 64 items, six PR-sized themes, no new dependency.

- **[Phase 66 · API Client](phases/phase-66-api-client.md)** (26% · 19/73 · **Refined x1**) — **Themes A and B landed** ([PR #222](https://github.com/bilo-io/midnite-studio/pull/222), [PR #225](https://github.com/bilo-io/midnite-studio/pull/225)); C–H open. A Postman-compatible API client, joining the Workspace sidebar group as its **sixth** entry — Explorer/Search/Tests/Optimizer/Database, not the three the pre-refinement doc named. The x1 refinement **split the phase**: eleven themes and 58 items was a multi-week plan whose halves shared only a file format, so what stays here is everything on the path *open a real collection → build a request → send it → read the answer*, and environments, the `pm.*` sandbox, the runner and history all moved to [Phase 70](phases/phase-70-api-client-environments-tests-and-runs.md). The largest correction is that **the phase's whole editor premise was dead**: every body and script editor was specified on CodeMirror 6, and [Phase 64](phases/phase-64-offline-monaco-and-themes.md) Theme C replaced the Files-view editor with Monaco — `code-editor.tsx:24` says *"replaces CodeMirror 6"* outright, the seven `@codemirror/*` deps have **zero** importers left in `packages/app/src`, and Phase 64 Theme G exists to delete them. That also dissolves the old Decision 3: Monaco 0.56 ships Monarch grammars for `graphql`, `xml` and `html`, and `monaco-languages.ts` already maps `graphql: 'graphql'` — no community package, no hand-rolled highlighter, no plain-text fallback. Four more premises were wrong on grep: the channel names violated `channels.ts:6`'s own documented `mstudio:<domain>:<verb>` kebab convention; the dirty-tab item cited a *"`tab-strip.tsx` unsaved-state convention"* that does not exist (`WorkbenchTab` is a closed four-arm union of read-only surfaces, and the app's only dirty glyph is in `file-preview.tsx`); `view-registry.tsx`'s `VIEW_COMPONENT: Record<ViewId, ViewEntry>` — a `Record`, so a new `ViewId` without an entry fails typecheck — went unmentioned entirely, as did `ui-store.ts`'s separate `VIEW_IDS` array that `viewForPath` actually derives from; and the `*.local.json` gitignore rule was written into *this* repo's root `.gitignore`, which would have protected exactly one repository's secrets — the user's collections live in the user's repo, so Phase 70 writes `.midnite/api/.gitignore` there instead. Testing was corrected the same way: the send engine is proven against `startFixtureServer()` on loopback, not a `fetch` mock, because that is the repo's own precedent and it exists so the suite passes *"with the machine's network cable out"*. Send stays in main behind one channel, with interpolation resolved there so a secret never lives in renderer state, an unresolved `{{var}}` left literal rather than substituted into a lie, and a `{{var}}` tier limited to the collection's own `variable[]` so a collection is self-contained without an environment. 73 items, eight themes, no new dependency, no new package.

- **[Phase 65 · Somewhere for a crash to go](phases/phase-65-somewhere-for-a-crash-to-go.md)** (88% · 43/49, [PR #170](https://github.com/bilo-io/midnite-studio/pull/170) + [PR #178](https://github.com/bilo-io/midnite-studio/pull/178)) — **All five themes landed**; only human verification passes remain. [Phase 60](phases/phase-60-view-registry-and-error-boundaries.md) builds the error boundaries; this is the place their reports go, because today there is nowhere. The renderer has **no logging channel at all** — `console.error` and `console.warn` return **zero** hits in `packages/app/src`, enforced by `no-console: 'error'`, and there is no `window.onerror`, no `unhandledrejection` and no `componentDidCatch` anywhere. Main's seam is 14 lines (`Logger = (message: string) => void` over `console.warn`) with ~40 call sites writing to a stderr a packaged user never sees — and its own docstring wrongly claims the broker redirects it to a file, when `broker-client.ts:181` only redirects the *child process's* stdio. Nothing in `packages/desktop/src` rotates anything (**0** hits), and main catches almost none of its own crashes: no `uncaughtException`, no `unhandledRejection`, no `child-process-gone`, no `crashReporter`. Fifth fact: there is no **"report a bug"** link anywhere (**0** hits) even though the tracker lives in `bilo-io/midnite-apps` and `release.ts` already holds four URLs into it. This adds levels to the one seam (a callable type with methods, so all ~40 call sites compile unchanged), a rotating NDJSON sink under `userData` following the injected-directory convention every other store uses, an `mstudio:report:*` channel group (**not** `mstudio:diag:*` — already taken by the repo-lint runner), the missing `handleSend` counterpart to `handle.ts`'s four invoke helpers, and two buttons in the Diagnostics accordion that already exists. Home-dir redaction is a deliverable, not a nicety: the "Copy diagnostics" output is designed to be pasted in public. 49 items, five small themes, no new dependency, no telemetry, nothing leaves the machine without a button press.

- **[Phase 64 · Offline Monaco Editor & Cross-Surface Theme Engine](phases/phase-64-offline-monaco-and-themes.md)** (79% · 57/72 · **Refined x1**, [PR #164](https://github.com/bilo-io/midnite-studio/pull/164), [PR #171](https://github.com/bilo-io/midnite-studio/pull/171), [PR #221](https://github.com/bilo-io/midnite-studio/pull/221)) — **all seven themes now landed** (2026-09-06); only human/real-machine `## Verification` passes are left open (see below). Themes A-F (2026-09-05): offline Monaco bundling with inlined workers, the `StudioPalette` cross-surface theme registry (chrome/terminal/editor/Shiki, 6 built-in presets), the writable Monaco editor in the Files view, chord-yielding so Monaco keeps its own keystrokes, a VS Code theme JSON importer, and a "Palette" accordion on the Appearance page with light/dark finally surfaced there too. **Theme G** (PR #221, 2026-09-06) decommissioned CodeMirror: its gate on [Phase 61](phases/phase-61-database-explorer.md) cleared the same day — Phase 61 Theme G (PR #211) landed with a Monaco-based `query-editor.tsx`, not the CodeMirror one this paragraph (and Phase 61's own doc) originally described, overriding Phase 61's own Decision 9 because Phase 64's Monaco migration had by then already shipped — reconfirmed against the tree at execution time (zero `@codemirror/*` import sites). Removed the seven now-unused `@codemirror/*` dependencies (`pnpm install`: -30 packages; doesn't survive transitively via `@bilo-io/ui`/`@bilo-io/shell` either, unlike `lucide-react`). Two of the theme's four remaining checklist items turned out to be **already done** by PR #164, stale in the phase doc rather than actually open: the e2e suite's CodeMirror selectors and the lazy-boundary comment had both been migrated onto Monaco when Theme C landed. Bundle re-measured packaged-equivalent: entryKb/totalJsKb are **identical** before and after the dependency removal (1434.7/35405.7 KB both) — `@codemirror/*` had zero source importers left and was already excluded from the build by tree-shaking, so the win is dependency-tree hygiene, not a bundle-byte reduction. Disclosed, not fixed (pre-existing, confirmed unrelated to this removal): `totalJsKb` now exceeds its 15950 KB budget from accumulated Monaco worker-chunk growth in Themes A-C — `bundle-budget.spec.ts` runs outside `moon run :test`'s gate. The x1 refinement corrected **four premises**, each one grep. `themeMode` in `ui-store.ts` **does not exist** — light/dark is owned by `@bilo-io/ui`'s `ThemeProvider`, persisted at `localStorage['midnite.theme']`, with **four** modes including a `time` mode the plan never knew about. **There is no CSP** (`Content-Security-Policy`, `onHeadersReceived`, `webSecurity` → 0 hits each, and `index.html` already ships an inline script one would break); the real constraint is the `file://` opaque origin from `loadFile`, which is why workers must be `?worker&inline` — the same reason Shiki's WASM already ships inlined. **Theme D was backwards**: the dispatcher is a capture-phase `window` listener calling `stopPropagation()`, so the app already wins every chord and Monaco never sees one — the work is teaching it to yield, plus three native accelerators (`Cmd+G` find-next, `Cmd+L`, `Cmd+O`) that bypass the yield list entirely. And **an Appearance page already exists** whose first accordion is titled "Theme". It also found a **fifth themed surface** — Shiki, pinned to two GitHub themes by a literal return type, painting the read-only preview this phase deliberately keeps plus diffs and slides, so "Monokai" would have left three code surfaces on GitHub — and a **direct conflict with [Phase 61](phases/phase-61-database-explorer.md)**, which states in writing that it uses CodeMirror *"rather than adopting Monaco or a second editor stack"* and builds on the very file this phase replaces; the new Theme G is gated on it. Decision 12 recommends splitting: honestly refined it is 50 deliverables in two halves that share only that Monaco needs a theme.
...
- **[Phase 61 · Database Explorer](phases/phase-61-database-explorer.md)** (79% · 74/94 · **Refined x1**, [PR #165](https://github.com/bilo-io/midnite-studio/pull/165), [PR #173](https://github.com/bilo-io/midnite-studio/pull/173), [PR #211](https://github.com/bilo-io/midnite-studio/pull/211), [PR #217](https://github.com/bilo-io/midnite-studio/pull/217)) — **all ten themes now landed** (2026-09-06); Phase 61 is buildable-complete, with only human/real-machine `## Verification` passes left open (see below). A DataGrip-style database client: shared contracts, `packages/db-engine`'s five drivers (Postgres/MySQL/MariaDB/MSSQL/SQLite) behind a connection pool, `desktop`'s IPC + `credential-vault.ts`, a schema tree, a query tab editor and a virtualized, inline-editable results grid. Decision 6 (the dual-ABI question gating Theme C) resolved to `better-sqlite3` rather than its own `node:sqlite` recommendation — checked against the tree, not assumed: Electron 33.4.11 bundles Node 20.18.3, which has no `node:sqlite` at all (needs ≥22.5), confirmed by running the packaged Electron binary directly. Decision 9 (the CodeMirror-vs-Monaco question with Phase 64) resolved the other way too: Phase 64's own Monaco migration had already landed by the time Theme G executed, so `query-editor.tsx` is Monaco — no new dependency, and Phase 64 Theme G is now unblocked to remove `@codemirror/*` (see its own row). Decision 7 (the workbench-store scoping question) resolved to its own recommendation — a single unscoped store with a second `activeQueryTabId` cursor. **Theme J** (PR #217) closed the phase's testing debt: four Playwright specs covering the full query loop, the destructive-statement gate, an inline-edit round trip plus a manufactured staleness conflict, and independent query tabs; three more shots (query+results, confirm, conflict) added to Phase 56 Theme G's shared `database-shots.spec.ts` rather than a bespoke file; and Decision 5's CI service containers (Postgres/MySQL/MariaDB on a new `ubuntu-24.04` job — `gate` runs macOS, where Docker service containers don't exist — MSSQL still a documented manual pass). It also fixed a real bug found running those specs: the query-editor and results-grid panes were collapsing to ~5px tall (a missing `flex` class on their wrapper), found because a Monaco click could never land. Left open, disclosed rather than silently dropped: Theme H's staleness re-check runs as two sequential calls rather than one wrapped transaction, and the packaged-app verification (`desktop:dist && desktop:verify-dist`'s new native-module assertion, already CI-wired in the macOS `package` job on push-to-`main`, and a real round-trip against live Postgres/MySQL/MariaDB/MSSQL/SQLite instances) is left for a human pass.
...
- **[Phase 58 · Notes, and the menu that holds them](phases/phase-58-notes-and-the-menu.md)** (97% · 76/78 · **Refined x1**) — **Landed.** A thought you have while reading a diff has nowhere to go: every surface in this app is about work that already exists, and nothing catches work that doesn't yet. This phase adds **Notes** — a per-repository list in a centred, gradient-ringed modal on `localStorage`, edited in place — where each note can hand itself to the workflow the repo already runs, in a terminal cwd'd to that repo (typed, never auto-sent), flipping the note to `planned`. The x1 refinement changed the phase's shape twice. **The handoff is already built**: `features/agent/midnite-menu.tsx` already resolves the agent, cwd's the terminal and types-not-sends, and the two skill strings are *user-configurable settings* (`DEFAULT_AGENT_SKILLS`), so the original Theme D would have been a fork that silently ignored anyone who re-pointed them — it is now an extraction. And **the modal count was wrong and so was the diagnosis**: there are twelve `z-dialog` overlays, not seven, ten of them already use `useFocusTrap`, and what they actually get wrong is the occluder pair — **eleven of the twelve are painted underneath a live `WebContentsView`**, which turns Theme B from a tidy into a bug fix. Also reversed: notes are no longer GC'd on repo close (the store it copies declines exactly that), and Theme F's "confirm no native accelerator" became a fix, because `menu.ts:120` registers `CmdOrCtrl+L` today and it fires with a terminal focused. Themes E, F, G ([PR #183](https://github.com/bilo-io/midnite-studio/pull/183)) close the phase: one `QuickAccessMenu` behind both the FAB and the previously-blank assistant-menu popover (`L`/`N`/`I`/`G` mnemonics), `Mod+l` re-pointed from the Loops panel straight to that menu, a chord-free `notes.toggle`, and full Playwright coverage of both entry points, the note lifecycle, the typed-not-sent handoff, and the browser occluder contract. Two human-only bullets (a few days' lived-in feel, a visual pass on the gradient ring) stay open by design.

- **[Phase 57 · Midnite Studio speaks MCP](phases/phase-57-mcp-server.md)** (96% · 73/76 · **Refined x1**, [PR #166](https://github.com/bilo-io/midnite-studio/pull/166), [PR #177](https://github.com/bilo-io/midnite-studio/pull/177)) — **All six themes (A–F) landed** (2026-09-05): the app is now an **MCP server** — a build-fingerprinted Unix socket under `userData` plus a bundled stdio shim, cribbing the transport trick the pty broker already proved — serving eight read-only tools (`repo.list`, `repo.resolve`, `status.get`, `graph.log`, `diff.file`, `branch.list`, `forge.pulls`, `forge.checks`), every one resolving `repoPath` through `resolveRepoRoot` + the repo registry rather than `fs-scope.ts`. Tool **inputs** are new path-keyed schemas (`McpRepoTarget`) rather than reused `ipc/schemas.ts` `RepoId` extensions — an agent knows a path, never an id — while **outputs** reuse `StatusResultSchema`, `GraphRowSchema`, `RefSchema` and the forge result schemas verbatim; `checksVerdict` was lifted from `packages/app` into `shared` since `forge.checks` needed the identical logic from main. **Themes E and F** (PR #177) closed the gap: the enable flag now lives in a main-side store (`mcp-store.ts`, off by default) wired to two new IPC channels (`mcp.get`/`mcp.set`), `repoPath` resolution is hardened with a `realpath` symlink/TOCTOU check against the repo registry (refuse, never clamp), the 50-entry audit ring logs tool/repo-root/outcome/duration with no payload bodies, and a new Settings → MCP Server page (registered in all three of `SettingsPageId`, `SETTINGS_PAGES` and `settings-view.tsx`'s `PAGES`) renders the socket path, the `claude mcp add` line, the live tool list and the audit ring, plus a status-bar indicator visible only while the server is on. Remaining scope is three **human verification passes** (live `claude mcp add` smoke test, quit/relaunch continuity, an end-to-end agent task over MCP only). Read-only and off by default on purpose: write tools need the write queue and a real consent model, and are deferred to a follow-up.
...
- **[Phase 53 · The first release](phases/phase-53-first-release.md)** (66% · 39/59 · **Refined x1**, [PR #179](https://github.com/bilo-io/midnite-studio/pull/179), [PR #188](https://github.com/bilo-io/midnite-studio/pull/188), [PR #220](https://github.com/bilo-io/midnite-studio/pull/220)) — **Themes A–E landed; F blocked on a human; G, H partial.** PR #220 is a verification-only pass that closed five stale `## Verification` checkboxes that PRs #179/#188 had already satisfied without anyone checking them off: `moon run root:version-check` (the CI gate `moon ci` runs as its "Version check" step) does fail on a deliberately divergent package version, proven live by a mutate-and-revert; the CLI wrapper's derived version leaves nothing to disagree with `package.json`; the status-bar pill's `error`/`checking` states are already rendered and tested; the six skill helpers (`planVersionBump` et al.) all resolve to real, tested exports in `packages/shared/src/version.ts`; and the ⚠️ banner is confirmed gone from all six skill files, with `.claude`/`.agents`/`.codex` differing only in the same established per-tool frontmatter substitution every other mirrored skill carries. `git tag | wc -l` is still **0**, `bilo-io/midnite-apps` still has **zero releases**, and its `midnite-studio/version.json` still reads `"version": null` — the phase's central premise stays intact until a human runs Theme F. **B**: a seeded root `CHANGELOG.md`, `scripts/version-check.mjs` asserting the lockstep invariant as a grouping (not pairwise equality) and wired into CI, the CLI wrapper's hardcoded `0.1.0` (Theme A's own release-blocker) now **derived** from the bundle rather than checked, the six helpers the release skills call by name but that existed nowhere ported into a new `packages/shared/src/version.ts`, and `/midnite-release-complete`'s unimplementable changelog precondition restated against the real `string | null` signature. **C**: `verify-dist.mjs` gains three gates additive to its existing ten — `latest-mac.yml` (version, path and a real computed `sha512` match against the zip), the `.blockmap`, and `Info.plist`'s `CFBundleShortVersionString` — each proven by breaking it on purpose against a real `moon run desktop:dist` build. **D**: `.github/workflows/release.yml`, tag-triggered, cross-repo publish to `bilo-io/midnite-apps` under the namespaced tag, reusing `ci.yml`'s `package` job as the literal build template and cribbing four guards from the sibling app's own broken-release history; `RELEASES_REPO_TOKEN` still needs creating before it can run for real. **E**: a `publish-feed` job (`needs: release`) commits `latest-mac.yml` and mirrors the released changelog section into `bilo-io/midnite-apps`, and all six release-skill files had their stale `⚠️` banner removed. **G (partial)**: the status-bar pill now shows an error and a checking state instead of `null` for both, and the persisted update channel is pushed to main once on boot so a beta user survives a relaunch — its one remaining item (confirming the pill reaches `available` against a live feed) is blocked on Theme F. **H (partial)**: `docs/RELEASING.md` documents all six release secrets, `verify-dist` records which signing mode a build shipped in; flipping `notarize: true` (needs a purchased cert) and a receiving-repo README edit remain, both genuinely blocked (credential / different repo). **F remains**: the actual v0.1.0 cut, needing a human to create `RELEASES_REPO_TOKEN` and run the release flow for real.
...
- **[Phase 37 · A glow that knows which tab](phases/phase-37-fab-tab-glow.md)** (100% · 44/44) — All six themes landed in one batch ([PR #8](https://github.com/bilo-io/midnite-studio/pull/8), 2026-09-02). The FAB panel's rotating rainbow border now grows an **inner glow**: a blurred conic layer masked to the rim so it falls off smoothly to nothing before the centre, breathing rather than static, and pulsing at a cadence tied to loop state. The glow is **tab-reactive** — each of the four loops claims the 180° of ramp centred on its own hue (Medic→rose, Watchdog→amber, Automate→emerald, Innovate→blue), and the far half is subtracted, with border and glow driven from one shared arc mask so they never disagree; the collapsed FAB button and each tab's own Start/Stop button pick up the same arc for free. Also tokenised the seven-stop rainbow that used to sit hard-coded in five places. Rotation and pulse gated unconditionally on window focus; layout bounds and theme parity verified.
- **[Phase 36 · Faster, lighter, same app](phases/phase-36-performance-diet.md)** (100% · 64/64 · refined x1) — All eight themes landed (2026-09-01, local). The app's first dedicated performance phase, and it kept its own rule: every landed item carries a number. **Entry chunk 2 481.3 → 1 084.7 KB** (−56%) by putting thirteen views, xterm and the markdown pipeline behind lazy boundaries under one Suspense; **`ready-to-show` 683 → 570 ms** by taking the synchronous login-shell probe (a median 284 ms of blocked main thread) off the boot path and parallelising the three `whenReady` chains; **the broker went from 12.74% to 1.16% of a core per MB/s** — 11× less CPU per byte — once pty output was coalesced into one frame per 16 ms instead of one socket write *and* one whole-buffer scrollback realloc per chunk; and the `ps` probe's cadence doubled after being costed at 4.08% of a core. `moon run app:perf` is the phase's legacy: strict budgets plus absence assertions that fail the day someone re-adds a static import. Four of the doc's items were **acquitted rather than churned**, each with the measurement that acquits it — the three handler-module deferrals, the `@dnd-kit` split, `manualChunks`, and a `lucide-react` assertion a dependency makes unassertable. All automated tests pass and manual verification criteria satisfied.
...
- ✅ **A** — typed ports + edge kinds (data/conditional/loop/error), per-port output shapes, `canConnect`, legacy-edge migration ([PR #557](https://github.com/bilo-io/midnite-studio/pull/557))
...
- ✅ **C** — controlled cycles: `loop` back-edges, iteration-scoped runs, failures carried forward, convergence/budget stop, `exhausted` escalation ([PR #560](https://github.com/bilo-io/midnite-studio/pull/560))
- ✅ **D** — human `gate` node: `waiting` status, run panel, bell, glow, timeout, MCP decide, PR/issue comment, Auto-mate + kill switch ([PR #562](https://github.com/bilo-io/midnite-studio/pull/562))
- ✅ **E** — `verify` node: agent-as-checker verdict + exit-code / test-counts / JSON-path checks ([PR #564](https://github.com/bilo-io/midnite-studio/pull/564))
- ✅ **F** — `router` node: expression or agent-label cases with a default port ([PR #563](https://github.com/bilo-io/midnite-studio/pull/563))
- ✅ **G** — durable run state (`state` node), checkpoints, resume `interrupted` runs, per-node failure policy ([PR #574](https://github.com/bilo-io/midnite-studio/pull/574))
- ✅ **H** — `trigger` node: manual, cron while the app is open, forge PR opened/updated via the poller ([PR #565](https://github.com/bilo-io/midnite-studio/pull/565))
...
- ✅ **A** — Ollama row on the Health page: binary + daemon probe, brew install/update, Start Ollama (never stop) ([PR #540](https://github.com/bilo-io/midnite-studio/pull/540))
- ✅ **B** — main-side Ollama client (`tags/show/ps/pull/delete/create/version`), `mstudio:ollama:*` IPC, streamed pull queue ([PR #540](https://github.com/bilo-io/midnite-studio/pull/540))
- ✅ **C** — Models rail view: Installed tab, pull-by-name, pull queue, daemon-down state, Settings ▸ Ollama (host override + default model) ([PR #541](https://github.com/bilo-io/midnite-studio/pull/541))
- ✅ **D** — Discover: main-side ollama.com search scrape + cache, fixture-tested parser, local/cloud filters ([PR #547](https://github.com/bilo-io/midnite-studio/pull/547))
- ✅ **E** — model detail modal: stats, capabilities, context, fit-for-agents verdict, Modelfile/template/licence tabs ([PR #548](https://github.com/bilo-io/midnite-studio/pull/548))
- ✅ **F** — cloud models: `ollama signin` detection, vault-held `OLLAMA_API_KEY`, cloud catalogue, name normalisation ([PR #547](https://github.com/bilo-io/midnite-studio/pull/547))
- ✅ **G** — context-length fitness check and one-click `<model>-64k` variant ([PR #548](https://github.com/bilo-io/midnite-studio/pull/548))
- ✅ **H** — per-session pty env + per-agent backend binding; hybrid launch recipe (env / `--oss` / `ollama launch`) on every launch path ([PR #542](https://github.com/bilo-io/midnite-studio/pull/542))
- ✅ **I** — per-launch Ollama model override, Launch with… from the modal, wand / Plan-with-AI via `/api/chat` ([PR #549](https://github.com/bilo-io/midnite-studio/pull/549))
...
- ✅ **F** (PR #536) — Plan with AI: editable blueprint sheet, re-plan, confirm creates project + issues + links (or sub-issues)
- ✅ **G** (PR #532) — start / stop / `>_` controls on cards and nodes; drag-to-column fires the mapped skill behind an Undo toast
- ✅ **H** (PR #535) — session attribution refs, Auto-mate (next unblocked, cap default 1), and the five-scope kill-switch modal
...
- ✅ **J** (PR #537) — agent/script workflow nodes in real ptys (existing broker path), grouped per run in a terminal accordion with reveal + a Flow-scoped kill switch, node glow threading real sessions through Theme C's existing precedence
...
already carrying `bug`/`feature` issue-form templates and the `bug`/`enhancement`/
`app: midnite-studio` labels this phase applies, with zero issues filed today. `bilo-io/midnite-studio`
(this repo) has a real but private, single-collaborator issue tracker no external reporter can
reach. Phase 65 built the report machine end to end and stopped at one `openExternal` call; this
...
- ✅ **A** — extract the two copy-pasted Play-button implementations into one `useCardPlay` hook (PR #478)
- ✅ **B** — `composeSkillLaunchPrompt` (skill + issue link); `composeCardPrompt` itself untouched (PR #478)
- ✅ **C** — `cardSkillByTask` local state + an `IconSelect` picker in `CardDetail` (PR #481)
- ✅ **D** — Play forks on whether a skill is set: launch directly, or a pointer-anchored menu (PR #481)
...
holds a username — right for one forge, and neither survives four. The seam is narrower than it looks:
`ForgeKindSchema` already carries `gitlab`, `remote-url.ts` already classifies self-hosted GitLab hosts,
and `forgeIssueUrl` already branches on kind for GitLab's `/-/` paths. What stops there is
`githubForge(repoId)` (`forge-handlers.ts` line ~70, **24 call sites**) and `repos-panel.tsx`'s
`hasGithubForge`. This phase widens both, generalises `db/credential-vault.ts`'s `safeStorage` vault for
per-account PATs, builds the "who am I" concept the app has never had (avatar, switching, `gh auth
switch`), implements three HTTP adapters, and ships a pricing page on the off-graph website. **Billing
enforcement is explicitly out of scope** — a tier vocabulary lands unwired and a verification line
asserts it stays that way. Grounding corrected one premise: **there is no setup wizard** —
...
- ✅ **A** (PR [#490](https://github.com/bilo-io/midnite-studio/pull/490)) — the seam: `ForgeKind` widens to five, Azure's `_git`/`v3` URL grammar, `githubForge` → `repoForge` across 24 call sites
- ✅ **B** (PR [#501](https://github.com/bilo-io/midnite-studio/pull/501), PR [#503](https://github.com/bilo-io/midnite-studio/pull/503), PR [#508](https://github.com/bilo-io/midnite-studio/pull/508)) — accounts, the vault as a third binding over `main/secure-store.ts` (Phase 76 Theme D had already extracted it, so no `secret-vault.ts` was created), `whoami` per provider, five `mstudio:forge:*` channels, the title-bar avatar, Settings ▸ Accounts, and the `agentApiKeys['github']` → vault migration. #503 closed two of the three items #501 left open: `forge.scopeReposToActiveAccount`/`forge.syncGhAuthSwitch` now join the persisted settings (`forgeAccounts`/`forgeActiveAccountId` had already reached `ui-store.ts` with #501), and `normalizeForgeAccountHost` closes the account `host` field's scheme gap (the general `shell.openExternal` path was already checked before this phase). **#508 closed the last item**: the per-provider redaction patterns were genuinely deferred to E/F/G, and by the time #508 checked, all three had already landed theirs — GitLab's `glpat-`/`gldt-`/`glrt-` (#505), Bitbucket's Atlassian `ATATT3x…` token (#504), and Azure's deliberate no-pattern decision (#506, a 52-char base64 PAT with no fixed prefix). No code change needed; `redact.ts`/`redact.test.ts` already covered all four providers
- ✅ **C** (PR [#503](https://github.com/bilo-io/midnite-studio/pull/503)) — switching the active user: repos hidden not closed (`isRepoVisibleForAccount` never hides an org repo without positive evidence), a GitHub-only reachable-repos listing with a clone-or-open flow, `gh auth switch` gated behind a default-on setting and only for a `delegated: 'gh'` account, and the cache-poisoning fix as `cancelQueries` + broad `invalidateQueries` rather than literal per-key `accountId` segments (avoiding a 16-key-builder rewrite across five view files Theme D/H were editing in parallel)
- ✅ **D** (PR [#502](https://github.com/bilo-io/midnite-studio/pull/502), PR [#504](https://github.com/bilo-io/midnite-studio/pull/504), PR [#508](https://github.com/bilo-io/midnite-studio/pull/508)) — `ForgeAdapter` and `registry.ts`, with the existing `gh-*.ts` moved verbatim behind `createGitHubAdapter()`; `forge-handlers.ts` dispatches through `registry.ts`'s `adapterFor(forge, account)` instead of calling `gh-cli.ts`/`gh-write.ts`/`gh-graphql.ts` directly, and a GitLab/Bitbucket/Azure repo now reports "no forge" honestly instead of running `gh` against the wrong owner/repo. Every moved `gh-*.test.ts` passes unchanged. #504 built the item this theme deferred — `main/forge/http.ts` — against its own first real caller (Theme F); #505 (Theme E) landed in parallel and adopted that same `http.ts` rather than keeping the near-identical copy it had built concurrently, keeping one provider-neutral client rather than two. **#508 closed the last item**: `ForgeCliStatusSchema`'s docblock (`shared/src/domain/forge.ts`) now describes what it means with three HTTP adapters live — `binPath` stays `gh`-only, `not-installed` unreachable for GitLab/Bitbucket/Azure, only GitHub still delegates to `gh` (`delegated: 'gh'`)
- ✅ **E** (PR [#505](https://github.com/bilo-io/midnite-studio/pull/505)) — GitLab: MRs (`iid`, not `id`), issues, pipelines, discussions, approvals with no `CHANGES_REQUESTED`, Issue Boards as the kanban via one synthetic label-backed field. `setThreadResolved` carries no MR number in its request, so the adapter encodes it into the thread id it hands out (`${mrNumber}:${discussionId}`) rather than reshaping `ForgeAdapter` for one provider
- ✅ **F** (PR [#504](https://github.com/bilo-io/midnite-studio/pull/504)) — Bitbucket Cloud: PRs, Pipelines (a tested `state.name`/`result.name` table), inline threads synthesised from Bitbucket's flat `parent`-chained comments (the phase's first `threadResolution: 'partial'` row), the opt-in issue tracker reusing Phase 54's disabled state; no boards at all — Jira is the board, and this phase does not invent one
- ✅ **G** (PR [#506](https://github.com/bilo-io/midnite-studio/pull/506)) — Azure DevOps: PRs, **work items** (not issues, `System.State`'s category), Pipelines, threaded PR discussions and the numeric `-10`/`-5`/`0`/`5`/`10` vote scale (the phase's third `'partial'` capability row, `requestChanges`), Azure Boards' column `stateMappings` read directly, `azure-diff.ts`'s synthesised unified diff for an API with no diff endpoint of its own
- ✅ **H** (PR [#502](https://github.com/bilo-io/midnite-studio/pull/502), PR [#507](https://github.com/bilo-io/midnite-studio/pull/507)) — `capabilitiesFor` is a compiler-enforced `Record<ForgeKind, ForgeCapability>`, `capabilities.test.ts` asserts the same exhaustiveness at the value level, and `app.tsx`'s `FORGE_GATED_VIEWS` reads it per field (`actions → checks`, `reviews → pulls`, `issues → issues`, `projects → projects`) instead of one boolean gating all four. #507 landed the deferred "here's the limit" sentence against the two `'partial'` rows in its own scope — a one-line note in the Projects view header for GitLab's `projects: 'partial'` and one above the PR Files tab's thread list for Bitbucket's `threadResolution: 'partial'`. Azure's `requestChanges: 'partial'` (Theme G, PR #506, landed the same day) is a third real case with no existing view-level sentinel to attach a sentence to — the Reviews view never reads `requestChanges` at all yet — logged in [`outstanding.md`](outstanding.md) as its own follow-up
- ✅ **I** (PR [#507](https://github.com/bilo-io/midnite-studio/pull/507)) — the step frame on `onboarding-modal.tsx` (a `WizardStep` model, Back/Skip/Continue with Skip only on optional steps, Escape/close now join Phase 62's `useDismiss` stack as "skip the rest"), the optional "Connect your forges" step reusing Theme B's own account-add form, and the welcome step's three hard-coded rows replaced by the real `HealthChecklist`. A skip is remembered (`ui-store.ts`'s `onboardingSkippedStepIds`) and offered again from Settings ▸ Accounts. Found, not fixed: a second first-run modal (`first-run-modal.tsx`) renders alongside this one on a genuine first run — logged in [`outstanding.md`](outstanding.md)
...
- ✅ **K** (PR [#511](https://github.com/bilo-io/midnite-studio/pull/511)) — verification: one captured, real-shaped `__fixtures__/*.json` per provider run end to end through each adapter's real read surface, plus a cross-provider comparison test; a no-token-on-wire test wiring the real account registry behind the real IPC handler for all four providers; the cache-bleed switch test extended from 2 spot-checked query keys to all 16; one e2e (`onboarding-wizard.spec.ts`) and one visual baseline (`pricing-page.spec.ts`, reaching into `packages/website`'s own dev server via a second `webServer` entry). Left open, in `outstanding.md`: the committed Linux visual baseline (needs Docker, unavailable in this session's sandbox) and the phase's own human-only live-credential passes
- ✅ **L** ([PR #516](https://github.com/bilo-io/midnite-studio/pull/516), 2026-09-23) — the account switcher: the title-bar avatar becomes a dropdown (accounts with provider icon + brand colour, hidden-repos toggle, Add/Manage), a zero-account Add affordance, `forge.switcherPlacement` (titlebar right/left, rail top/bottom, hidden), and chord-free palette commands
...
- ✅ **A** (PR [#437](https://github.com/bilo-io/midnite-studio/pull/437)) — the `KnowledgeRenderer` seam, the pill bar inside the canvas column, an overflow menu, a persisted `rendererVariant`
- ✅ **B** (PR #436) — expand from a core: a position-channel tween, centroid origin, degree stagger, and the re-indexation problem solved out loud
- ✅ **C** (PR [#463](https://github.com/bilo-io/midnite-studio/pull/463)) — focus alpha, animated: `AlphaRampTracker` ramps dimmed/rest/neighbour/focus instead of snapping, premultiplied invariant preserved, `DEFAULT_NODE_ALPHA` retuned 0.65 → 0.5
- ✅ **D** (PR [#438](https://github.com/bilo-io/midnite-studio/pull/438)) — four sigma looks: Atlas, Constellation, Orbit, Clusters (over the existing community-collapse machinery)
- ✅ **E** — layout variants in the worker (circlepack, hierarchical, noverlap), layout id in the cache key, tweened transitions ([PR #439](https://github.com/bilo-io/midnite-studio/pull/439))
...
*Every graphified repo already carries a 14,881-node map of itself that only the CLI and a hook can
see. A pinned **Knowledge** rail row beside Notes renders the active repo's own `graph.json` with
WebGL — the full graph, not the 600-node aggregate, and never the generated `graph.html`, which
CDN-loads vis-network from unpkg and would walk straight into Phase 76's renderer CSP.*
...
*Two surfaces that outgrew where Phase 58 and Phase 67 put them. The Sessions view is a **graveyard**
— `sessions-view.tsx` renders only `ClosedSession` rows and its one row action is Purge, while live
sessions live in an entirely separate store behind `terminal-session-list.tsx` — so "make it a
manager" is really *merging two lists*. The resume machinery is three-quarters built
(`AgentDefinition.resume` is populated for ten of twelve agents, `initialInput`/`queueInput`/
`startAgent({autoSend})` all exist) and the missing quarter is the only hard part: **nothing in this
repo captures the agent's own conversation id** — `sessionId` always means Midnite's. The grounding
found both agents that matter already keep it on disk keyed by cwd (`~/.claude/projects/<slug>/
<uuid>.jsonl`, 312 files for this repo; `~/.codex/sessions/…/rollout-<ISO>-<uuid>.jsonl`), which is
why exact-id resume ships for two agents and the other ten keep their existing args. Meanwhile Notes
...
- ✅ **C** — Resume, in one click ([PR #381](https://github.com/bilo-io/midnite-studio/pull/381)): a pure `buildResumeCommand`, an icon button + tooltip that opens a terminal prepopulated **and auto-sent** — an explicit documented exception to `startAgent`'s `autoSend: false` default — absent for `agy`/`cline`, honest about precision when there is no captured id.
- ✅ **D** — The pane that shows a live terminal ([PR #387](https://github.com/bilo-io/midnite-studio/pull/387), [PR #424](https://github.com/bilo-io/midnite-studio/pull/424), [PR #426](https://github.com/bilo-io/midnite-studio/pull/426)): closed → transcript, running → the real xterm, with **one xterm per pty** via `revealSession()`/a `sessionsPaneSessionId` claim rather than a second mount; a store-level send-input action.
...
- ✅ **F** — Notes on disk ([PR #380](https://github.com/bilo-io/midnite-studio/pull/380)): a main-process per-repo store under userData (never in the user's repos), a channel quartet, the `Note` shape moved to `shared`, and a localStorage→disk migration that writes-then-verifies and never deletes.
- ✅ **G** — The Notes page proper ([PR #389](https://github.com/bilo-io/midnite-studio/pull/389), [PR #391](https://github.com/bilo-io/midnite-studio/pull/391), [PR #396](https://github.com/bilo-io/midnite-studio/pull/396)): notes sidenav + content pane, lazy-loaded Monaco (a *third* Monaco surface — flagged against Phase 77), a markdown preview toggle over the existing `react-markdown` path, and Brainstorm / Execute-adhoc / status / delete carried across via `use-skill-handoff`.
- ✅ **H** — Verification ([PR #464](https://github.com/bilo-io/midnite-studio/pull/464)): merged-selector, `buildResumeCommand` and both conversation-id-adapter unit tests, notes-migration tests, a keyboard-tooltip view test, visual specs for the Sessions list and the Notes content pane (no baseline committed yet — Linux-only per `.gitignore`, needs Docker), and one e2e for the real-xterm live pane. Three human passes remain open — a real Claude resume, a real Codex resume, and a long dual-surface Notes editing session — genuinely un-automatable and left unchecked in the phase doc.
...
dot only, and on a comma locale all 679 rows fall through to a four-column fallback that hard-codes
zeros — shifting the command line too, which kills `PROTECTED_PROCESS_NAMES` outright and makes the
Optimizer's kill button reject legitimate kills. Nothing can catch it: `nonnegative()` accepts `0`,
and every fixture is four-column. **Refinement x1** found the fix already exists one package over
(`git-exec.ts:52` has pinned `LC_ALL:'C'` since day one), that **`idle-cpu.mjs:98` has the same bug**
so Theme F's own instrument reads zero, that **Electron 33 cannot supply macOS `phys_footprint`**
(so the footprint item is dropped and the RSS delta documented), and that **32 `infinite` CSS
...
- ✅ **C** — Name the retainer ([PR #364](https://github.com/bilo-io/midnite-studio/pull/364)): one shared `classify-process.mjs` (the two scripts' classifiers have diverged), CDP `HeapProfiler` snapshot diffing, an unattended `--soak`, and a verdict on the recorded `retainedPerCycleKb` breach — "measurement characteristic" only with the heap diff behind it.
...
- ✅ **E** — A monitor that reports real numbers ([PR #363](https://github.com/bilo-io/midnite-studio/pull/363)): `app.getAppMetrics()` for our own tree via a pure `mergeAppMetrics`, `ps` for foreign, an Owner column joining pids to tabs/sessions/windows, and the RSS-vs-Activity-Monitor delta **documented** rather than chased with a subprocess.
- ◻ **F** — The window nobody touched: Phase 36 Theme G's episodic renderer ~32% + GPU ~55%, measured on a fixed instrument, attributed against 32 ranked ungated loops (the Optimizer hero's three drifts and its `background-clip: text` shimmer first), fixed with the `:root:has(…)` pause idiom already in `styles.css:809`, and held by a `findUngatedLoops()` guard beside `findUnguardedKeyframes`.
- ◻ **G** — Numbers, before and after: every budget with the run that justifies it, the README's "what is not measured here" updated, and `outstanding.md`'s two stale entries resolved.
...
- 🔄 **E** — Terminal unload + rehydrate (visible + 3 recent stay mounted, dispose after 2 min, replay
  from main's scrollback with no gap, per-session entrance fade) — E.1-E.5 landed; E.6 renderer number outstanding
- 🔄 **F** — Browser tab discard (Chrome-style after 10 min hidden, sleeping glyph, Keep awake,
  audible/active excluded, per-app opt-in in Settings ▸ Apps) — F.1-F.4 landed; F.5 renderer number outstanding
- 🔄 **G** — Bounded keep-alive for heavy views (`keepAlive` on `view-registry`, TTL + row ceiling,
  deferred re-stream while hidden) — G.1-G.4 landed, G.5's number outstanding
...
- 🔄 **I** — Liveness dot (per-window `liveness-store`, `syncStatus` push, status-bar zone in every
  window)
- ✅ **J** ([PR #352](https://github.com/bilo-io/midnite-studio/pull/352)) — Numbers, not adjectives
  (baseline first, before/after per theme, `budgets.json` lines, zero subprocesses when blurred)
- 🔄 **K** — Cascading reveal, everywhere (`useCascadeReveal`, mount/reveal/repo-switch only, file
  tree + terminal + companion + Loops + popout first paint + Changes/Projects/Sessions cascade, app-owned reduced-motion rule, guard
  scans Tailwind keyframes) — K.1-K.6 and K.8 landed; K.7's perf number outstanding
...
*The companion reads SHAs, paths and URLs aloud, its digest summaries are one fixed sentence every
time, `speechSynthesis` is the only engine it has ever had, and it answers to exactly one
hardcoded name nobody can change. Plans a spoken-form redaction pass, category-and-count phrasing
with randomised templates, a local free/low-RAM voice (`sherpa-onnx-node` + Piper), and an
...
*Every agent surface in this app is a terminal you type into, and the status-bar assistant popover
is still a placeholder string. This phase adds a companion that talks: grounded by the MCP tools
called in-process, routed to the agent CLI the app already launches, timed off the pty activity
signal, and voiced by the OS. No new inference path, no MCP client, no audio assets, default off.*

- ✅ **A** — Companion state machine + phrase banks (greetings, sign-offs, fillers, unattributed quotes) as pure data in `shared`; a no-repeat picker; settings in `ui-store`. ([PR #269](https://github.com/bilo-io/midnite-studio/pull/269))
...
### [Phase 78 — Whose hands were on the keyboard](phases/phase-78-whose-hands-were-on-the-keyboard.md)

*The app owns both halves of an agent's work — the session it ran in and the commits it produced —
and cannot connect them. Read-side first: trailers agents already write, a pure provenance
vocabulary that says* probably *when it is guessing, a mark on the row, a crosswalk from a closed
session to its commits and from a commit to its transcript. The write-side fingerprint is last and
opt-in.*

- ✅ **A** — Read the trailers: `Co-Authored-By` and `Midnite-Session` into `LOG_FORMAT` (NUL-delimited, `%x1f` inside) and `CommitSchema`; measured on the 50k fixture. ([PR #456](https://github.com/bilo-io/midnite-studio/pull/456))
- ✅ **B** — `classifyProvenance` in `shared`, pure, with an ordered confidence and per-agent `signatures` on the roster; full branch coverage. ([PR #456](https://github.com/bilo-io/midnite-studio/pull/456))
- ✅ **C** — The mark on the graph row and in commit detail; an All/Humans/Agents filter that dims, never hides; *probably* in the tooltip for the window join. ([PR #460](https://github.com/bilo-io/midnite-studio/pull/460))
- ✅ **D** — Sessions ↔ commits: "N commits" per closed session → the graph narrowed to them → a commit's Provenance line → the archived transcript. ([PR #459](https://github.com/bilo-io/midnite-studio/pull/459))
- ✅ **E** — Opt-in write side: `MSTUDIO_SESSION_ID`/`MSTUDIO_AGENT_ID` in agent ptys; a never-clobbering `prepare-commit-msg` hook behind a default-off per-repo switch. ([PR #465](https://github.com/bilo-io/midnite-studio/pull/465))
- ◻ **F** — An agent-share tile on the dashboard, computed over loaded rows with B.

### [Phase 77 — Thirteen megabytes of editor, and the lanes that never move](phases/phase-77-thirteen-megabytes-of-editor.md)

*`budgets.json` already knows total JS is 2.2× over budget. **Refinement x1 re-grounded all four
findings and corrected three**: the `emacs-lisp`/`cpp`/`wasm` tail is **Shiki's, not Monaco's**
(Monaco ships no emacs-lisp and its whole cpp tokenizer is 8 KB); the app calls **no**
`editor.create`/`setModelLanguage` anywhere, so the lazy registry gates the `<Editor>` mount instead;
and `useResizable` already commits on pointer-up and says so in its own docblock, so Theme C
retargets at four `<input type="range">` handlers and the dashboard's `onLayoutChange`. Every theme
opens with a measurement and closes with a before/after table in the PR body.*

- ◻ **A** — Monaco from `editor.api` with the feature list the code actually proves (find/suggest/parameter-hints context keys, `formatDocument`, `addCommand`), a lazy contribution registry behind a `useMonacoLanguage(id)` mount gate, an on-demand `ts.worker`, and a Shiki audit before any Shiki fix.
- ◻ **B** — Pass-through lanes leave `edges` for a dense `through: number[]` with `.default([])`; **no run-builder** (each row is its own `<svg>`, so a multi-row line has no host) — the wins are layout time, IPC bytes and heap, proved by a painted-lane multiset assertion, not by screenshots `MSTUDIO_SHOTS` never compared.
- ◻ **C** — A `debouncedStorage` that *replaces* a `PersistStorage` (it cannot wrap `sharedSettingsStorage`), applied to `midnite-studio.ui` **and** `midnite-studio.dashboard`, flushed on unload, with no version bump.
- ◻ **D** — Rebaseline only on a documented miss; a non-blocking `perf-report` job writing the bundle table to **`$GITHUB_STEP_SUMMARY`** — not a PR comment, which would cost this repo its first job-level `permissions:` block.

### [Phase 76 — The renderer in a sandbox, and the headers it never sent](phases/phase-76-the-renderer-in-a-sandbox.md)

*Five findings around boundaries that are otherwise well drawn: an end-of-life Electron, a
`sandbox: false` whose stated reason `bundle.mjs` made false, no CSP and no `will-navigate` guard on
the app's own window, one plaintext credential and one silent IP lookup, and forty-five one-way
channels still hand-rolling validation. Theme A is human-run; nothing touches Renovate's PRs.*

- ◻ **A** — Electron off the end-of-life line (oldest supported major), `node-pty` rebuild, full perf suite before/after, broker legacy-peer handshake, a day's soak — **human-run**.
- ✅ **B** — `sandbox: true` on both windows: `homedir`/`hostname` via `additionalArguments`, `node:os` out of the preload, a bundle test that keeps `node:` out. (PR #484)
- ✅ **C** — A header-based CSP from a pure `buildCsp()` (dev/packaged, `persist:browser` skipped) and a `will-navigate`/`will-redirect` guard on the app window; zero violations across `app:e2e`. ([PR #487](https://github.com/bilo-io/midnite-studio/pull/487))
- ✅ **D** — The Twelve Data key into a `safeStorage` vault behind an enum-keyed `secrets` channel, finance fetches proxied through main, `ipwho.is` behind a default-off Privacy switch ([PR #485](https://github.com/bilo-io/midnite-studio/pull/485)).
- ✅ **E** — 45 `ipcMain.on` sites onto `handleSend`, two raw `ipcMain.handle` onto `handle`, an eslint rule confining both to `ipc/handle.ts` ([PR #486](https://github.com/bilo-io/midnite-studio/pull/486)).

### [Phase 75 — What blocks what, and the agent you point at it](phases/phase-75-what-blocks-what.md)

*A board is a set; a backlog is a graph. [Phase 40](phases/phase-40-github-projects.md) brought a
ProjectV2 board in, [Phase 41](phases/phase-41-agentic-kanban.md) let an agent run a card, and
neither can say what has to finish first. This adds a third `projectsMode` beside table and board: a
left-to-right dependency DAG with blockers upstream, blocked nodes greyed, and edges animated by the
state of their two ends. **Refined x1** re-grounded every claim against the repo and found five
premises wrong: the kanban card has worn the rainbow ramp since Phase 37 (Theme F generalises, it
does not build); `workflow-canvas.tsx` paints nodes in raw SVG, which `.card-run-glow` cannot survive
(nodes are DOM over an SVG edge layer); there is **no reusable card start path** — `launch()` is a
component-local closure and `startAgent` the only export, so the node opens the composer rather than
starting anything; `CardPanelStack` has no exported open action, so selection lifts out of
`BoardView` into `ProjectsView`; and this repo has no `--status-*` vars at all, so Theme E defines
its own `--dep-*` trio. It also replaced 300 per-node `useCardStatus` subscriptions with one
...
- ✅ **B** — The `api` → `field` → `body` ladder plus a separate containment layer, as a pure resolver in shared: `ready` = blockers closed or merged, foreign nodes, `parseBlockerRefs`, `describeGraphSources`, the 300-node cap. (The shared `ForgeProjectItem` fixture factory is left to Theme C/D, whose own `packages/app` test suites are what actually need it.) ([PR #206](https://github.com/bilo-io/midnite-studio/pull/206))
- ✅ **C** (PR #207) — Longest-path LR ranking with barycentre ordering, a deterministic cycle-break rule and `topAlignedViewport` — `dagre`'s job in ~40 lines, pure and unit-testable. Components pack independently so disconnected parts of the board never share a bounding box.
- ✅ **D** ([PR #208](https://github.com/bilo-io/midnite-studio/pull/208)) — The canvas: `ProjectGraphView({ graph, items, fields, projectId, selectedItemId, onSelectItem, agentStates })` and `ProjectGraphNode({ node, item, fields, glow, selected, tabIndex, detailed, onSelect })`. DOM nodes over an SVG edge layer reusing `workflow-path.ts`'s `edgePath`/`panBy`/`zoomAtPointer`/`clientToGraph`/`rectsIntersect`/`viewportRect` verbatim (`outPort`/`inPort` are re-declared locally — the workflow canvas's own bake in its own 160×56 node size, not this graph's 200×64); a non-passive wheel listener; graph-space culling as the only culling; a component-local viewport re-fit once on mount, `Home` re-fits; edge-following arrow keys (`graph-keyboard.ts`); level-of-detail below `scale: 0.5`; a collapsible per-project legend; and four empty states (two inherited for free from the mode-agnostic early returns already above the mode switch, two new: zero-edges, all-drafts-or-PRs). `card-chrome.tsx` extracts `CardTitleRow`/`CardNumberRow`/`CardAssignees`/`CardFieldChips`/`CONTENT_ICON` out of `task-card.tsx` so the node wears the same chrome a kanban card does. Also closed all four of Theme F's items deferred here (the node wired onto `deriveCardGlowState`, `waiting`/`open`/`idle` proven on the real node, the graph-only bloom `::after`, and `project-graph-glow-shots.spec.ts`) — closing them surfaced a real bug (`useGraphAgentStates` read an always-empty terminal store because nothing reachable from graph mode ever called `hydrate()`; fixed inside the hook itself). `selectedItemId`/`onSelectItem` are wired all the way through but `CardPanelStack` is not mounted yet — that stays Theme G's own checklist item.
- ✅ **E** ([PR #210](https://github.com/bilo-io/midnite-studio/pull/210)) — `--dep-done`/`--dep-active`/`--dep-idle` in both theme blocks (borrowing `--success`/`--muted-foreground` where one fits; `--dep-active` is its own literal, nothing else means "work is happening now"); `edgeAppearance(edge, source, target, sourceGlow?)` — the five blocking-edge states, `contains` always quiet, `body`-sourced dotted at reduced opacity on top of whichever state the table chose. `sourceGlow` is a deliberate fourth, defaulted parameter beyond the doc's literal two-node signature — "agent running" lives in the glow map, not on `ForgeGraphNode`. A blocked node dims its inner content only, never the `agent-run-glow` ring (Theme F's ring wins, stated as an explicit precedence); a `ready` node gets an affirmative `LuCircleCheck` badge. The dash is a CSS keyframe, reduced-motion- and focus-gated the same way `.agent-run-glow` already is.
- ✅ **F** (PR #205, node/bloom/shots closed by Theme D) — `.card-run-glow` generalised to `.agent-run-glow` (also caught `workflows-view.tsx`'s run-history button, which wore it too), and one `useGraphAgentStates` subscription for the whole canvas instead of 300. `kanban.spec.ts`'s literal class assertions moved in the same commit.
- ✅ **G** ([PR #215](https://github.com/bilo-io/midnite-studio/pull/215)) — Selection lifted into `ProjectsView` so both modes share one panel — a card opened in board mode stays open across a switch to graph mode; the graph mounts `CardPanelStack` on the same terms `board-view.tsx` does; the node opens the composer rather than starting an agent. Start (and Launch and run) disable on a blocked item via a new pure `apiFieldBlockersFor(graph, itemId)` helper, but only for `api`/`field`-sourced blockers — a `body`-inferred one never gates the button, and the disabled title names the blockers (`Blocked by #199, #204`).
- ✅ **H** (PR #216) — The graph now reads the whole board, not `filteredItems` — a `filterForgeGraph` step (applied after `resolveForgeGraph`, before `layoutForgeGraph`) drops a node whose own item the shared toolbar filter hid, along with its edges, instead of the item resolving as an indistinguishable foreign node. Four graph-only facets (`showContains`/`only`/`depth`/`hideIsolated`) live as one optional, shallow-merge-safe object on `ProjectViewState.graph`, rendered as `ItemFilterToolbar` children gated to graph mode — zero edit to the shared toolbar itself. `depth` walks `blocks` edges only, BFS from the selected node's own key. A non-default facet now also trips the shared filter-active indicator (a small "Filtered" chip). `blockedByFieldName` is a new global preference (`projects-page.tsx`'s fourth accordion), defaulting to `'Blocked by'` and disabling the field layer entirely when cleared.
...
*The Sessions rail row has rendered a placeholder since Phase 23 — the only one of nineteen `ViewId`s whose `VIEW_COMPONENT` entry is a stand-in — and the app already wrote down what it should be (`LuHistory`, 'Agent Sessions', 'agent session history transcripts'). There is no history to show, because closing a session deletes its row and its transcript. Record the ending, then render it.*

- ✅ **A** (PR #199) — A session that ends leaves a record: a `ClosedSession` schema (a fresh `z.object`, never an `.extend()` of a `ZodEffects`), a `session-history-store.ts` in `trust-store.ts`'s shape, a `forgetTerminal` that **flushes the in-memory ring before renaming** the transcript — without which the last 15 s is lost — a 200-cap that evicts the file alongside the row, `reason`/`exitCode` derived from `pty-service.ts`'s unused `onSessionExit` seam, and a real `purge` channel since 'forget' no longer forgets.
- ✅ **B** (PR #199) — The history channel: three free `mstudio:sessions:*` channels, `Uint8Array` transcripts structured-cloned never base64 (`PtySnapshotResponse`'s shape), and the `register`/`configure` split handlers need because they register at `index.ts:298` and `userData` only resolves at `:371`.
...
*Refined x1 and **split**: this is now the shippable core — open a real Postman collection, build a request, send it, read the answer — and the rest is [Phase 70](phases/phase-70-api-client-environments-tests-and-runs.md). The refinement's largest correction is that the whole editor premise was dead: [Phase 64](phases/phase-64-offline-monaco-and-themes.md) replaced CodeMirror with Monaco, the seven `@codemirror/*` deps have zero importers left, and Monaco 0.56 already ships the GraphQL grammar the old Decision 3 was open about.*

- ✅ **A** (PR #222) — Shared contracts: the recursive `.passthrough()` Postman v2.1 tree (both `z.lazy` and `.passthrough()` are firsts in `packages/shared`), an editable `ApiRequestDraft` whose `bodies: Record<BodyMode, string>` makes "switch modes without losing a body" structural, `ApiOpResultOf` copying `database.ts`'s two-arm envelope (success field `value`, per the doc's own literal spelling), and seven `mstudio:api-client:*` channels with **kebab verbs** per `channels.ts:6`.
- ✅ **B** (PR #225) — Nav and registration, which turned out to be **fifteen edits across eleven files, not the four the doc named**: the `ViewId` union **and** the separate `VIEW_IDS` array, `VIEW_COMPONENT`, `VIEW_ICON`, the **sixth** `WORKSPACE_NAV_ITEMS` entry, a chord-free `view.apiClient` beside `view.video` — plus nine sites `moon run :typecheck` caught that the doc's own file table never listed: three more `Record<ViewId, string>` label maps (`sidebar-page.tsx`, `title-bar-nav.tsx`, `providers.ts`), `providers.ts`'s `VIEW_KEYWORDS`, `view-sections.ts`'s `VIEW_FILTERS`, `view-sections.test.ts`'s hand-written exhaustiveness check and its `toHaveLength` count, and — once `view.apiClient` existed as a `CommandId` — `command-icons.ts`'s `COMMAND_ICONS` and `use-command-handlers.ts`'s `CommandRuntime`. Two flagged deviations: `ApiClientView` carries no `<PageDetachMark>` (that needs `apiClient` in `PAGE_WINDOW_ROLES`, real out-of-table scope), and `apiTreeWidth` needed no `persisted-keys.ts` edit at all — that exhaustiveness check is over `PersistedUi`'s top-level keys, and `layout` (which it's a field of) is already one, classified `SESSION_STATE_KEYS`.
- ✅ **C** (PR #233) — Collection tree, request tabs, and its **own** store: `WorkbenchTab` is a closed four-arm union of read-only surfaces with no draft and no dirty concept, so there is nothing to extend; dirty is derived from `draft !== savedDraft` the way `file-preview.tsx` does it, and nothing is persisted because restoring an unsaved edit into a file that moved is a data-loss shape.
- ✅ **D** (PR #234) — Request builder across all eight body modes on **Monaco**, via a new controlled `monaco-field.tsx` (`CodeEditor` is bound to `useFileEditorStore` and a file name, so it cannot be reused); the URL↔params sync rule stated in both directions; auth in plain text with copy that admits it, because masking without the overlay implies a protection that is not there.
- ✅ **E** (PR #227) — The send engine in main: `http.ts`'s cap/timeout/`readCapped` pattern imported rather than copied, interpolation resolved in main so a value never lives in renderer state, an unresolved `{{var}}` left **literal** with a warning rather than substituted to nothing, and every handler try/caught because `handleOp` converts a schema failure to an envelope but not a throw.
- ✅ **F** (PR #233) — Response viewer: content-type-driven rendering, a persistent truncated banner (not a toast — it describes the body it sits above), ten in-memory responses per tab, and a `{ok:false}` that renders a message and a Retry rather than reaching the error boundary.
...
- ✅ **E** ([PR #178](https://github.com/bilo-io/midnite-studio/pull/178)) — A user can get at it in two clicks: Reveal log (with the path printed beside it — a support thread asks *where* it is), Copy diagnostics and Report a bug inside the Diagnostics accordion that already exists (not an 18th settings page), plus the bug link in the release-notes panel. Both copies on purpose: `version-pill.tsx` hides on `'0.0.0'`, so the panel never opens in a dev build.
...
- ◻ **G** — Decommissioning CodeMirror (**new in x1**): retarget the six `.cm-content`/`.cm-gutters` assertions in `files-editor.spec.ts` that would go red on Theme C's first commit, drop the seven `@codemirror/*` deps. **Gate on Phase 61 cleared (2026-09-06)** — its Theme G shipped a Monaco `query-editor.tsx` instead of the CodeMirror one originally planned, so nothing in the tree depends on `@codemirror/*` any more; this theme is unblocked.
...
- ✅ **C** (PR #211, 2026-09-06) — SQLite (**M, was S**): six packaging edits — `dependencies` not dev, esbuild `external`, `rebuild-native --only` list, `asarUnpack`, an unpacked-path `require` fallback, and the `verify-dist` native assertion that did not exist. Decision 6's recommended `node:sqlite` was checked against the tree rather than assumed: Electron 33.4.11 bundles Node 20.18.3, which does not have it — so `better-sqlite3` shipped, verified by an actual `rebuild-native.mjs` run that flipped the binary's ABI and back.
...
- ✅ **G** (PR #211, 2026-09-06) — Query tabs (**L, was M**): a `'query'` arm carrying no `repoId` — which breaks `closeRepoTabs` and Phase 28's pruning, both fixed — plus the `+` button `TabStrip` never had, and a `●` in its `stats` slot. Decision 7's unscoped store, as recommended. `query-editor.tsx` is Monaco, not CodeMirror — Phase 64's Monaco migration had already landed by the time this theme executed, so Decision 9 resolved to the other branch: no new dependency, and Phase 64 Theme G can now drop `@codemirror/*` with nothing left depending on it.
- ✅ **H** (PR #211, 2026-09-06) — Results grid: rows stream in by subscription (`query-results-store.ts`, one entry per tab) with stale-batch discard, `truncated` rendered visibly, a parameterised PK-keyed `UPDATE` (this repo's first parameterised statement anywhere, threaded through all five drivers) with a staleness re-read — **not** wrapped in one transaction, a disclosed narrowing of Decision 2 — editing refused on joins and PK-less tables, and CSV via the `Blob` + `<a download>` precedent.
...
- ✅ **J** (PR #217, 2026-09-06) — Suites + CI: four Playwright specs (the full query loop, the destructive gate, edit + a manufactured staleness conflict, two independent tabs), three more shots (query+results, confirm, conflict) through Phase 56 Theme G's shared helper, and Postgres/MySQL/MariaDB as CI service containers with MSSQL manual (Decision 5). Also fixed a real bug it found: the query-editor/results-grid panes collapsing to ~5px tall for want of a `flex` class on their wrapper.
...
- ✅ **A** — The notes store (S): `midnite-studio.notes` on zustand `persist` in `dashboard-store`'s exact shape, `Record<noteId, Note>` with a pure `notesForRepo` selector, `status` and `done` as two independent axes — and **notes are not GC'd on repo close**, reversing the first draft to match the precedent it claimed to copy. (PR #160)
- ✅ **B** — The modal primitive (M): `Modal` with enumerated `size`/`variant`/`align`, occluder registration, focus *restoration* (which none of the twelve get right), and a `.gradient-frame` variant — proved by migrating `prompt-dialog` and `browser-launcher`, while the other ten get the two-line occluder fix immediately and a regression test that counts all twelve. (PR #160)
- ✅ **C** — The Notes surface (M): a 900px/80vh panel, in-place editing on Midnite's exact key contract (Enter commits · Shift+Enter newlines · Escape cancels *without closing the modal* · empty cancels), hide-completed, per-value status badges, two distinct empty states. (PR #160)
- ✅ **D** — Status and handoff (M): **an extraction, not a second implementation** — `use-skill-handoff.ts` lifted out of `midnite-menu.tsx`, reading the user-configurable `agentSkills` rather than hard-coding two literals, and passing the prompt as a plain string because `startAgent` shell-quotes it. (PR #160)
- ✅ **E** — The quick-access menu (M): one component, two entry points, rows reusing `context-menu.tsx`'s existing `MenuItem` shape plus a mnemonic — `L` Loops · `N` Notes · `I` Report Issue · `G` Guided tour — with the bare-letter gate mirroring the palette's in `use-keybindings.ts`. (PR #183)
- ✅ **F** — Commands, keybindings and the doc sync (S): `fab.toggle` re-pointed and re-labelled, a chord-free `notes.toggle`, both open flags kept **out** of `PersistedUi` so there is no version bump — and `menu.ts:120`'s native `CmdOrCtrl+L` accelerator, already fixed by Phase 64 before this branch existed, confirmed still in place. (PR #183)
- ✅ **G** — Verification (M): both entry points, the mnemonic paths, the full note lifecycle, a pty assertion that the handoff is typed and not sent, the occluder contract against a live browser pane, and an explicit test that closing a repo does *not* delete its notes. (PR #183)
...
- ✅ **E** — Consent, scope and audit (M, was S): the enable flag as main-side state in `mcp-store.ts` (`repo-store.ts`'s shape) plus `mcp.get`/`mcp.set` channels; `repoPath` gated by `resolveRepoRoot` + `listRepos` with `realpath` on both sides, not `fs-scope.ts`; a bounded 50-entry audit ring with no payload bodies. ([PR #177](https://github.com/bilo-io/midnite-studio/pull/177))
- ✅ **F** — The Settings page and status readout (S): the switch reading main, not `useUiStore`; **three** registration points asserted (`SettingsPageId`, `SETTINGS_PAGES`, `settings-view.tsx`'s `PAGES`); the copyable `claude mcp add` line; the tool list from the registry; the last 50 calls, pulled not pushed. ([PR #177](https://github.com/bilo-io/midnite-studio/pull/177))
...
- ✅ **B** (PR #179, 2026-09-05) — Lockstep as a check: a seeded root `CHANGELOG.md`, a `version-check.mjs` grouping by `MAJOR.MINOR` (root `moon.yml` has exactly one task today), the CLI wrapper's hardcoded `0.1.0` derived rather than checked, the six missing skill helpers ported into `version.ts`, and `extractChangelogSection`'s `.date` precondition reconciled with a helper that returns `string | null`.
- ✅ **C** (PR #179, 2026-09-05) — `verify-dist` learns the feed: `latest-mac.yml`, `.blockmap` and `CFBundleShortVersionString`, on top of the **ten** gates it has today (PR #155 added two, not one) — and the version skew it reads but never compares.
- ✅ **D** (PR #179, 2026-09-05) — Tag-triggered `release.yml`: cross-repo publish under the namespaced tag with a `RELEASES_REPO_TOKEN` that **does not exist yet**, `contents: write` (CI's `package` job holds `read`), and four guards cribbed from the sibling — each of which cost it a broken release, one leaving an untagged draft.
- ✅ **E** (PR #188, 2026-09-05) — The feed stops being manual: a `publish-feed` job (`needs: release`) committing `latest-mac.yml` into a directory that holds only a README today, **plus the changelog mirror** via new `scripts/publish-feed-changelog.mjs`, ordered after the assets exist — and the ⚠️ banner de-staled across **six** skill files, not two. `RELEASES_REPO_TOKEN` still doesn't exist, so it can't run end to end yet — same posture as Theme D.
- ◐ **F** (PR #199, 2026-09-06) — The rehearsal, not the release. **The flow as written could not have cut v0.1.0**: with 0 tags, `/midnite-release-prep` falls back to the root commit, meets a `feat` among 799 of them, and plans **v0.2.0** against a repo that is entirely `0.1.0`; `/midnite-release-complete` reads `previousVersions` from a tag tree that does not exist, and the pairwise `planReleaseTags(current, current)` it would otherwise reach returns `[]` — a release that publishes nothing, quietly. Fixed with `planRelease` (one entry point, `previous === null` bumps nothing and tags what is in the tree) and the `previous = null` branch written into all six skill copies; the pre-release baseline is recorded command-by-command in `docs/RELEASING.md`, including the `"version": null` → empty-`sed` → *"no published release yet"* proof. Also found and fixed en route: **`moon.yml` excluded `test` from the `root` project**, so `moon run :test` skipped `scripts/*.test.mjs` entirely and 36 tests — two files of them guarding release machinery — had never run in CI. The four remaining items are barred from a session, not merely unfinished: a secret to create, an irreversible tag and Release, a README in a different repo, and an install on a machine with no checkout. `docs/RELEASING.md` carries them as a numbered run-book for the human pass.
- ◐ **G** (PR #188, 2026-09-05) — The updater's blind pill fixed: an error affordance and a checking state (both rendered `null` before), the channel now pushed to main once on boot so a beta user survives a relaunch, and the `stable → 'latest'` mapping's *reason* written down as a docblock. One item remains: confirming the pill actually reaches `available` needs Theme F's live feed to observe against.
- ◐ **H** (PR #188, 2026-09-05) — **Six** secrets (five signing + `RELEASES_REPO_TOKEN`) documented in a new `docs/RELEASING.md`, and the silent notarization skip made visible in `verify-dist` (`spctl --assess` required only for a signed build). Flipping `notarize: true` (needs a purchased Developer ID) and stating ad-hoc signing's permanence in `bilo-io/midnite-apps`' own README both remain, genuinely blocked — a credential not held, and a different repo's edit left for the human pass alongside Theme F.
...
- ✅ **E** — The small ones, each with the assertion that catches it: `runLocks` never pruned (the `evictIfCurrent` idiom already exists in `write-queue.ts`), two `.then`-without-`.catch` sites that retain a handle *and* raise an unhandled rejection, `dropKey` missing one of thirteen per-session records (fixed **structurally** — its `Pick<>` was a hand-written list a fourteenth would slip past just as quietly), a closed browser tab's un-detached `webContents` listeners, and an unbounded `workflowCache` now sharing the run cache's LRU. `sessionExitHooks` verified per doc criteria: no second caller appeared to justify an `off`. (PR #51)
- ✅ **F** — Verification, run for real: `retention.spec.ts` extended to `repo`/`browser-tabs` (`terminal` already there) and actually executed — finding and fixing a harness export bug and a false-positive `browser-tabs` slope (10-cycle Chromium subprocess-pool warm-up, confirmed flat at 20). Every Theme C–E fix's assertion verified to fail against the unfixed code. Full automated suites and long-running session human verification pass completed. (PR #51)
...
- ✅ **A** ([PR #92](https://github.com/bilo-io/midnite-studio/pull/92)) — Shared contracts: `Workflow*` zod schemas, nodes as a discriminated union of exactly five kinds, channels + one bare `workflowRunChanged` event. Copies `GitOpResult`'s nested-union shape exactly — a flat `discriminatedUnion('ok')` is a zod error — and adds the opt-in `describe('workflow contract')` block to `ipc.test.ts` without which a channel ships unvalidated.
...
- ✅ **I** (PR #108, PR #187) — Wiring + verification: a chord-less `workflow.run` command, which turned out to need `PALETTE_SAFE`'s allowlist too (declaring it in `COMMANDS` alone left it undiscoverable — caught by an e2e run against the real palette) and a new `workflow-run-command-store.ts` seam (the `commit-box-store.ts` shape) since "which workflow is open" is local view state. The settings page (default timeout, run-history cap) reaches main for real over a new one-way `workflowSetDefaults` channel — `update.setChannel`'s shape, `EngineDeps`/`trimRunsPerWorkflow` gained injected overrides. Full gate, boundary/import checks, every unit/RTL test passing, lazy bundle budget confirmed, and human-verified real end-to-end pass confirmed.
...
- ✅ **G** — Verification: the assertion that proves the parser rule (an unrecognised field type must not drop the item), the assertion that catches the `-f`-vs-JSON mistake, and live board validation. Four Vitest suites landed alongside E/B; `e2e/projects.spec.ts` covers the gated-fetch, not-optimistic edit, refused-write and missing-scope paths against the mock bridge (PR #45). All automated and manual verification passes complete.
...
- ✅ **H** (PR #232) — Retire the ratchet: full suite green twice, then delete `playwright.ci.config.ts`, the `app:e2e-ci` task, and point CI back at `app:e2e`. Unblocked by [PR #228](https://github.com/bilo-io/midnite-studio/pull/228): `KNOWN_RED` is now empty.
...
- ✅ **F** — Reduced motion, and proof: `animation-name: none !important` (not a pause), computed-custom-property assertions, per-tab shots. Window focus/blur gate on rotation and pulse shipped unconditionally; layout bounds and theme parity verified. (PR #8)
...
- ✅ **F** — Memory caps: 10k true-LRU + per-key notify in `line-highlight.ts`,
  ...
  Landed 2026-09-01; memory bounds asserted by unit tests.
- ✅ **G** — Profile-gated claims, taken to numbers. **Broker: INDICTED** — 96.8% of a core for
  ...
  Baseline table completed and profiling procedures established. (2026-09-01, local)
...
- ◻ **A** — Polished DMG Package & macOS Desktop Integration. Not started: `dmg:` window layout + @1x/@2x PNG artwork, hardened-runtime entitlements, `protocols:` registration, an env-gated `afterSign` notarize hook, and a `verify-dist` gate asserting `codesign --verify` / `hdiutil verify`.
...
- ✅ **C** — `midnite-studio://` Custom Protocol Handler & Deep-Link Dispatch. The single-instance lock, `open-url`, argv forwarding and a pure `parseDeepLink` already existed; this closes out the jail's missing half — the renderer now renders the `known:false` proposal as a `ConfirmDialog` naming the absolute path (confirming opens and selects it), and a `clone` link as a notice, since no clone flow exists yet. ([PR #262](https://github.com/bilo-io/midnite-studio/pull/262), 2026-09-07)
- ◐ **D** — Auto-Updater Service & Update Status Banner. Partial: the pure `update-state.ts`/`feed-channel.ts` modules and the `publish:` block (generic provider) exist; `electron-updater`, `update-service.ts`, the preload `update` group and the settings/status-bar UI are not yet built.
...
- ✅ **F** (PR #248) — the surfaces that arrived later (S): six of the nine `react-markdown` render sites in
...
- ✅ **G** (PR #248) — verification: run what is already written (S): every artifact the original Verification
...
- ✅ **H** (PR #249) — drop the dead CodeMirror dependencies (S): Phase 64 C swapped the editor for Monaco and
...
- ✅ **I** (PR #249) — the verification gaps that are real (S): `grep-parser.test.ts` has 2 cases against a
...
*The keymap module had named "(later) a command palette" as dispatch source number three since
Phase 9, and the registry could not feed one: it lives in `shared/src/keybindings.ts` (not the
`commands.ts` path two docs linked to, which has never existed), and at the time `COMMAND_IDS` had
fifteen entries against thirteen bindings with only nine ids carrying a handler. A fixed the
registry, B lifted the handler map out of `app.tsx` into the dispatcher all three feeds share, C–D
