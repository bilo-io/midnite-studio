...
- **[Phase 56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md)** (0% · 0/29) — **Planned, not started.** The renderer's browser-driven Playwright suite runs ~713 tests across 66 functional specs against a Vite dev server with a mocked bridge. In CI, it takes 6m39s–7m30s per shard across 4 shards (~7–10 min total wait). While [Phase 38](phases/phase-38-e2e-suite-repair.md) repairs the failing specs and empties `KNOWN_RED`, this phase targets execution speed: scaling to 8 shards, enabling `fullyParallel: true`, running a workers:2 trial on Linux runners, trimming failure retries from 2 to 1, caching Vite dev artifacts in CI, gating raw `page.screenshot()` calls in functional specs, and extracting a shared `shots-helper.ts` for screenshot specs to cut CI wall-clock runtime down to 2–4 minutes.
...
| [56 · E2E Suite Speed Run](phases/phase-56-e2e-speed-run.md) | 🔄 WIP | — | 0/29 | `░░░░░░░░░░` | 0% | A B D E | C F G |
...
- ◻ **A** — Shard scale-up: 4 → 8 shards on `ubuntu-24.04`, with `timeout-minutes` recalibrated to 10.
- ◻ **B** — Inter-test parallelism: `fullyParallel: true` in `playwright.config.ts`.
- ◻ **C** — Worker concurrency trial: evaluate `workers: 2` vs `workers: 1` on CI and land the measured winner.
- ◻ **D** — Retry trim: 2 → 1 in CI to avoid 3-minute burns per failure now that `KNOWN_RED` is down to 1 file.
- ◻ **E** — Vite dev server build cache in CI: cache `packages/app/node_modules/.vite` keyed by source and config hash.
