...
|-------|--------|---------|------|----------|---|--------|--------|
| [75 · What blocks what, and the agent you point at it](phases/phase-75-what-blocks-what.md) | ◻ TODO | — | 0/72 | `░░░░░░░░░░` | 0% | — | A B C D E F G H |
| [74 · Media caches and the Trash](phases/phase-74-media-caches-and-the-trash.md) | 🔄 WIP | x1 | 67/70 | `██████████` | 96% | E | — |
| [73 · The optimizer leaves the repo](phases/phase-73-the-optimizer-leaves-the-repo.md) | 🔄 WIP | x1 | 63/68 | `█████████░` | 93% | — | — |
| [72 · Every build system's leftovers](phases/phase-72-every-build-systems-leftovers.md) | 🔄 WIP | x2 | 98/102 | `██████████` | 96% | F | — |
| [71 · Links that open in place, and the dev server they point at](phases/phase-71-links-that-open-in-place.md) | 🔄 WIP | x1 | 0/41 | `░░░░░░░░░░` | 0% | A C | B D |
...
- **[Phase 75 · What blocks what, and the agent you point at it](phases/phase-75-what-blocks-what.md)** (0% · 0/72) — **Planned, not started.** A board is a set; a backlog is a graph. [Phase 40](phases/phase-40-github-projects.md) brought a ProjectV2 board into the app and [Phase 41](phases/phase-41-agentic-kanban.md) let an agent run a card, but nothing in either can say that #212 cannot start until #199 lands, or that #204 has been startable for a week. This adds a third `projectsMode` beside table and board: a left-to-right dependency DAG with blockers upstream, blocked nodes greyed, each edge animated according to the state of its two ends, and an agent pointable at a node exactly as it already is at a card. `~/Dev/midnite`'s task graph is the design brief — the same `ready`/`unmetBlockerCount`/`foreign` vocabulary and the same five-state edge appearance — with everything underneath replaced, because that app owns its database and this one reads somebody else's board over `gh api graphql`. **The grounding audit changed two themes before they were written.** First, **the kanban card has worn the rainbow since Phase 37**: `styles.css:2094` defines `.card-run-glow` as a `--rainbow-ramp` conic border spun by `loop-glow-spin`, applied by `task-card.tsx:105` off `glow-state.ts`, with `kanban-glow-shots.spec.ts` already holding it — so the requested treatment is *already true on the board*, and Theme F generalises the class to a second surface rather than building it, with the card's screenshots as the regression proof. Second, **`workflow-canvas.tsx` paints its nodes in raw SVG** (`<rect>`/`<text>`/`<circle>`, no `foreignObject`), and `.card-run-glow` is a `background-clip: padding-box, border-box` trick that no SVG element can wear — SVG has no conic gradient either — so painting nodes as `<rect>` would make this phase's one visual requirement impossible with the code that already meets it. Nodes are therefore DOM over an SVG edge layer, with `workflow-geometry.ts` and `workflow-path.ts` reused verbatim. Dependencies come from a four-source ladder — GitHub's own `blockedBy` (verified live on this account, no preview header, no extra scope), a conventionally-named project field, then a prose parse of the issue body — with `parent`/`subIssues` kept as a **separate** containment layer, because merging it would grey out every epic on the board. The whole data change is four fields inside `PROJECT_ITEMS_QUERY`'s existing Issue fragment, so there is **no new IPC channel, no second round trip and no N+1**; and because `blockedBy` returns its nodes' titles and repos, a blocker that is not on the board renders as a foreign node for free. `dagre` was offered and declined at ~90KB for ~40 lines of ranking. 72 items, eight themes, read-only, no new dependency.
...
left-to-right dependency DAG with blockers upstream, blocked nodes greyed, edges animated by the
state of their two ends, and an agent startable from a node the same way it already is from a card.
Modelled on `~/Dev/midnite`'s task graph. Two grounding findings shape it — the kanban card has
already worn the rainbow ramp since Phase 37, so Theme F generalises rather than builds; and
`workflow-canvas.tsx` paints nodes in raw SVG, which `.card-run-glow` cannot survive, so nodes are
DOM over an SVG edge layer. Costs four fields inside a query that already runs: no new IPC channel,
no new dependency, read-only.*

- ◻ **A** — Four fields (`blockedBy`, `blocking`, `parent`, `subIssues`) into `PROJECT_ITEMS_QUERY`'s existing Issue fragment, and onto the issue variant of `ForgeProjectItemContent`.
- ◻ **B** — The three-source ladder (`api` → `field` → `body`) plus a separate containment layer, as a pure resolver in shared: readiness, foreign nodes, `parseBlockerRefs`, the 300-node cap.
- ◻ **C** — Longest-path LR ranking with barycentre ordering and `topAlignedViewport`, pure and unit-testable — `dagre`'s job in ~40 lines.
- ◻ **D** — The canvas: HTML nodes over an SVG edge layer, reusing `workflow-geometry.ts`/`workflow-path.ts` verbatim; `'graph'` on `projectsMode`; four distinct empty states; a legend.
- ◻ **E** — The five blocking-edge appearances, containment edges, body-inferred edges drawn less certainly, blocked dimming, an affirmative `ready` badge, motion-policy and focus gates.
- ◻ **F** — `.card-run-glow` generalised to `.agent-run-glow` so card and graph node wear one ramp off one `useCardStatus` subscription. The card must come out pixel-identical.
- ◻ **G** — Start an agent from a node through Phase 41's existing path, gated on `!blocked`, with the blocker numbers in the disabled tooltip.
- ◻ **H** — Phase 52's shared filter toolbar drives the graph, plus four graph-only facets (hierarchy, blocked/ready only, depth from selection, hide isolated) and the configurable field name.
...
- ✅ **E** (PR #193 + PR #196) — A gated "System" section inside the existing Storage tab with its own empty/loading/error/approximate states, a distinct-accent banner, and a third `blastRadiusKind`. PR #193 landed everything except two items blocked on [Phase 72](phases/phase-72-every-build-systems-leftovers.md) Theme D's generic `SegmentedBar`; Theme D landed in PR #196, which also closed these two out — the section now renders `label="System caches by ecosystem"`, and the `'go'` `ECOSYSTEM_HUES`/`ECOSYSTEM_LABELS` entries were already satisfied by construction (Theme D's exhaustive maps). Storage-tab screenshots against the now-barred section are still owed.
...
- 🔄 **F** (PR #196, partial) — Verification lands 23 of 25 items and all 16 of the shared Verification section's items this batch covers. Two stay genuinely open: `detectors.test.ts` has no assertion pinning `DETECTOR_COUNT`'s literal value (the doc's own "24" is stale — Theme B already corrected it to 28 in prose, but no test enforces either number), and the human pass over a real Rust/Gradle/Python checkout is unautomatable and undone.
