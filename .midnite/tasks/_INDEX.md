...
- ✅ **C** (PR #TBD, 2026-09-06) — SQLite (**M, was S**): six packaging edits — `dependencies` not dev, esbuild `external`, `rebuild-native --only` list, `asarUnpack`, an unpacked-path `require` fallback, and the `verify-dist` native assertion that did not exist. Decision 6's recommended `node:sqlite` was checked against the tree rather than assumed: Electron 33.4.11 bundles Node 20.18.3, which does not have it — so `better-sqlite3` shipped, verified by an actual `rebuild-native.mjs` run that flipped the binary's ABI and back.
