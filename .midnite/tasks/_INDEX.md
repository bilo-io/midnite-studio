...
| [90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md) | 🔄 WIP | — | 46/101 | `█████░░░░░` | 46% | B D E H | G I K |
...
- **[Phase 90 · Multi-forge integration and account switching](phases/phase-90-multi-forge-integration.md)** (46% · 46/101, [PR #490](https://github.com/bilo-io/midnite-studio/pull/490), [PR #491](https://github.com/bilo-io/midnite-studio/pull/491), [PR #501](https://github.com/bilo-io/midnite-studio/pull/501), [PR #503](https://github.com/bilo-io/midnite-studio/pull/503), [PR #502](https://github.com/bilo-io/midnite-studio/pull/502), [PR #504](https://github.com/bilo-io/midnite-studio/pull/504)) — **Themes A, C, F and J landed; B landed 12 of its 13 items; D landed 3 of 5; H landed 3 of 4.** **#504** landed Theme F (Bitbucket Cloud): the adapter over REST 2.0, `main/forge/http.ts` (the provider-neutral HTTP client Theme D deferred), and the phase's first genuinely mixed capability row (`projects: 'none'`, `threadResolution: 'partial'`, otherwise `'full'`) — the first provider to reach Theme H's own deferred `'partial'`-capability item. Two GO-LIVE requirements merged, because they share one provider/credential abstraction: GitHub-equivalent Actions/Issues/Reviews/Projects for GitLab, Bitbucket and Azure DevOps, and an active-user avatar whose switch rescopes the repo list and runs `gh auth switch`. The grounding found the seam is two symbols wide — `githubForge()` and `hasGithubForge` — and that the credential decision is already precedented by `db/credential-vault.ts`. Billing is **out**: a pricing page ships on the website, a tier vocabulary ships unwired. **#503** closed Theme B's `forge.scopeReposToActiveAccount`/`forge.syncGhAuthSwitch` settings and the account-host scheme gap (its one remaining item, the per-provider redaction patterns, is genuinely deferred to Themes E/F/G by the checklist's own wording) and landed all of Theme C: repos hidden rather than closed on a switch (never hiding an org repo without positive evidence from the reachable-repos listing), a GitHub-only clone-or-open picker, a gated `gh auth switch`, and a cache-poisoning fix built as `cancelQueries` + broad `invalidateQueries` rather than an `accountId` segment on all sixteen forge query-key builders — smaller, and clear of Theme D/H's concurrent edits to the same view files. **#502** landed Theme D's adapter seam — `main/forge/adapter.ts`, the moved `gh-*.ts` files behind `createGitHubAdapter()`, and `registry.ts`'s `adapterFor(forge, account)` as `forge-handlers.ts`'s single dispatch point, with a disclosed side benefit: a GitLab/Bitbucket/Azure repo now reports "no forge" instead of misfiring `gh` against the wrong owner/repo — and Theme H's exhaustive `capabilitiesFor` matrix (a compiler-enforced `Record<ForgeKind, ForgeCapability>`) read per-field by `app.tsx`'s `FORGE_GATED_VIEWS`. Found and fixed along the way: `packages/app`'s `"type": "module"` broke every e2e spec's load of `test-support/mock-bridge.ts` the moment it imported a real value (not just types) from the CommonJS `@midnite/studio-shared` build, and that same mock had no `forgeAccounts` namespace at all (the gap #501's own log entry flagged) to answer the new per-view capability read.
