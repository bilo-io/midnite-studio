...
- **[Phase 64 · Offline Monaco Editor & Cross-Surface Theme Engine](phases/phase-64-offline-monaco-and-themes.md)** (0% · 0/72 · **Refined x1**) — **Planned, not started.** Replaces the CodeMirror 6 editor in the Files view with a locally-bundled Monaco, and adds a palette layer across the surfaces the app paints code on. The x1 refinement corrected **four premises**, each one grep. `themeMode` in `ui-store.ts` **does not exist** — light/dark is owned by `@bilo-io/ui`'s `ThemeProvider`, persisted at `localStorage['midnite.theme']`, with **four** modes including a `time` mode the plan never knew about. **There is no CSP** (`Content-Security-Policy`, `onHeadersReceived`, `webSecurity` → 0 hits each, and `index.html` already ships an inline script one would break); the real constraint is the `file://` opaque origin from `loadFile`, which is why workers must be `?worker&inline` — the same reason Shiki's WASM already ships inlined. **Theme D was backwards**: the dispatcher is a capture-phase `window` listener calling `stopPropagation()`, so the app already wins every chord and Monaco never sees one — the work is teaching it to yield, plus three native accelerators (`Cmd+G` find-next, `Cmd+L`, `Cmd+O`) that bypass the yield list entirely. And **an Appearance page already exists** whose first accordion is titled "Theme". It also found a **fifth themed surface** — Shiki, pinned to two GitHub themes by a literal return type, painting the read-only preview this phase deliberately keeps plus diffs and slides, so "Monokai" would have left three code surfaces on GitHub — and a **direct conflict with [Phase 61](phases/phase-61-database-explorer.md)**, which states in writing that it uses CodeMirror *"rather than adopting Monaco or a second editor stack"* and builds on the very file this phase replaces; the new Theme G is gated on it. Decision 12 recommends splitting: honestly refined it is 50 deliverables in two halves that share only that Monaco needs a theme.

- **[Phase 63 · The preferences with nowhere to live](phases/phase-63-settings-diff-and-orphan-preferences.md)** (0% · 0/32 · **Refined x1**) — **Planned, not started.** Four preferences have no settings page: `diffLayout`, `diffShowOldGutter`, `commitFileView` and `changesFileView`. And the two diff controls are **conditionally rendered** — the layout toggle does not exist while you are looking at a binary or deleted file, so you can enter split layout on a text diff, open a binary one, and have no way back until you find another text file. One page, four `Choice` blocks, and an annotated partition of every persisted key into preference vs session state, asserted by a test. **The x1 refinement ran that audit instead of leaving it to be discovered, and corrected three premises.** `PersistedUi` holds **71** keys, not the 77 the doc claimed. **34** of them are named nowhere under `features/settings/` — **25 session state, 9 preferences** — so the "fifth orphan" is not hypothetical and there are **five**: `browserLayout` (the browser page) and `loopChoices`/`loopAgents`/`loopModels`/`loopSchedules`, which all belong in the **agent** page's existing Loops accordion beside their sibling `loopModifierDefaults`. Five exceeds Decision 6's own three-key threshold, so they are recorded rather than built and the phase stays small — but the test now needs a named five-entry `KNOWN_ORPHANS` allow-list, or it fails the day it is written. The refinement also found a **contradiction**: the docblock over the four `*Detached` flags says *"NOT persisted, deliberately"* and explains why a saved `true` would flash a spurious detached placeholder — yet all four are in `partialize` and the v7→v8 `migrate` arm seeds them `false`. Decision 7 fixes the comment, not the code: un-persisting them is a version bump this phase forbids itself.
...
- **[Phase 57 · Midnite Studio speaks MCP](phases/phase-57-mcp-server.md)** (0% · 0/76 · **Refined x1**) — **Planned, not started.** Every agent the app launches runs blind: a `claude`, `codex` or `opencode` process started in one of its terminals shells out for `git status` while `status-handlers.ts` holds the parsed answer, and pages `git log` while the lane layout for those commits sits in git-engine. This phase makes the app an **MCP server** — a build-fingerprinted Unix socket under `userData` plus a bundled stdio shim, cribbing the transport trick the pty broker already proved — serving eight read-only tools (`repo.list`, `repo.resolve`, `status.get`, `graph.log`, `diff.file`, `branch.list`, `forge.pulls`, `forge.checks`). The x1 refinement corrected the phase's central cost claim: tool **inputs** cannot reuse `ipc/schemas.ts` (nearly every request there extends `RepoId`, and an agent knows a path, never an id) — the reuse is on the **output** side, where `StatusResultSchema`, `GraphRow` and `Ref` are already written. It also moved the enable flag into a main-side store (a renderer `localStorage` value is unreadable at `whenReady()`), replaced `fs-scope.ts` path gating with `resolveRepoRoot` + the registry (`joinWithin` refuses absolute paths outright), cut `repo.current` for `repo.resolve` (multi-window has no single focused repo), and pointed `forge.checks` at main's `gh-cli.ts` rather than a renderer module main cannot import. Read-only and off by default on purpose: write tools need the write queue and a real consent model, and are deferred to a follow-up.
...
- ◻ **A** — Vite offline Monaco & worker pipeline: `getMonaco()` lazy singleton in `highlighter.ts`'s shape, workers built `?worker&inline` because a `file://` origin blocks `new Worker(new URL(...))` (Shiki's inlined WASM is the precedent), a `MonacoEnvironment.getWorker` default arm, and `monaco-editor` added to the `MUST_BE_ABSENT` entry-chunk tripwire — 184 KB of budget headroom against a ~2 MB dependency.
- ◻ **B** — Unified cross-surface palette registry: `StudioPalette` over the 22 real `@bilo-io/ui` tokens as HSL triplets (Tailwind wraps them in `hsl()`), six presets, a store persisted in `appearance-store` (which gains the `partialize` and `migrate` it has never had), and **five** surfaces synced — chrome, xterm (16 ANSI keys, net-new), Monaco, Shiki and popouts — reusing the in-place re-theme path that already exists rather than adding a broadcast.
- ◻ **C** — Writable Monaco editor in Files view: same `{fileName}` signature and `data-testid`, wired to the store's ten real actions with dirty left derived, a Shiki-id → Monaco-id translation table, the five editor preferences that do not exist yet (four sites + a `version` 8 → 9 bump), focus restoration on unmount, and the latent missing `key={editorKey}` fixed.
- ◻ **D** — Chord yielding, both directions: `insideTerminal`'s hard-coded `.xterm` becomes a `YIELD_ROOTS` registry with per-root command sets, and the three native accelerators that bypass it entirely — `Cmd+G` (find-next), `Cmd+L`, `Cmd+O` — move to `itemNoAccelerator()`.
- ◻ **E** — VS Code theme JSON importer: a zod-validated result envelope, array-form `scope` flattening (the usual reason a naive importer renders grey), hex → HSL triplet conversion, ANSI fallbacks, and three committed fixtures.
- ◻ **F** — Palette accordion in the **existing** Appearance page (not an 18th settings page), plus the light/dark control that page has never had, and two palette commands registered in all four places a command needs — including the `PALETTE_SAFE` allowlist that fails silently.
...
*`useUiStore` persists 77 keys and every preference cluster among them has a settings page that owns it — except four. `diffLayout`, `diffShowOldGutter`, `commitFileView` and `changesFileView` are reachable only from controls that are conditionally rendered, so the diff-layout toggle does not exist at all while a binary file is open. One page, and one test that stops a fifth from being orphaned.*

- ◻ **A** — The page: `settings-pages/diff-page.tsx` in `sidebar-page.tsx`'s shape, two accordions ("Diff view", "File lists"), four `Choice` blocks whose hints name the constraints the toolbar hides, and a per-accordion reset to the literal store defaults.
- ◻ **B** — Registration, and the one the compiler will not catch: `SettingsPageId`, `SETTINGS_PAGES`, `settings-view.tsx`'s `PAGE_CONTENT` (not `PAGES`), plus the exhaustive `SETTINGS_PAGE_ICON` record — and verifying that the palette and title-bar nav need no edit because both derive from `SETTINGS_PAGES`.
- ◻ **C** — No orphan preference: `store/persisted-keys.ts` partitioning all 77 persisted keys into preference vs session state, each exclusion annotated, typed so a key in neither list fails typecheck, and a test asserting every preference key is named somewhere under `settings-pages/`.
...
- ◻ **F** — Verification (M): fifteen checklist items — the first draft's `## Verification` was prose with **zero** checkboxes, which is how nine deliverables reached a plan unverified. Twelve of fourteen covered by A/B/C/E's own PR #163; the two Theme-D items (Memory tab + kill-path automation, and the human posture check) stay open.
