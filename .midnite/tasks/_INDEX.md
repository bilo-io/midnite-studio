...
- **[Phase 77 · Thirteen megabytes of editor, and the lanes that never move](phases/phase-77-thirteen-megabytes-of-editor.md)** (0% · 0/27) — **Planned, not started.** The performance audit, read against `budgets.json`'s own `_measured` block: total JS is **35.4 MB against a 15.95 MB budget** (2.2×, disclosed by Phase 64 G, unfixed) and `dist/` says where — `monaco-loader` 13.2 MB, `ts.worker` 6.8 MB, a tail of basic-languages the app never opens (`emacs-lisp` 771 KB, `cpp` 767 KB). `monaco-loader.ts:2` imports the package root, which is `editor.main` with every language contribution. **A** switches to `editor.api` + explicit features, a lazy per-language contribution registry (the `highlighter.ts` shape), and an on-demand TS worker, aiming at the *existing* budget; **B** moves the lane layout's O(lanes)-per-row pass-through edges (`lane-layout.ts:154–165`, its own class doc's named bottleneck, Kilo scan #6) out of `edges` into a dense `through: number[]` and draws them as column runs in `graph-svg.tsx`, measured on the 50k fixture with a new `graph-scroll` budget; **C** measures then debounces `ui-store`'s whole-slice `localStorage` write on every `set`; **D** rebaselines only on a documented miss and adds a non-blocking `perf-report` CI comment so a 2.2× breach never again sits unseen for a month.
...
*`budgets.json` already knows total JS is 2.2× over budget; `dist/` says it is Monaco's full
catalogue. The lane layout's own class doc has named its bottleneck since Phase 2. Every theme opens
with a measurement and closes with a before/after table in the PR body.*

- ◻ **A** — Monaco from `editor.api`, explicit features, a lazy per-language contribution registry, an on-demand TS worker — aimed at the existing 15,950 KB budget.
- ◻ **B** — Pass-through lanes leave `edges` for a dense `through: number[]`; `graph-svg.tsx` draws them as column runs; a `graph-scroll` perf budget on the 50k fixture.
- ◻ **C** — Measure, then debounce `midnite-studio.ui`'s whole-slice `localStorage` write with a flush on unload.
- ◻ **D** — Rebaseline only on a documented miss; a non-blocking `perf-report` CI job that comments the bundle table on PRs.
