...
| [53 · The first release](phases/phase-53-first-release.md) | 🔄 WIP | x1 | 33/59 | `██████░░░░` | 56% | F G H | — |
...
- ◻ **F** — v0.1.0 end to end through both skills, installed via `midnite-studio/install.sh` (not the repo root) on a machine with no checkout, with the pre-release "no published release yet" message captured first so the change is proven. Blocked on a human creating `RELEASES_REPO_TOKEN` and running the flow — deliberately left for a human-supervised pass.
