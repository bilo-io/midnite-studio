...
| [78 · Whose hands were on the keyboard](phases/phase-78-whose-hands-were-on-the-keyboard.md) | 🔄 WIP | — | 21/32 | `███████░░░` | 66% | — | E F |
...
- ✅ **H** — Verification: merged-selector, `buildResumeCommand` and both conversation-id-adapter unit tests, notes-migration tests, a keyboard-tooltip view test, visual baselines for the Sessions list and the Notes content pane, and one e2e for the real-xterm live pane. Three human passes remain open — a real Claude resume, a real Codex resume, and a long dual-surface Notes editing session — genuinely un-automatable and left unchecked in the phase doc.
...
- ◻ **E** — Opt-in write side: `MSTUDIO_SESSION_ID`/`MSTUDIO_AGENT_ID` in agent ptys; a never-clobbering `prepare-commit-msg` hook behind a default-off per-repo switch.
