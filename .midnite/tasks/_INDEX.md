...
- ◻ **A** — Shared contracts: the recursive `.passthrough()` Postman v2.1 tree (both `z.lazy` and `.passthrough()` are firsts in `packages/shared`), an editable `ApiRequestDraft` whose `bodies: Record<BodyMode, string>` makes "switch modes without losing a body" structural, `ApiOpResultOf` copying `database.ts`'s two-arm envelope, and seven `mstudio:api-client:*` channels with **kebab verbs** per `channels.ts:6`.
