...
- **[Phase 47 · Conflict Resolution Studio](phases/phase-47-conflict-resolution-studio.md)** (70% · 16/23, [PR #63](https://github.com/bilo-io/midnite-studio/pull/63) + [PR #64](https://github.com/bilo-io/midnite-studio/pull/64) + [PR #103](https://github.com/bilo-io/midnite-studio/pull/103) + [PR #107](https://github.com/bilo-io/midnite-studio/pull/107)) — **Theme D landed** (2026-09-04, PR #107): resolving a conflict no longer means leaving the app. The Studio (`features/conflicts/conflict-resolution-studio.tsx`) opens from a now-clickable path in `ConflictBanner`, in the graph's existing side-panel slot beside `CommitDetail`/`StashInspector`; per-region Accept mine/theirs/both call Theme C's `applyConflictHunk`, file-level Accept-all calls Theme B's whole-file resolve, and a resolved region disappears via the same status-prefixed React Query invalidation every write op and the file watcher already trigger — no bespoke local state. Narrowed from the doc's plan: plain monospace rendering, not `DiffCell`'s virtualization/shiki — Phase 26's two-way `SplitRow` model has no notion of a three-sided region, and stretching it cost more than a new render did. A new read-side IPC (`mstudio:conflict:regions`) was needed, since Themes A–C's `parseConflictedFile`/`readFileDiff` had only ever run main-process-side; its response carries a `truncated` flag, self-review having caught `readFileDiff`'s `DIFF_LINE_CAP` silently dropping trailing regions from a large conflicted file with no signal. A real worktree-targeting bug was also caught and fixed (`useTargetedGitOp` with an explicit target, not the globally-active checkout). On top of **Theme C** (PR #103): `applyConflictHunk`, the phase's own flagged "biggest risk" — a hunk-level `git apply` corrected mid-build to worktree-only, since an unmerged path has no stage-0 entry to patch `--index`/`--cached` against. **Theme B** (PR #64): whole-file accept-ours/theirs/base, proven against both merge and rebase's inverted convention. **Theme A** (PR #63): the marker parser. **The gap left open since [Phase 8](phases/phase-8-drag-drop-ops.md), and declined on purpose by [Phase 26](phases/phase-26-side-by-side-diffs.md)**, whose own "Not in this phase" list named the prerequisite this phase built. **E** (agent-assisted suggestion, reusing Phase 34's council run unchanged) and **F** (wiring + verification) remain open. No manual free-text editing, no `rerere`, no binary/LFS/submodule conflicts — the Studio picks a side, it does not become an editor.
...
| [47 · Conflict Resolution Studio](phases/phase-47-conflict-resolution-studio.md) | 🔄 WIP | — | 16/23 | `███████░░░` | 70% | F | E |
