...
- [ ] **E.4** The rehydrating session wears Theme K's terminal fade rather than flashing an empty canvas; `fitSignal` fires after replay so the first live frame is already sized. **Partially done**: `fitSignal`/`safeFit` already run before any replayed content is written, so sizing is never the problem — but "Theme K's terminal fade" is a forward reference to a `terminalTween`-style per-reveal fade Theme K has not built yet (it exists today only as the whole PANEL's open/close height tween, not a per-session dispose/reveal fade). A revived session can flash a blank canvas for the length of the snapshot round-trip until Theme K lands its fade; recorded in `outstanding.md`.
...
- [ ] **E.6** Number: `memory-report.mjs` with 10 sessions open and 1 visible, before and after, in `done.md`. **Not captured this PR** — needs the packaged-equivalent build (`moon run app:build desktop:bundle`) this session didn't have time to produce; the dispose/rehydrate mechanism itself is unit- and e2e-tested. Recorded in `outstanding.md` as an owed follow-up.
...
- [ ] **F.4** Third-party apps ([Phase 83](phase-83-third-party-apps-rail.md)'s `apps-service.ts`) are excluded from discard by default — Spotify playing in the background is the canonical case — with a per-app opt-in beside their on/off switch. **The exclusion holds today, by construction**: the discard sweep only ever iterates `browser-service.ts`'s own tab map, which apps never enter, so "a Phase 83 app is never discarded by default" already passes with zero code. The per-app **opt-in** itself is deferred (`outstanding.md`) — `apps-service.ts` has no hidden/visible bookkeeping today, so wiring idle-discard into it is a real feature addition, sized more like its own small theme than a follow-up line.
- [ ] **F.5** Number: RSS with 8 tabs open, 1 active, after the threshold — before and after; the discarded tabs' renderer processes are gone from `ps`. **Not captured this PR** — same packaged-build gap as E.6; `browser-service.test.ts` covers the discard/reactivate mechanism itself. Recorded in `outstanding.md`.
...
- [ ] Popout RSS for the Graph role is recorded against J.1's baseline and `budgets.json` has the line. **Half landed in this PR**: `budgets.json`'s `popoutRss: 457500` line exists and is a real measurement (H.4), but J.1's own idle/+10-sessions/+8-tabs/+1-popout baseline table — Theme J's item, not built here — has not landed in this doc yet, so there is nothing to record the popout number "against" yet; this box stays open until J.1 lands.
