...
- [x] `app.tsx` — `NAV_ITEMS` gains a `files` entry (react-icons, e.g. `LuFolderTree`) **above**
...
- [x] `store/ui-store.ts` — `ViewId` gains `'files'`; new persisted
      `settingsPage: 'appearance' | 'graph' | 'terminal' | 'agent'` (+ setter);
      `pathForView`/`viewForPath` keep round-tripping
- [x] `features/settings/settings-view.tsx` — becomes a shell: slim page list on the left
...
- [x] `settings-pages/terminal-page.tsx` — hosts whatever terminal preferences exist at build
...
- [x] `ui-store.test.ts` — `settingsPage` persisted, `viewForPath('/files')` round-trips,
...
- [x] `desktop/src/main/ipc/fs-handlers.ts` — the jail: join scope root + relPath, `realpath`
...
- [x] `git-engine` — ignored-flag pass for `fs:list-dir` via one batched
...
### C — Folder explorer view (M) ✅ DONE (merged to main, 2026-08-26)

- [x] `features/files/files-view.tsx` — new `activeView === 'files'` branch in `app.tsx`;
...
- [x] `features/files/file-tree.tsx` — lazy: a directory lists on first expand, never before.
...
- [x] File-type icon map (react-icons — new icons follow CLAUDE.md's react-icons rule) small
      and extension-keyed; directories get open/closed chevron affordances matching the repos
      sidebar's `TreeSection` styling
- [x] Selection (`selectedPath`, expanded set) in a `features/files` zustand store,
      non-persisted, reset on repo switch; the tree follows the same active-repo/worktree
      selection the graph uses
- [x] Refresh: re-list open directories on repo/worktree switch plus a manual refresh button.
      Wiring into Phase 10's watcher invalidations is a stretch item, not required

### D — Preview pane (L) ✅ DONE (merged to main, 2026-08-26)

- [x] Add `shiki` using the fine-grained core (`shiki/core` + lazily imported grammars keyed
...
- [x] `features/files/preview/code-preview.tsx` — highlighted read-only text with line
...
- [x] Fallback card for `binary` / `too-large` / unloadable: file name, kind, human size — an
      info card, no escape hatch to the OS (guarded `shell:open-external` is Phase 12 E's)
- [x] Preview header: filename, size, language badge, markdown toggle. The component takes a
      **content descriptor, not a path** — the seam that later lets Phase 12's inspector mount
      it against a blob-at-commit
- [x] No edit affordance anywhere: no contentEditable, no context-menu rename/delete —
      read-only is a feature of this phase, not an omission

### E — Agent settings page (M) ✅ DONE (merged to main, 2026-08-26)

- [x] `settings-pages/agent-page.tsx` — `~/.claude` browsed read-only with Theme C's tree
      against Theme B's `claude-home` scope (skills, projects, plans, settings — the folder
      structure as-is, lazy like the repo tree)
- [x] `agent:claude-info` channel — main runs `claude --version` through a **login shell**
...
- [x] Version card: installed version, detected install method, a refresh button, and a clear
      "not installed" state that offers the install command instead
- [x] **Update** runs directly (hybrid decision): main spawns the method-matched update
      ...
      a throw
- [x] **Uninstall** never auto-runs: the button opens the terminal panel (Phase 15) and pastes
...
- [x] `claude-info` degradation: `claude` missing, `--version` output unparseable, or the
...
- [x] `fs-handlers.test.ts` — the jail holds: `..`, absolute paths, and out-of-root symlinks
      all rejected in both scopes; binary sniff and size cap behave
- [x] git-engine test — `check-ignore` batching round-trips names containing newlines and
      spaces (the NUL rule is why)
- [x] `ui-store.test.ts` — new persistence assertions from Theme A
