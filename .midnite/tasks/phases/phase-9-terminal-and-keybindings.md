...
- **The renderer could not import a *value* from `shared`.** `packages/shared` emits CommonJS
  ...
  kind to find late. Vite, vitest and the app's tsconfig now all resolve `@midnite/studio-shared` to
...
- **`app.setName('midnite-git')`** — Electron takes the name from package.json, so the menu bar,
  the About dialog and `~/Library/Application Support` all read `@midnite/studio-desktop` without it.
