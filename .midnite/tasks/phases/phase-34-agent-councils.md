...
- [ ] `Council`, `CouncilMember`, `CouncilRun`, `CouncilRunMember` zod schemas in a new
...
- [ ] Every write channel returns the `GitOpResult`-style discriminated envelope
...
- [ ] `packages/desktop/src/main/councils-store.ts`: `createCouncilsStore(directory)` with
      ...
      roster).
- [ ] A separate `councils-runs-store.ts` (or a second key in the same store — decide during
...
- [ ] Unit tests: round-trip a council + members through the store, one malformed entry does not
...
### C — Run orchestration (M)

The highest-risk theme — the auto-send exception and the settle-barrier both live here.

- [ ] `packages/desktop/src/main/council-runner.ts`: `startRun(councilId, prompt)` spawns one
...
- [ ] The **settle barrier**: once every member's session has exited (or been explicitly skipped),
      ...
      phase doesn't ship.
- [ ] A per-member and per-synthesis timeout (config value, following `CouncilsConfigSchema`'s
      `runTimeoutMs` precedent) that settles a hung member as `timeout` rather than blocking the
      barrier forever.
- [ ] `skipMember(runId, memberId)`: kills that member's session and settles it `skipped` so the
...
- [ ] Output capture reuses the broker's existing scrollback/ring-buffer mechanism rather than a
...
- [ ] Preload exposes `window.midniteStudio.council.{create, list, get, updateMembers, remove}` and
      `.run.{start, get, retryMember, skipMember}`, following the existing `bridge()` shape other
      features use — no new bridging pattern.
- [ ] Main-process handlers wire Theme A's channels to Theme B's store and Theme C's runner,
      translating `council-runner.ts` failures into the `GitOpResult` envelope at the boundary.
- [ ] `use-council.ts` / `use-council-run.ts` hooks in `packages/app`, modelled on the shape of
...
- [ ] `packages/app/src/features/councils/` becomes a real feature folder; the `councils` case in
      `view-sections.ts` stops resolving to `WORK_IN_PROGRESS` and renders this feature's list view.
- [ ] A council list (name, description, member count, last-run time) with an empty state for zero
      councils.
- [ ] A minimal create modal — name + optional description only, matching upstream's
      `council-create-modal.tsx` — members are added afterward on the detail page, not at creation
      time.

### F — UI — detail & members panel (M)

- [ ] Council detail view: a members panel (add/remove/edit inline — name, provider `<select>`
      restricted to the 3-agent pool, role `<textarea>`) and a synthesizer picker (same 3-agent
      pool), no drag-reorder for this phase.
- [ ] A bottom-pinned topic composer: a prompt input and a Run button (no format picker — brainstorm
      is the only format, so the control has nothing to select between yet).
- [ ] The auto-send safety-exception note (see the framing section above) rendered as a small inline
      line near the composer, visible before the first run.

### G — UI — run view (M)

- [ ] Per-member tabs: while `running`, embed the existing xterm terminal component
...
- [ ] A synthesis tab: a waiting state while members are still running (with each pending member
      named), a live terminal once the synthesizer starts, then the rendered synthesis on completion.
- [ ] Run-thread rail: a list of past runs for the open council, prompt preview + status pill +
      relative time, selecting one loads it read-only — the smallest version of upstream's
      `council-run-thread.tsx`.

### H — Retry/skip controls (S)

- [ ] A skip button on a still-running member's tab, calling Theme C's `skipMember`.
- [ ] A retry button on a `failed`/`timeout` member's settled tab: re-reads the member's *current*
...
- [ ] Boundary lint clean: `council-runner.ts` and the stores stay in `packages/desktop`; `shared`
...
- [ ] Vitest (Theme A/B): schema round-trips, one malformed council/member entry does not cost the
...
- [ ] Vitest (Theme C): the settle barrier fires once every member is `succeeded`/`failed`/
...
- [ ] Playwright (`e2e/councils.spec.ts`): create a council, add a member, run against a mocked
...
- [ ] Screenshot, per the visual-phase convention: the council list, the detail view with the
      members panel open, and a run mid-flight with one member tab live.
- [ ] **Open, for a human:** run a real council (2–3 members, real `agy`/`codex`/`opencode`
      installs) end to end and confirm each member's command actually executes without a manual
      Return, and that the synthesis reads as a coherent distillation rather than a concatenation.
- [ ] **Open, for a human:** confirm the auto-send note on the composer reads as reassuring rather
      than alarming — this is a judgment call worth a second pair of eyes before it ships.
...
- **Open — where run/member output is persisted.** Theme B sketches a second store
  (`councils-runs-store.ts`) versus a second key in the same file; either is fine, decide during
  implementation based on which keeps `councils-store.ts` closest to `agents-store.ts`'s existing
  shape. Recommendation: separate file — a long-running council accumulates run history much faster
  than the roster accumulates agents, and mixing the two invites one giant JSON file with very
  different write frequencies for its two halves.
- **Open — per-member/synthesis timeout default.** Upstream carries this as
  `MidniteConfig.councils.runTimeoutMs`; no strong opinion on the number itself, just that it should
  live in the equivalent shared config location here rather than a hardcoded constant in the runner.
