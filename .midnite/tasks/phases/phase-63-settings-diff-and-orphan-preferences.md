...
- [x] Add [`packages/app/src/features/settings/settings-pages/diff-page.tsx`](../../../packages/app/src/features/settings/settings-pages/diff-page.tsx)
      ...
      `<div className="p-3">`.
- [x] Two accordions, not one: **"Diff view"** (`icon={<LuDiff className="h-4 w-4" />}`, `defaultOpen`)
      ...
      hide that.
- [x] `diffLayout` as `<Choice<DiffLayout> label="Layout" value={diffLayout} onChange={setDiffLayout}
...
- [x] `diffShowOldGutter` as a `Choice` over `on`/`off` (not a bare switch), because
...
- [x] `commitFileView` as `<Choice<FileView> label="Commit files" …
      ...
      silently change one.
- [x] A **"Reset to defaults"** button per accordion, writing the literal defaults from
...
- [x] One `useUiStore` selector per value and one per setter — never a single selector returning an
...
- [x] `diff-page.test.tsx` in the shape of
...
- [x] Add `| 'diff'` to the `SettingsPageId` union at
      [`ui-store.ts:139`](../../../packages/app/src/store/ui-store.ts).
- [x] Add `{ id: 'diff', label: 'Diff', group: 'general' }` to the `SETTINGS_PAGES` array at
...
- [x] Add a `diff` entry to the `PAGE_CONTENT` record at
      ...
      wrong.
- [x] Add a glyph to `SETTINGS_PAGE_ICON` at
...
- [x] Confirm and record that **no edit is needed** in
...
- [x] Add `packages/app/src/store/persisted-keys.ts` — **new.** Export two `readonly` string-literal
...
- [x] Type the partition so it cannot rot: assert
...
- [x] Add `persisted-keys.test.ts`: every `PREFERENCE_KEYS` entry is named in at least one file under
...
- [x] Seed `SESSION_STATE_KEYS` from the x1 audit rather than from scratch. Of the 34 keys with no
...
- [x] Put the remaining **9 preferences** in `PREFERENCE_KEYS`. Four are this phase's own
...
- [x] **The five new orphans will make `persisted-keys.test.ts` fail the moment it is written** —
      ...
      `outstanding.md`.
- [x] Correct the `*Detached` docblock at
...
- [x] Where the test fails on a key **other** than the four this phase fixes and the five in
...
- [x] `Settings ▸ Diff` appears in the settings nav, in the command palette, and in the title-bar
      settings nav — all three, without any of the three being edited.
- [x] Each of the four controls shows the store's current value on open, and changing it in Settings
      is visible in the corresponding surface without a reload.
- [x] Changing `diffLayout` **from Settings while a binary file is open** takes effect — the case the
      toolbar cannot serve, and the reason the page exists.
- [x] "Reset to defaults" restores `false` / `'unified'` / `'tree'` / `'list'`.
- [x] `localStorage`'s `midnite-studio.ui` is still `version: 8` after using the page — no migration,
      no new key.
- [x] Removing a key from `PREFERENCE_KEYS` and `SESSION_STATE_KEYS` while leaving it in
      `PersistedUi` fails typecheck. Check by hand once, then revert the probe.
- [x] `persisted-keys.test.ts` passes, and deleting `diff-page.tsx` makes it fail — otherwise the
      test proves nothing.
- [x] `PREFERENCE_KEYS` has **9** entries and `SESSION_STATE_KEYS` has **25** of the 34 audited keys;
      the two together cover all **71** `PersistedUi` keys with no overlap and no gap.
- [x] `KNOWN_ORPHANS` has exactly five entries — `browserLayout`, `loopChoices`, `loopAgents`,
      `loopModels`, `loopSchedules` — each with a matching line in
      [`outstanding.md`](../../../.midnite/tasks/outstanding.md).
- [x] The `*Detached` docblock no longer claims the flags are unpersisted, and `partialize`,
