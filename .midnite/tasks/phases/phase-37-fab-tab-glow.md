...
- [ ] Declare the ramp once on `:root` in
...
- [ ] Replace all five verbatim copies in `styles.css` with `var(--rainbow-N)` references —
...
- [ ] Add a `--rainbow-ramp` shorthand holding the whole seven-stop list, so a plain
      ...
      omit some.
- [ ] Leave `@bilo-io/ui`'s `tokens.css` alone. Its two copies are upstream and out of scope;
...
- [ ] Add `relative` to the `className` in
...
- [ ] Add `.fab-panel-gradient::before` in
...
- [ ] Paint it with a conic gradient sharing the border's rotation:
...
- [ ] Mask it to the edge with a radial alpha mask — this is what makes the falloff smooth and
...
- [ ] Add `@keyframes fab-glow-pulse` animating **`--fab-glow-inner` and `opacity` only** —
...
- [ ] Run both animations on the pseudo-element: `animation: fab-panel-spin 4s linear infinite,
      fab-glow-pulse 3.2s ease-in-out infinite`. Deliberately co-prime-ish periods so the two
      do not visibly re-sync every cycle.
- [ ] Tune blur radius, mask stops and pulse amplitude by eye against both themes. The written
...
- [ ] Set `data-fab-tab={activeFabTab}` on the gradient div in
...
- [ ] Register the two arc endpoints: `@property --fab-arc-from` and `@property --fab-arc-to`,
...
- [ ] Add the four-row arc table in `styles.css`. **180° spans centred on each tab's own ramp
...
- [ ] Build the arc gradient so it has no seam: the lit span runs from `--fab-arc-from` to
...
- [ ] Apply the arc to **both** layers — the border's `conic-gradient` in `.fab-panel-gradient`
      and the glow's in `::before` — from the one pair of vars.
- [ ] Sweep between tabs: `transition: --fab-arc-from 0.5s ease, --fab-arc-to 0.5s ease` on
...
- [ ] Check the wrap-around cases by hand. `medic` (270° → 90°) crosses 0°, and a naive
...
- [ ] Give the collapsed FAB button the active tab's arc. It wears
      ...
      consumers.
- [ ] Make `.loop-run-glow`'s conic honour `--fab-arc-from`/`--fab-arc-to` when they are set
...
- [ ] Drive a `data-loop-state` attribute on the gradient div from the existing
...
- [ ] Map it to pulse cadence in CSS only — `idle` ~4.0s and low amplitude, `running` ~2.6s,
      `thinking` ~1.8s with a brighter peak. One keyframe, three durations, no new keyframes
      per state.
- [ ] **`waiting` overrides the arc entirely**: a steady amber glow (`#f59e0b`), rotation
      ...
      the one you need."*
- [ ] Decide and document what the glow shows while the *active* tab is idle but **another**
...
- [ ] Add `html[data-motion='reduced']` rules for the new surfaces:
...
- [ ] Suppress the 0.5s arc sweep too under reduced motion (`transition: none`). The glow keeps
      the active tab's colour and swaps instantly — identity survives, movement does not.
- [ ] Confirm the pulse holds at a *sensible* frame when stopped. With the animation removed,
...
- [ ] Extend [`fab-loops.spec.ts`](../../../packages/app/e2e/fab-loops.spec.ts) — it already
...
- [ ] Assert the arc actually changes: click each of the four tabs and read
...
- [ ] Add before/after screenshots per tab to
      [`fab-loops-shots.spec.ts`](../../../packages/app/e2e/fab-loops-shots.spec.ts), in both
      light and dark.
- [ ] Re-run [`panel-glow.spec.ts`](../../../packages/app/e2e/panel-glow.spec.ts) deliberately.
...
- [ ] Check the idle cost. The panel is a persistent blurred-window animation, which is the
      ...
      If the blurred cost is material, gate the pulse on window focus.
...
- [ ] Theme A changes **nothing visible**: screenshot-diff the FAB panel, the breadcrumb pill, a
...
- [ ] Open the FAB panel: a soft rainbow light hugs the inside edge, fades to nothing well
      before the centre with no visible inner rim or banding, and breathes slowly.
- [ ] Click through all four tabs: the glow **and** the border narrow to that tab's half of the
...
- [ ] The narrowed arc reads as the tab's colour at a glance from normal viewing distance —
      "the green one" — while still visibly being a gradient rather than a flat ring.
- [ ] Collapse the panel with a tab active: the FAB button carries the same arc, so the colour
      does not jump.
- [ ] Start a loop: the pulse quickens. Drive it to a prompt awaiting input: the glow goes
...
- [ ] `html[data-motion='reduced']`: no rotation, no pulse, no sweep — and the glow still shows
...
- [ ] `panel-glow.spec.ts` still green — popovers mount at their intended coordinates.
...
- **Does the transition on a registered custom property inherit to the pseudo-element, or must
  it be declared there?** Custom properties inherit; whether the *transition* fires on the
  originating element or the pseudo-element depends on where the changing property is read.
  Theme C carries an item to determine this empirically and comment the answer rather than
  guess.
- **Wrap-around interpolation for `medic` (270° → 90°).** Whether Chromium sweeps the short way
  between arbitrary angle pairs, or whether the table needs unwrapped negative values to force
  it. Theme C item; decide by observation.
- **Does the blurred-window idle cost justify a focus gate on the pulse?** Deliberately left to
  the measurement in Theme F rather than pre-decided — Phase 36's rule is that a perf claim
  comes with a number.
