...
- **Resolved — the one thing the v6 delta really did break, found the hard way.** The `.d.ts`
  diff above is honest and the source port genuinely needed no edits, but v6 added
  `InputHandler.requestMode` — the DECRQM handler, `CSI ? Ps $ p` — and its `const enum V`
  survives into `lib/xterm.mjs` as a runtime enum object that nothing reads
  (`let r; ((P) => (P[P.NOT_RECOGNIZED = 0] = …))(r ||= {})`). Rollup rewrites the `||=`, esbuild's
  minifier then substitutes `void 0` for the read, drops `let r` as unused, and **keeps the
  write** under a name declared nowhere. A bundled ES module is strict-mode code, so the packaged
  renderer threw `ReferenceError` on every mode query it received, the throw escaped
  `WriteBuffer._innerWrite`, and the write loop never rescheduled: one DECRQM froze that pane for
  good. `agy` (Bubble Tea v2) queries modes 2026 and 2027 in its first hundred bytes, so it read
  ...
  unit tests, the dev server and the whole e2e suite all run unminified, where the line is exactly
  as inert as it looks — so `vite-xterm-decrqm-plugin.test.ts` runs a real minified build of a
  one-line entry that imports xterm, and asserts on the bytes.
