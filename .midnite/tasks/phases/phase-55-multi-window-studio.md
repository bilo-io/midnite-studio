...
### Theme A — Window Lifecycle & IPC Infrastructure (L)
...
- [ ] **A.1** Define window management contracts in [`packages/shared/src/domain/window.ts`](../../../packages/shared/src/domain/window.ts) *(new)*:
...
- [ ] **A.2** Create [`packages/desktop/src/main/window-manager.ts`](../../../packages/desktop/src/main/window-manager.ts) *(new)*:
...
- [ ] **A.3** Add `resolveWindow(event.sender)` and convert **only the four handler families a popout actually needs** — `pty`, `browser`, `watch`, `menu`:
...
- [ ] **A.4** Teach a popout renderer its own role via `additionalArguments`, not a URL query:
...
- [ ] **A.5** Persist popout geometry in a new main-process store [`packages/desktop/src/main/windows-store.ts`](../../../packages/desktop/src/main/windows-store.ts) *(new)*:
...
- [ ] **A.6** Window lifecycle and quit semantics:
...
- [ ] **B.1** Top-left hover morph dock/undock affordances. All four imitate one existing pattern — [`terminal-session-list.tsx:350–367`](../../../packages/app/src/features/terminal/terminal-session-list.tsx), where a fixed-size box holds an `aria-hidden` glyph at `absolute … transition-opacity group-hover:opacity-0` and an `IconButton` at `opacity-0 transition-opacity group-hover:opacity-100`. The box keeps its size in both states, which is what makes the morph free of layout shift:
...
- [ ] **B.2** Create `DetachedWindowFrame` in [`packages/app/src/components/detached-window-frame.tsx`](../../../packages/app/src/components/detached-window-frame.tsx) *(new)*:
...
- [ ] **B.3** Commands and keyboard access, in [`shared/src/keybindings.ts`](../../../packages/shared/src/keybindings.ts):
...
- [ ] **B.4** The uniform re-dock strip — one affordance, four surfaces:
...
- [ ] **B.5** Palette wiring for all five commands:
...
- [ ] **C.3** Per-`ptyId` subscriber registry in [`pty-service.ts`](../../../packages/desktop/src/main/pty-service.ts) — **the fix that makes C.1 possible**:
...
- [ ] **D.3** Re-docking, and the all-tabs rule:
...
- [ ] **D.4** Make tab activation window-scoped:
