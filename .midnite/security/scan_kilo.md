...
### 1. Module-level global listener never removed
...
### 2. write-queue.ts never evicts finished repo chains
...
### 4. O(n²) deduplication in status and changes panels
...
### 7. Palette re-attaches global keydown on every scored result change
...
### 8. use-browser-tabs.ts calls bridge() three times per effect run
