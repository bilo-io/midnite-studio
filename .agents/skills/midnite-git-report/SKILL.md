...
## 1 · Pick the window
If `$ARGUMENTS` already names a period (`today`, `yesterday`, a `YYYY-MM-DD`, `this-week`, `this-month`, or a `START..END` range), use it. Otherwise **a direct question to the user** — one question, options **Today · Yesterday · This week · This month** (Other = a specific date or `START..END`).
...
For a week/month window, or if the user asks for a visual, offer to build a small self-contained HTML page with a bar chart of PRs (and lines changed) per day plus a phase-completion chart. Otherwise the unicode bars suffice — they render in the terminal, whereas mermaid does not.
...
A bundled, data-driven template lives at `.agents/skills/midnite-git-report/pdf-template.html` — all three tables render from one `DATA` object, so you only edit that block (no hand-written rows). Steps:

1. Copy the template into your scratchpad: `cp .agents/skills/midnite-git-report/pdf-template.html "$TMP/git-report-$END.html"`.
...
4. Share the PDF file (the path) alongside the chat report; if that's unavailable, print the absolute path.
