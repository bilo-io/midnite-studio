---
name: midnite-setup
description: Bootstrap a brand-new project repo from scratch — ask ~10 questions about the stack and product, then scaffold a moon-style repo with a README, the onboarding kit (`.claude`/`.agents`/`.codex` skills, tailored CLAUDE.md/AGENTS.md/GEMINI.md, subagents), and a `.midnite/tasks/` tracker with one connected MVP phase doc per app.
---

**Invoke with:** [optional: a project name or one-line description, and/or a target directory]

You are running the **setup** workflow: an interactive, human-in-the-loop session that **bootstraps a brand-new project repository** from nothing. You ask the user **~10 questions** — first about the **repo shape + stack**, then (the majority) about the **product's context** — and then scaffold the repo so it's immediately workable with the `midnite-brainstorm` and `midnite-exec` skills.

This skill is modelled on how **midnite-studio** itself is laid out (moon + proto monorepo, `.claude`/`.agents`/`.codex` skill mirrors, a `.midnite/tasks/` phase tracker) — and it emits **exactly** [`templates/midnite/`](../../../templates/midnite/), the same checked-in onboarding kit the app's own Setup menu leaf copies into a repo (Phase 49 Theme A). One layout, one source of truth: this skill and the app must never describe two different shapes of "an onboarded repo" again. The output is a *seed*, not a finished product — one MVP phase doc per app, connected where the apps depend on each other, ready for `/midnite-exec` to pick up.
...
- The **target** is a *new* repo, not midnite-studio. Never write into the midnite-studio tree except to *read* [`templates/midnite/`](../../../templates/midnite/), the tree you copy out. Resolve the target dir in Stage 0.
...
- The skills you copy assume a `.midnite/tasks/` tracker (`_INDEX.md`, `phase-N-*.md`, `done.md`, `outstanding.md`) and a `CLAUDE.md`/`AGENTS.md`/`GEMINI.md` trio of conventions — `templates/midnite/` already carries the skeleton of both, so Stage 5 is filling those in, not inventing the shape from scratch.
...
**b. README.md** — at the repo root. Cover: what the project is, the stack, the repo layout, common commands (moon/pnpm if monorepo), and a **"Working with Claude Code"** section that **documents the `midnite-brainstorm` and `midnite-exec` skills** (what each does, when to use it, the `.midnite/tasks/` flow they drive) and the generated subagents.

**c. The onboarding kit** — copy [`templates/midnite/`](../../../templates/midnite/) **verbatim** into `<target>/`, exactly as it is in this repo: this is the one step that keeps this skill and the app's own Setup menu leaf from describing two different shapes of "an onboarded repo." That single copy gives the target:
- `<target>/.midnite/` — the tracker skeleton (`settings.json`, `tasks/_INDEX.md`, `tasks/done.md`, `tasks/outstanding.md`, an empty `tasks/phases/`, `_features.md`), filled in below.
- `<target>/.claude/skills/`, `<target>/.agents/skills/`, `<target>/.codex/skills/` — three identical mirrors of the same eight repo-agnostic skills (`midnite-brainstorm`, `midnite-exec`, `midnite-exec-adhoc`, `midnite-refine`, `midnite-address-issue`, `midnite-triage`, `midnite-git-report`, `midnite-git-cleanup`), copied as-is — **keep the `midnite-` prefix on every one of them**, matching the template exactly; do not rename or strip it.
- `<target>/CLAUDE.md`, `<target>/AGENTS.md`, `<target>/GEMINI.md` — stubs with `<!-- TODO -->` placeholders for the toolchain, package boundaries, and house conventions. Fill each placeholder in for *this* stack — project overview, the chosen apps' one-way dependency graph, code style, testing layers, the "shared is the contract" rule if there's a shared package — keeping every one of the three files identical outside the parts that are genuinely agent-specific, per the sync rule already written into the stub. Don't discard the stub and write from scratch; the sync-rule and worktree-policy sections it already carries are the point of starting from it.
...
**d. `.midnite/tasks/` phase docs** — the template ships the tracker's skeleton empty; this stage fills it in:
- `_INDEX.md` — replace the empty `## Phases` table with one row per phase you're about to write, plus a **Headlines** bullet per phase (the `midnite-exec`/`midnite-brainstorm` house style: what it is, why it exists, the design decisions worth knowing before opening the doc).
- **One phase doc per app**: `.midnite/tasks/phases/phase-1-<appA>.md`, `phase-2-<appB>.md`, … in the **house style** the `midnite-brainstorm` skill describes: `# Phase N — <App> MVP`, framing blockquotes (what it builds on + scope guardrails + an S/M/L legend), **Themes** with `- [ ]` checklist items + S/M/L tags, a **Files this phase touches** map, a **Verification** checklist, and a **Decisions / open questions** section. Fold the Stage 2 context into the right app (pricing/legal/socials/i18n/theming/look-and-feel land in the website/webapp docs; API contracts in the API doc).
...
- `done.md` and `outstanding.md` stay as the template's empty skeletons — there is nothing to log yet.
...
3. Tell the user the next move: **`cd` into the repo, run `pnpm install` (if monorepo), then `/midnite-exec`** to pick up the first MVP slice — or `/midnite-brainstorm` to add a new phase.

---

Be genuinely collaborative through Stages 0–4 — the interview is the point; the scaffold is mechanical. Don't write a single file until the user has confirmed the blueprint (Stage 4), and tailor every generated file to their answers — never emit midnite-studio-specific content (its own package names, its own conventions) that doesn't apply to their project. `templates/midnite/`'s own stubs and placeholders are what keep that boundary honest — fill them in, don't paper over them with copied specifics.
