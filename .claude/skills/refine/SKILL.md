...
`/brainstorm` invents a phase. `/exec` builds a slice of one. **`/refine` sits between them** and is
...
- **This is doc-only work and lands on `main`** — like `/brainstorm`. Do not ask the worktree
...
column. **Refining a theme another `/exec` loop has claimed will collide** — flag any such theme and
...
- **Tag every option** the way `/exec` does — a single dominant-nature tag plus effort:
...
Doc-only, source-of-truth change — same landing path as `/brainstorm`. No PR.
...
3. Report, terse: the doc path · `Refined: xN` · items `before → after` per theme · the opens
   resolved · the commit/push result · and the one line that matters — **what a `/exec` run can now
