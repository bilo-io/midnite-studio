...
> **Scope guardrails**: macOS arm64/Apple Silicon distribution focus. Delivers a production-grade DMG installer with background layout, system `midnite-git` CLI tool installer + shell completion, custom `midnite-git://` deep-linking protocol scheme, auto-updater pipeline, and first-run setup onboarding. Windows `.msi` and Linux `.deb`/`.rpm` packages are explicitly deferred.
...
### Theme B — `midnite-git` CLI Binary & System PATH Symlinking (S/M/L: M)
- [ ] Create standalone executable shell wrapper script `packages/desktop/resources/bin/midnite-git` forwarding arguments (e.g. `midnite-git .` or `midnite-git open <path>`) to the running desktop app via IPC socket or `midnite-git://` protocol.
- [ ] Define IPC channels in `packages/shared/src/ipc-channels.ts`: `CLI_CHECK_STATUS`, `CLI_INSTALL`, `CLI_UNINSTALL`.
- [ ] Implement main process handlers in `packages/desktop/src/main/ipc/cli-handlers.ts` to manage `/usr/local/bin/midnite-git` or `~/.local/bin/midnite-git` symlinks.
- [ ] Generate shell completion scripts (`zsh`, `bash`, `fish`) for `midnite-git` commands and options in `packages/desktop/resources/completions/`.
...
### Theme C — `midnite-git://` Custom Protocol Handler & Single-Instance Dispatcher (S/M/L: S)
- [ ] Call `app.setAsDefaultProtocolClient('midnite-git')` in Electron main process (`packages/desktop/src/main/index.ts`).
...
- [ ] Implement protocol URL parser (`packages/desktop/src/main/protocol.ts`) supporting `midnite-git://open?repo=<path>` and `midnite-git://clone?url=<repo-url>`.
...
- [`packages/desktop/electron-builder.yml`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/electron-builder.yml)
- [`packages/desktop/package.json`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/package.json)
- [`packages/desktop/src/main/index.ts`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/src/main/index.ts)
- [`packages/desktop/src/main/ipc/cli-handlers.ts`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/src/main/ipc/cli-handlers.ts)
- [`packages/desktop/src/main/protocol.ts`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/src/main/protocol.ts)
- [`packages/desktop/src/main/update-service.ts`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/src/main/update-service.ts)
- [`packages/desktop/resources/bin/midnite-git`](file:///Users/bilolwabona/Dev/midnite-git/packages/desktop/resources/bin/midnite-git)
- [`packages/shared/src/ipc-channels.ts`](file:///Users/bilolwabona/Dev/midnite-git/packages/shared/src/ipc-channels.ts)
- [`packages/app/src/features/settings/cli-settings.tsx`](file:///Users/bilolwabona/Dev/midnite-git/packages/app/src/features/settings/cli-settings.tsx)
- [`packages/app/src/features/settings/updates-settings.tsx`](file:///Users/bilolwabona/Dev/midnite-git/packages/app/src/features/settings/updates-settings.tsx)
- [`packages/app/src/features/onboarding/first-run-modal.tsx`](file:///Users/bilolwabona/Dev/midnite-git/packages/app/src/features/onboarding/first-run-modal.tsx)
- [`packages/app/src/features/statusbar/statusbar.tsx`](file:///Users/bilolwabona/Dev/midnite-git/packages/app/src/features/statusbar/statusbar.tsx)
...
- [ ] `moon run desktop:dist` builds valid `midnite-git-0.1.0-arm64.dmg` and `.zip` packages in `packages/desktop/release/`.
- [ ] Installing CLI tool creates executable symlink in `/usr/local/bin/midnite-git` and `midnite-git .` opens current folder in desktop app.
- [ ] Deep link `midnite-git://open?repo=...` focuses app window and opens target repository.
...
1. **CLI Binary Privileges**: Symlink target attempts `/usr/local/bin/midnite-git` first; if permission denied, prompts user to install in `~/.local/bin/midnite-git` (with instructions to add to `PATH`).
