...
- ◻ **E** — a `markdown.presentAsSlides` `CommandId` in `COMMANDS`, unbound, with a
  `useCommandHandlers()` arm following the existing reactive `{enabled, disabledReason}` shape.
