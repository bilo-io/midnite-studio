...
import { createAzureAdapter } from './azure/create-azure-adapter';
 ...
 *
 * Returns `null` for a forge kind with no adapter — only `unknown` now that
 * Theme G lands the last of the three HTTP-backed providers — so a
 * repository on an unsupported host reports "no forge" honestly instead of
 * a GitHub adapter running `gh` against the wrong owner/repo, which is what
 * happened before Theme D (Theme A widened `isSupportedForgeKind` to all
 * four ahead of any adapter existing to serve them). A GitLab, Bitbucket or
 * Azure repo with no account, or one whose token failed to load, resolves to
 * `null` from *within* that adapter's own calls (each read/write reports
 * `cli.reason: 'not-authenticated'`) rather than from this dispatch point —
 * `resolveAdapter`'s caller in `forge-handlers.ts` only ever sees "no
 * adapter" for a kind with none built yet.
 */
export function adapterFor(forge: Forge, account: ForgeAccount | null): ForgeAdapter | null {
  const exhaustive: Record<Forge['kind'], ForgeAdapter | null> = {
    ...
    azure: createAzureAdapter(account),
    unknown: null,
  };
