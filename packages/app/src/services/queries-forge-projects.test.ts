...
describe('useSetProjectItemField', () => {
  it('is not optimistic and invalidates the board\'s items on success', async () => {
    const bridge = (window as unknown as { midniteStudio: Bridge }).midniteStudio;
    bridge.forgeProject.setField.mockResolvedValue({ ok: true, kind: 'ok' });

    const { client, wrapper } = withClient();
    const invalidate = vi.spyOn(client, 'invalidateQueries');
    const { result } = renderHook(() => useSetProjectItemField('PVT_1'), { wrapper });
    ...
    expect(bridge.forgeProject.setField).toHaveBeenCalledWith({
      projectId: 'PVT_1',
      itemId: 'item1',
      fieldId: 'f1',
      value: { fieldId: 'f1', dataType: 'text', text: 'hi' },
  ...
  it('a refused write never throws, and never invalidates', async () => {
    const bridge = (window as unknown as { midniteStudio: Bridge }).midniteStudio;
    bridge.forgeProject.setField.mockResolvedValue({
      ok: false,
      kind: 'error',
