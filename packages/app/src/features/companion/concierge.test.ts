...
import { greet, matchRepoByName, orient, say } from './concierge';
import {
  digestFixture,
  fakeConciergeDeps,
  fakeSpeaker,
...
describe('greet', () => {
  it('runs greeting → overview → digest → prompt, in that order', async () => {
    ...
    expect(lines[1]).toBe('You are in midnite-studio, on main.');
    // The digest's own sentences, from `summariseDigest`.
    expect(lines.some((line) => line.includes('the browser occlusion fix'))).toBe(true);
    expect(lines[lines.length - 1]).toBe('What would you like to do?');
  });
...
describe('orient — the switch offer', () => {
  it('offers a switch only when more than one repo is open', async () => {
    ...
    expect(many.transcript.map((t) => t.text)).toContain('Want to switch to another one?');

    const one = fakeStore();
    await orient(fakeConciergeDeps({ store: one }));
    expect(one.transcript.map((t) => t.text)).not.toContain('Want to switch to another one?');
  });

  it('does not greet — that is what greet() is for', async () => {
    const store = fakeStore();
    await orient(fakeConciergeDeps({ store }));
    expect(store.transcript[0]?.text).toBe('You are in midnite-studio, on main.');
  });
