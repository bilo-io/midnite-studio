...
export const WindowRelayMessage = z.object({
  ...
  /*
    The last four are page-selection slices (Theme H, plus `sessions` in
    Phase 67 Theme F) — which run Actions has open, which file the Explorer
    has open, which tabs the Changes workbench holds, which closed session
    Sessions has open. They exist because a PAGE popout duplicates rather
    than moves, so the same view can be live in two windows and its selection
    is the one thing that then visibly drifts. `broadcast-sync.ts` is the
...
//
// `runId` is generated by the renderer (a fresh id per Run click), exactly as
// `requestId` is for `apiSendRequest` — it is what `apiCancelRun` and the two
// `EVENT_CHANNELS` entries below correlate against, and what lets the store
// tell a superseded run's late events apart from the current one.
// `environmentId`/`runAnyway` mirror `ApiSendRequestRequest`/`ApiRunScriptRequest`
// exactly, for the identical reasons named on each of those.

export const ApiRunCollectionRequest = z.object({
  runId: z.string().min(1),
  repoId: z.string().min(1),
  collectionId: z.string().min(1),
  environmentId: z.string().nullable(),
...
export const ApiRunCollectionResponse = ApiOpResultOf(ApiRunStartOutcomeSchema);

export const ApiCancelRunRequest = z.object({ runId: z.string().min(1) });
/** An abort on an unknown/already-finished `runId` is a no-op `{ok:true}`,
 *  the same race `apiCancelRequest` already treats as normal. */
export const ApiCancelRunResponse = ApiOpResultSchema;
