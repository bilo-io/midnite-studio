...
export const TestimonialCard = ({ testimonial }: TestimonialCardProps) => {
  ...
  return (
    <GlowCard bare className="h-full">
      {/*
        A real `<figure>` wrapping the quote and its attribution, because that
        is the only structure in which a `<figcaption>` means anything —
        a bare `<figcaption>` after a `<blockquote>` is invalid, and the
        attribution is exactly what the element is for. The screenshot gets its
        own nested `<figure>`, which is allowed and is the right nesting: it is
        a figure *within* the quote, with its own caption naming the channel.
      */}
