 ...
 *
 * **Provenance.** Anthropic's mark, the same artwork the desktop app renders at
 * `packages/app/src/components/icons/claude-icon.tsx` — copied rather than
 * imported, because the marketing site shares no runtime with the app and the
 * eslint boundary forbids reaching across it. "Claude" and its logo are
 * Anthropic's trademarks, used nominatively to name an agent the app can run.
 *
 * `currentColor` on the fill is what lets the marquee tint it with the agent's
 * own brand colour from `../agents.ts` instead of a hard-coded orange.
 */
