 ...
 *
 * **Provenance.** From [lobe-icons](https://github.com/lobehub/lobe-icons) (MIT),
 * the same artwork the desktop app renders — copied rather than imported, because
 * the marketing site shares no runtime with the app. "Antigravity" and its logo
 * remain Google's trademarks, used nominatively to name an agent the app can run.
 *
 ...
 *
 * **The one API difference from its siblings.** The others fill with
 * `currentColor`, so the marquee can tint them. This mark carries its own colours
 * and cannot be tinted — a multi-colour brand logo has no single colour to
 * override. `style` is still accepted and applied, so a caller mapping over a
 * list of icons needs no special case; it simply has no visible effect.
 *
 * The mask and filter ids are scoped with `useId`, because ids in an inline SVG
 * are document-global and the marquee renders three copies of the whole roster.
 */
