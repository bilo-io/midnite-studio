...
**Invoke with:** [--apply] [--prune-remote] [--target <branch>] [--no-remote]
...
```bash
.codex/skills/midnite-git-cleanup/cleanup.sh
```
...
then **a direct question to the user** to confirm — one question, options along
the lines of *Delete local + remote · Local only · Cancel*. Only then:

```bash
.codex/skills/midnite-git-cleanup/cleanup.sh --apply                 # local only
.codex/skills/midnite-git-cleanup/cleanup.sh --apply --prune-remote  # local + origin
```
